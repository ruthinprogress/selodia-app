...
| A2 | Do the vertical masters exist with `_Female` variants, or are they male-only? (and the green-screen set) | **Resolved 2026-08-21 without needing to ask — Ruth checked the Verticals folder directly and confirmed a normal male/female mix, consistent with the rest of the library.** The earlier signal (zero `_Female` in header scans of the truncated archives — 0 of 58 and 0 of 26) was a **sample artefact of where the incomplete downloads happened to stop**, not a structural gap. It was raised as a signal rather than a finding, and checking the source beat asking the vendor. Clears the dependency for both the **4:5 portrait slot** (which sources from vertical masters) and the **female-only content rule**. *Scope note: the 57%-female measurement taken from the subfolder downloads is **Green Screen** content (3840×2160 landscape, correctly labelled — there was no mislabelling), so it evidences Green Screen female coverage rather than Vertical. This row rests on the direct Verticals check.* | 2026-08-21 |
| A3 | Is the source background consistent library-wide? (newer samples on white, one older clip on green screen) | **Resolved 2026-08-21 in-house — no vendor reply needed.** Sampled a mid-clip frame from **500 of the 2,592** extracted FULL HD 1080P clips and read the four corners (the figure is centred, so corners are background by construction): **487 pure white, 13 outliers, ZERO green-screen.** Every outlier keeps at least one white corner — twelve are near-white greys in the 197–237 range (soft shadow), and one is a dark object reaching into frame (a medicine ball on a rotational throw). None is a different background. **Conclusion: the main library is consistently white, and the green-screen clip seen earlier came from the separate `1200+ GREEN SCREEN VIDEOS` package** — two deliberate product variants, which is why they ship as separate downloads. Unblocks the LUT's background mapping. | 2026-08-21 |
