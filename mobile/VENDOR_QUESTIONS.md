...
| 6 | **Do the vertical masters exist with `_Female` variants, or are they male-only?** And the same question for the green-screen set. | **Live risk, raised by real evidence.** Scanning the local file headers of the (truncated) VERTICAL and GREEN SCREEN archives found **zero `_Female` files** — 0 of 58 and 0 of 26 respectively — where the main HD library runs about 30% `_Female`. The sample is small and drawn from the start of each archive, so this is a signal rather than a finding. But if it holds, it breaks two settled decisions at once: the **4:5 portrait slot** specifies sourcing from vertical masters (Part Ten), and the **female-only content rule** (2026-08-21) excludes unmarked male figures — so male-only verticals would leave no valid source for the demo slot. Confirm before any batch recolour is planned around vertical masters. | 2026-08-21 |
