...
## Database
Supabase (Postgres). **Twenty-one tables, counted against the live database on 2026-09-08** rather than maintained by hand: `activity_logs`, `allergies`, `almanac_entries`, `body_measurements`, `chat_messages`, `cycle_events`, `daily_activity_summaries`, `daily_summaries`, `food_items`, `food_logs`, `health_context`, `hydration_logs`, `interpretation_notes`, `personal_metrics`, `push_tokens`, `reminder_settings`, `user_context`, `user_profile`, `waitlist`, `workout_completion_log`, `workout_weight_log`. *(This line said "Eleven tables" until 2026-09-08 and had been wrong for weeks — a hand-maintained list of a thing the database already knows. `body_measurement_custom_metrics` was in that list and has since been dropped, replaced by `personal_metrics`. If this count is being read for anything load-bearing, query `information_schema.tables` instead of trusting the number.)* **Row Level Security is enabled across all of them, with per-user policies** — re-verified 2026-09-08 by checking `relrowsecurity` and the policy count on every table, which returned no table with RLS off and none with zero policies — switched on as part of real authentication (Part Four), which is exactly what it was waiting for. Any new table ships with RLS and its own policies in the same migration; never enable RLS without policies in place, as that locks out the app's own access entirely.
...
**`api.selodia.app` is live and is what the app calls (corrected 2026-09-08).** This paragraph read "Nothing points at it yet, and nothing in the build references it" until then, which stopped being true on 3 September when the custom domain was added and `EXPO_PUBLIC_API_URL` was pointed at it — a correction recorded in the very next bullet, while this sentence above it was left standing. Two adjacent lines disagreeing about whether a domain is in use is the same defect class as the tagline typeface, and it is recorded rather than quietly deleted for that reason.

**What is true now:** `selodia.app`, `www.selodia.app` and `api.selodia.app` are all aliased to the production Vercel deployment, the app calls the API host, and the Vercel project has been renamed `selodia-app`. `unflump.app` (primary), `unflump.com` and `unflump.online` (defensive) remain registered via Namecheap. **The rest of the cutover — the route rename, the package identifiers, the EAS project — still belongs to the code/infrastructure rename session**, and the outstanding list for it lives in `WORKFLOW.md` under RENAME STATUS, not here.
...
- Real authentication (Phase 1 step 5) — done: Supabase Auth wired for real in `account.tsx` (email/password `signUp` and Google OAuth via `expo-auth-session` + `expo-web-browser`), `emailRedirectTo` resolved per-platform rather than relying on a single dashboard Site URL, RLS enabled with per-user policies across all 8 tables then existing (11 now — `food_items`, `health_context`, and `almanac_entries` each shipped with their own policies), the 26 pre-existing single-tenant rows backfilled to the first real account, and date of birth/biological sex migrated from `auth.users` metadata into `user_profile` (their permanent home) whenever a session is available immediately at signup. That gap — a user who signs up without an immediate session, confirms later, and reopens the app with nothing to sync their data at that point — is now closed by step 6's fill-if-missing metadata sync, which carries DOB/biological sex from `auth.users` metadata into `user_profile` on reauth (never overwriting existing values). **GOOGLE SIGN-IN HAS NEVER WORKED — corrected 2026-09-12.** The code in `account.tsx` is sound, but the Google provider was never enabled in Supabase: `/auth/v1/settings` reports `google: false`, and Ruth's attempt on 12 September was refused with `provider is not enabled` (`validation_failed`). Email/password is the only sign-in that has ever worked. Enabling it is configuration, not a build: a Google OAuth web client under the `selodia-app` Google Cloud project, its ID and secret entered in Supabase, and the redirect allow-list checked. When the dev variant is built, `selodiadev://` needs adding to that allow-list too, because `makeRedirectUri()` resolves to the variant's own scheme. **ENABLED AND VERIFIED the same evening, 2026-09-12.** Ruth set up the Google Auth Platform (External, Testing, one test user) and a Web OAuth client under `selodia-app`, and switched the provider on in Supabase. She signed in with Google on her phone and landed in her existing account: Supabase linked a `google` identity to `unflumpapp@gmail.com` by email, and no duplicate account was created. **Before beta:** in Testing only listed test users can use Google sign-in, so the consent screen needs publishing. Google's screen reads "continue to jwyzpkxcaxdnjkykoahn.supabase.co" rather than Selodía, which needs brand verification or a custom auth domain. And there is no Terms of Service page yet (selodia.app/terms is a 404). **No password reset exists anywhere in the app** either, found the same day: someone who forgets their password has no way back in except Google.
...
- **Interpretation layer — surfaced (2026-08-24, item 38 slice 2).** The gap above is closed: `measurement-context.ts` gathers the five inputs and `reading-interpretation.tsx` renders the composed message at the top of the Measurements segment. Ten days unsurfaced, from 14 to 24 August. The layer's *own* code needed no change to reach a screen — nothing was ever wrong with it, only unreached — which is why the drift was invisible until the scope audit went looking. What is surfaced is the **live** reading interpretation; the persisted per-entry note (Part Nine, Persisted Interpretation Notes) is still unbuilt, and is tracked on item 38.

- **Feature screens — all three destinations built (corrected 2026-08-31).** Chat is built (real free-text logging and Q&A, step 10a). **The Body tab has all four segments** — Overview, Food, Measurements and Activity, each with its own view component and all four wired into the `dashboard.tsx` switcher. **The Almanac screen is built** — empty state, entry list and detail view, plus the chat-routed edit affordance; it is no longer an exception to principle 8. Everything else from Part Five onward still describes what needs building. *(The previous version of this line said the Body tab had two of four segments and the Almanac was "a bare title". Both were true on 21 August and neither survived the following ten days.)*
- **Known polish item (tracked, not blocking):** the resource card (`resource-card.tsx`, shown for eating-related-distress/acute-crisis moments per `SELODIA_LANGUAGE_RULES.md`) doesn't currently read as clearly tappable in the chat UI. Worth revisiting once real visual design work happens, per the cosmetic-can-wait principle — noted 2026-08-12 so it isn't forgotten.
...
- **Almanac data layer + write path built (2026-08-17, item 15):** migration `…181730` creates `almanac_entries` (open-text `kind`, JSONB `content`, check-constrained `status`, instance count, last-confirmed date; RLS `manage_own`); `app/lib/almanac.ts` holds `prepareAlmanacEntry` (validation/coercion, node-tested) and `saveAlmanacEntry` (returns null on non-save/failure, so a save is never falsely claimed); and `ask-unflump` gained a classify-tool save hook that persists on emit, with confirm-first enforced at the prompt level. **The Almanac screen was subsequently built** (UI slices 1-3, corrected 2026-08-31): `almanac.tsx` fetches active entries and renders `almanac-empty-state.tsx`, `almanac-list.tsx` and `almanac-detail.tsx`; detail renders by content *shape* rather than by `kind`, so an unfamiliar entry still opens readably (principle 13); and "Update this" routes into Chat with a prefill rather than adding a direct-edit path, preserving the chat-only-writer rule. **Still outstanding on the Almanac:** the lifecycle mechanism (the `status` / `instance_count` / `last_confirmed_at` columns exist and are typed, but nothing ever writes a transition, so the screen's `status = 'active'` filter is currently a no-op), category filtered views, the first-view introduction, the empty state's sage line-illustration, and consumption of saved insights by the interpretation layer.
...
- **Native-module verification debt (flagged 2026-08-21; largely CLEARED, updated 2026-09-08).** **Four successful Android development builds have shipped since**, read from `eas build:list` rather than recalled: `c0bbc84e` (3 Sept), `40225246` and `74eb995d` (4 Sept), and `955d35da` (4 Sept, 21:24), the last of which linked the LiveKit WebRTC modules for voice. One errored and one was cancelled on 3 September before those succeeded. **The stack this paragraph warned about was broken deliberately and has stayed broken up.** What follows is the original 21 August entry, kept because its reasoning is the standing rule rather than a status: the last recorded successful build *at that time* was **2026-08-09**. Since then three native modules were added (`react-native-health`, `react-native-health-connect`, `@react-native-community/datetimepicker`, all 2026-08-10) and a `minSdkVersion 26` fix landed (2026-08-12) *in response to a failed Gradle build*. **None has been verified on-device**, and no successful build is recorded since the failure. An Android development build was triggered 2026-08-21 to break that stack deliberately — verifying the health permission and the date picker in isolation **before** the remaining four native additions (image-picker, animation library, notifications, voice) join the queue. This is the exact stacking Development Workflow Principle 2 exists to prevent.

- **NATIVE BUILD BATCH — SHIPPED, NOT YET VERIFIED (opened 2026-09-01; status corrected 2026-09-08).** All four are now *in* a build on a device. **None is confirmed working**, and the distinction is the entire reason this list exists: shipping was never the bar. Items 1-3 rode the 3-4 September builds; item 4's manifest half is fixed and its other three defects are untouched. Read the sub-entries for what each one still needs.
  1. ~~**Push notifications** — `expo-notifications`, added 27 August. The JS is built and inert until a native build ships.~~ **VERIFIED ON A DEVICE 2026-09-12**, preview build `1291e191`: permission prompt, first token ever, and a test push delivered and seen. Shipping it took five fixes, not a build: two setup and two code on 10 and 12 September, plus the reminders wiped by the 10 September reinstall. See Part Fourteen, Daily Reminder. The first of this batch to clear the bar the batch exists for.
  2. **Seed Mark app icon** — the branded icon does not reach a device without a rebuild.
  3. **react-native-keyboard-controller** — **installed at 1.21.9 and `KeyboardProvider` is at the root** (verified 2026-09-08); the pin held, npm's 1.22.4 was correctly not taken. **Still outstanding: `KeyboardAwareScrollView` on the two form screens**, and on-device confirmation that the durable fix behaves at least as well as the ScrollView-owns-its-own-insets change (`9551b64`, `d527309`) it is meant to replace — which is confirmed working, so this one is an improvement with a working fallback rather than a fix under pressure.
  4. **Health permissions — three defects, found 1 September.** These were thought to be why step tracking has never worked. **They are not the whole of it (corrected 2026-09-12; see STEP TRACKING below this batch).** The first is a manifest fault no amount of JS can route around:
     - ~~**`android.permission.health.READ_STEPS` is not declared anywhere.**~~ **FIXED — verified in `app.json` on 2026-09-08.** `android.permissions` now declares `health.READ_STEPS`, `health.READ_DISTANCE` and `health.READ_EXERCISE` alongside `RECORD_AUDIO`. The original diagnosis stands as a record of *why*: the `react-native-health-connect` config plugin adds **no** `android.permission.health.*` entries — it wires the rationale activity and nothing else, and its own README says the app must declare them itself. So `requestPermission()` was asking for something undeclared, Android returned an empty array, and `step-permission.ts` read that as a decline. **Saying yes could not succeed**, which is exactly what device testing found. **The three defects below are still open**, so the manifest fix alone does not make step tracking work.
     - ~~**`getGrantedPermissions()` is never consulted.**~~ **CODE FIXED 2026-09-09, NOT YET VERIFIED ON DEVICE.** `requestPermission()` returns only NEWLY granted permissions, so a second run where access already exists returned empty and was misread as a decline — the app losing an access it already had, and writing down a refusal that never happened. `step-permission.ts` now asks `getGrantedPermissions()` **first**, so somebody who has already said yes is never prompted twice, and asks **again as a fallback** if the request returns empty. The granted list is the authority; the request's return value is not. A throw from either call is logged and treated as “not known to be granted” rather than as a refusal.
     - ~~**iOS is the mirror image: it always reports granted.**~~ **CODE FIXED 2026-09-09, NOT YET VERIFIED ON DEVICE.** `initHealthKit`'s callback error is set only on genuine failure — Apple deliberately never reveals whether READ access was allowed, so that an app cannot detect a refusal and pester — so a decline was recorded as success while the app never received any step data. Nobody reported it because it failed silently, which is worse than the Android one failing loudly.

       **The fix adds a fourth state, `'unknown'`, because on iOS the honest answer is often that nobody can tell.** This one cannot be fixed by reading the answer more carefully, because there is no answer. After `initHealthKit` succeeds the app now **asks for seven days of step samples**: data in hand proves access, and no data proves nothing, since somebody who genuinely did not walk looks identical to somebody who declined. A week rather than a day, so a quiet morning does not read as a refusal. **`'unknown'` is never written to `steps_permission_declined`** — that column exists to stop the app re-prompting somebody who said no, and suppressing a retry on a guess would permanently disable step tracking for somebody who had in fact allowed it.
     - ~~Secondary, and JS-only: `'unsupported'` and `'declined'` share a single fallback message.~~ **FIXED 2026-09-09.** A phone with no Health Connect at all was told it might own a tracker that does not sync to its health app — an answer to a question nobody asked. There is now **one line per outcome**. The declined line is unchanged and still quoted verbatim in Part Seven, step 4. Unsupported says plainly that the phone has no health app to read from, because there is no decision to reconsider. `'unknown'` promises nothing and asks for nothing, which is all that is honestly available when Apple will not say.

  **All four need the build to VERIFY, not merely to ship.** Nothing in this batch should be marked done on a green typecheck; that distinction is the whole reason this list exists separately from the build items. **As of 2026-09-08 the builds exist and the verification does not** — which is precisely the state this sentence was written to make visible rather than let pass as done.

  **Status 2026-09-12: item 1 is VERIFIED on a device. Items 2 to 4 are not.**

  **STEP TRACKING HAS NEVER WORKED, AND PERMISSION WAS NEVER THE WHOLE REASON (found 2026-09-12). LEFT AS IS, by decision.** Nothing in the app reads step data. `step-permission.ts` asks for access, and no code anywhere calls `getStepCount` or `readRecords`. `overview-panel.tsx` says so in a comment and deliberately shows no steps rather than a false zero. `activity_logs` has never held a row from the phone: every entry is manual text or a workout plan. The permission is asked only during onboarding (`equipment.tsx`), with no way to change the answer afterwards, and Ruth's profile has `steps_permission_declined = true`, which may be the old false-decline defect rather than a real no. **A fix needs:** a reader that pulls daily steps from Health Connect and HealthKit, somewhere to keep daily totals, the Activity square showing them, a way to change the permission answer, Samsung Health set to share with Health Connect on the phone (Ruth's side), and verification on a device. About half a day plus testing; the native modules are already linked, so it may not need a rebuild. **Ruth's decision, 2026-09-12: note it and leave it. The Almanac comes first.** **Update, the same day: step reading is back in scope.** The Movement answers make automatic step data one of Movement's four daily logging sources (Part Ten, Movement answers, 7), so it is built as part of Movement rather than parked.

  **A LIVE FALSE CLAIM, left as is by the same decision.** `app/lib/app-structure.ts` tells the chat model, under WHAT THE APP CAN DO THAT IS NOT A SCREEN: "STEP TRACKING. The app can read step counts from the phone's own health platform." It cannot. Claude added that line in Session 41, reading the permission work as meaning the feature worked. The same prompt forbids mentioning anything unbuilt, so the model will tell someone their steps are being tracked. Harmless while Ruth is the only user. **It must be corrected before any tester**, whether or not step reading is built by then.

  **Status 2026-09-09: all four are fixed in code and NONE is verified on a device.** That is deliberately not the same sentence as “done”, and this paragraph exists so the difference cannot be skimmed past. The changes are JS-only against native modules already linked into the build, so they reach a phone by EAS Update without a rebuild. **What device testing has to establish, and what no typecheck can:** on Android, that a second run no longer re-prompts and no longer records a decline; on iOS, that granting yields `'granted'` once step data exists and refusing yields `'unknown'` rather than a silent success; and that a phone without Health Connect now gets the unsupported line rather than the watch one.

- **KNOWN GAP: there is no test suite, and one comment claims otherwise (found 2026-09-01).** `app/lib/onboarding-openers.ts` says its duplication across the Next/Expo boundary is "kept honest by a parity test rather than by discipline". There is no such test — the repository contains **no test files at all**. The comment was written describing an intention, and reads today as a guarantee that does not exist, which is worse than saying nothing: someone changing an opener would reasonably trust it.

  The duplication it describes is real and now **three-way**, not two: the intro opener is quoted in `PHASE_OPENERS`, in the `OPENING_LINE` constant in `mobile/src/app/onboarding/intro.tsx`, and *verbatim inside `INTRO_ROLE`* in `app/api/onboarding-chat/route.ts`, which tells the model what greeting the person was shown. That third copy is the one most likely to rot silently, because nothing renders it and no screen looks wrong when it is stale — the model simply believes a question was asked that was not.

  Not fixed here, deliberately: writing the first test in a project is its own piece of work (choosing a runner, wiring it to both packages, deciding what else deserves coverage) and does not belong inside a copy change. The comment now carries the correction and the full list of what moves together. **A parity test over `PHASE_OPENERS` is the obvious first test to write** whenever test infrastructure is added.
...
**The Chat composer (2026-08-16 mockup).** The Chat screen has an input row: a terracotta **"+" button**, a text input pill ("Message…"), and a **mic button**. Tapping "+" opens an action sheet with three options — **Take a photo · Choose from library · Choose a file** — the entry point for **image-based logging**, which is broader than food: a food photo, *or* a photo of equipment or a screen (e.g. a treadmill display after a run, a nutrition label, a scale readout). Image logging routes through the existing photo parse paths (`parse-food`, `parse-body-measurement`; item 10b's `expo-image-picker`). The **mic button is voice input for logging** — confirmed a real feature (2026-08-16): speak a food or activity entry instead of typing, with the transcription fed into the same text pipeline. **Interaction is deliberately simple (2026-08-17), modelled on a familiar existing pattern (explicitly Claude's own voice input) — nothing elaborate:** tap the mic → the button changes colour and pulses while listening → speech converts to text → the transcribed text lands in the input field, **fully editable**, and is sent like any typed message. **No separate confirmation screen, and no special mishearing handling** beyond ordinary text editing. A native-dependency addition (mic + speech-to-text) in the native-module batch; not yet built (see build item 34).

## Screen Structure
Three top-level destinations, deliberately kept to three rather than more, accessed via small icon-only navigation (no text labels). **Order set 2026-09-03, left to right: Chat, Body, Almanac** — the journey the product describes, which is do, then measure, then understand. Body previously sat third, which put the tab opened most often furthest from the thumb and broke that reading:
- **Chat** — the home screen, where the app opens. A speech bubble icon.
- **Body** — logged data across food, body, and activity. **Label decided 2026-08-16 as "Body," not "Dashboard"** (the earlier working name); the person-outline silhouette icon is unchanged — chosen because it represents food, body, and activity together as facets of one body, not a chart icon that would read as clinical analytics rather than personal data. **The in-screen switcher was replaced by a nested stack on 2026-09-03.** It was one screen holding four views in local state behind a row of segment pills, and the row did not fit a phone at "Measurements" length — it had to scroll sideways, which is how Activity spent a while clipped off the right edge and looking unbuilt. The tab now opens on a summary screen and the three detail views are real routes beneath it. Pressing the Body tab from inside a detail screen returns to the summary, which is standard platform behaviour and needed no implementing: it is what a Stack inside a tab already does. **These remain review surfaces for what has already been logged — logging itself always happens conversationally in Chat, never via entry forms on these screens.**
...
- **Food** — today's calories and protein, each against its target where one exists. Heading taps through to the Food detail.
- **Body** — the latest weight, muscle, and body fat %, each with its week-over-week movement. Heading taps through to Measurements. **Body fat is shown only when a bioimpedance reading supplies it** (2026-09-03), and omitted entirely otherwise rather than rendered as a dash or a zero: plenty of scales weigh and nothing more, and an empty percentage reads as a missing measurement rather than a scale that never takes one. A 0% reading would be alarming nonsense. **Not waist/thigh/hip**, which the Cut note below already ruled intrusive on a screen someone opens every day.
- **Activity** — how much has been logged today, as a count of sessions and total minutes. Heading taps through to the Activity detail. No longer omitted: the Activity screen exists, so principle 8 no longer applies to it.
- **Hydration** — the day's total with the quick-tap beside it. **No heading, deliberately**: it is the one thing on this screen you can DO rather than a view to go into. Explicitly not gamified.
...
- **Everything else** — a second table, stacked directly under the weekly scale table on the same continuous screen, both visible together with **no toggle and no separate tab** (confirmed 2026-08-27). This supersedes the earlier *waist / thigh / hip cards*, and the 2026-08-16 reason for their placement still binds: these numbers stay in a view someone chooses to open, never on the default landing, because surfacing them on every open reads as intrusive.
  - **Why split by source.** Scale metrics and everything else come from genuinely different sources and different habits — someone can stop using a scale and keep tape-measuring, or the reverse. A single combined table would leave **permanent empty cells** for whichever source they stopped. Stacking rather than switching means the full picture still reads at a glance.
  - **Genuinely open-ended, not "tape-measured".** Waist, thigh, blood pressure, resting heart rate, or anything else the person chooses — which is what Part Eight already promised ("any personally meaningful point, not limited to a preset menu").
  - **Transposed: one row per METRIC, not per day.** The scale table works as day-rows × fixed-metric-columns because a bioimpedance scale emits all three numbers at once on a regular cadence. These have neither property — waist monthly, resting heart rate daily, blood pressure occasionally, each logged independently. A week grid with a column per metric would be mostly empty and worsen with every metric added: **the empty-cell problem the split exists to solve, rebuilt one level down.** A metric appears only once it has data, so there are never empty cells, and the table grows downward, which is the direction a phone scrolls. Columns: Metric / Latest (with how long ago, beneath) / Change.
  - **Ordering is chronological by first-logged.** Any other rule — recency, size of change, notional importance — would be the app making an editorial judgement about which of someone's metrics deserves the top row. It also keeps the table stable: a metric never jumps position because of what a reading did.
  - **Change is absolute, in the metric's own unit** ("−1.5 cm"), never the "% 7d" the scale table uses. That percentage exists because three metrics share adjacent columns and only a percentage reads down them together; here each row is its own unit, and an absolute change is both more honest and something you can picture. A **paired reading gets no change at all** — which of two numbers moved is the whole question, and one arrow cannot answer it without picking a side. No previous reading shows an em dash, never a zero: "not measured before" and "no change" must never look the same.
  - **Data (`personal_metrics`, built 2026-08-27).** Standalone: own `user_id`, own `measured_at` (so backdating works), open `metric_name`, `value`, `value_secondary` for paired readings like blood pressure, and the `unit` **as the person said it**. Weight can be canonicalised because its unit space is known; an unbounded metric space has no equivalent, so the unit is carried rather than silently guessed. This **replaced `body_measurement_custom_metrics`**, whose `metric_name`/`value`/`unit` triple was right but whose `measurement_id NOT NULL` tied every entry to a scale reading — making a waist measured on a non-weighing day unstorable, which is exactly the assumption this overturns. It held 0 rows and had 0 code references (both verified against the live database), so it was dropped rather than adapted: its name asserted a relationship that no longer exists.
  - **Metric-name normalisation is done at log time**, with the names this person already tracks handed to the parser so it reuses them. Without it, "waist", "my waist" and "Waist" become three rows — the open-ended equivalent of a misclassification, and invisible until the table looks wrong. This is principle 13's approach, not a downstream keyword list.
  - ~~**Not yet built:** correcting a personal metric conversationally.~~ **BUILT — corrected 2026-09-09, and it was built on 2026-08-28.** `CorrectionKind` includes `personal_metric`, the classify tool's enum offers it, and `ask-unflump` gives it a dedicated branch: a personal metric is corrected **by metric name, not by “the most recent row”**, because somebody who logged a waist and a thigh a minute apart and says “no, the waist was 71” means the waist while the most recent row is the thigh. The claim that "no, waist was 71" logs a second reading has been false for twelve days.
...
- "Ask about this" reposts the card at the bottom of the live chat thread rather than scrolling back to the original entry — this keeps the interaction feeling present rather than like reopening old mail. The conversation can go anywhere from there — a nutrient question, saving the entry as a recipe to the Almanac, or wherever the person takes it — never a fixed form.
- **Mechanism, resolved 2026-08-21 (build item 30):** the card is carried into the thread **as an image** — a genuine shared visual reference that both the person and Unflump can see. This is deliberately not a hidden context injection where the model is handed structured data the person can't see: an image posted into the thread means both parties are demonstrably looking at the same thing, which is what makes "this" unambiguous in a conversation about it. It also means the reference stays visible in the scrollback exactly as it appeared at the time, consistent with the point-in-time treatment of Persisted Interpretation Notes.
- **The opening line stays simple and genuinely open — resolved 2026-08-21:** Unflump opens with **"What's on your mind?"**, the same way regardless of which entry type was tapped. Not a scripted opener tailored per entry type, and not a guess at why they tapped: the card already establishes *what* is being discussed, so the words only need to establish that the floor is theirs. A tailored opener would narrow the conversation toward whatever Unflump assumed the question was — the same directive-phrasing trap Part Two principle 4 rules out.
...
**The knowledge half is built (2026-08-26).** `app/lib/app-structure.ts` holds an `APP_STRUCTURE_PROMPT_BLOCK` — a plain description of the three tabs and what is genuinely on each screen — injected into `ask-unflump`'s system prompt. It exists because the absence of it was actively harmful, not merely a missing nicety: with no knowledge of its own app, the model answered "how do I clear this chat" by inventing a settings path ("usually a 'clear chat' or 'new conversation' option near the top of the screen... it might be under your account or app settings"), sending a real person hunting for controls that do not exist. The block therefore carries a hard **never invent a control** rule alongside the description, and describes only what is on screen today — which satisfies the fourth-wall rule at the same time, since anything unbuilt is simply not mentioned. **This block is the single place to update when a screen ships or changes**; a stale description fabricates as effectively as no description. The *spotlight* half (Part Six) shipped 2026-08-31 as Tier A (item 23): Unflump can now highlight any of 18 in-screen elements, and still explains the way in words whenever the route runs through a tab icon, which cannot be pointed at. **The block was also found badly stale that day** — it had gone on asserting there was "no settings screen, no account screen" for five days after item 41 shipped one, and instructing Unflump never to point at a settings path for a data-removal request that now has one. A stale negative is worse than a stale omission: an omission makes Unflump quiet about something real, a false negative makes it deny a control the person is looking at. The maintenance rule in the file now records that it was broken and how, since the rule by itself evidently was not enough.
...
**KNOWN PLATFORM CONSTRAINT — the tab icons cannot be spotlighted, established 2026-08-31 and not a gap to be worked around.** The tab bar is `NativeTabs` from `expo-router/unstable-native-tabs`, which renders a genuine native `UITabBarController` on iOS and `BottomNavigationView` on Android. Its typings expose **no ref, no `onLayout` and no frame API** to JavaScript — the only hook shipped is `usePlacement`, for bottom accessories. So JS cannot measure where a tab icon is, and a React Native overlay rendered inside a screen cannot draw over the native bar. A transparent `Modal` can cover it, but that only yields a dim: it cannot make a native icon glow through a cut-out whose position is unknown. The geometry is not even stable within a session, since `tabBarMinimizeBehavior` is wired through and the iOS 26 bar floats and shrinks on scroll, so a hardcoded "three icons, evenly split, 49pt plus safe area" hole would land wrong on exactly the devices being tested.

The consequence is that Layer 1 splits in two, and only one half is buildable: **in-screen elements can be spotlighted; tab icons cannot.** For a cross-tab question — "where is my Almanac", "how do I get back to today" — **Unflump continues to explain in words**, which it already does well via the app-structure block. **A synthetic affordance was considered and rejected**: pulsing a button the overlay invented instead of the real tab icon is "here is a control I made", not "look, it is down there", and it undercuts the entire premise of showing rather than telling. Replacing `NativeTabs` with a JavaScript tab bar would buy the full design at the cost of the platform's own tab behaviour; that is a separate decision, deliberately not taken here. Anyone revisiting this should start by rechecking whether `expo-router` has since exposed trigger geometry — that, not cleverness with coordinates, is what would unblock it.
...
1. **Consent screen** — required, an explicit tick, not a click-through (health data is special category data under UK data protection law). Text:
   > **Welcome to Selodía**
   >
   > Selodía asks about things like your food, weight, body measurements and activity so it can actually understand you, not just log numbers. This is health data, so we want to be upfront: it's yours, it's kept secure, and it's never sold or shared.
   >
   > Selodía isn't a medical service and doesn't replace advice from your doctor. Think of it as a very attentive companion for the day-to-day. You can delete your data at any time.
   >
   > ☐ *I understand and agree to Selodía collecting and using my health data as described, and I've read the Privacy Policy.*
   >
   > ☐ *Keep me posted with occasional tips and updates from Selodía* (unticked by default — separate consent from the above, never bundled)
   >
   > ☐ *I'm happy for de-identified data from my use of Selodía to be used to help improve the product and understand patterns across users. This is separate from selling data, which Selodía never does.* (unticked by default, its own separate consent)
   >
   > *(Heading changed 2026-09-02. Rule 2 of the user-facing copy pass gives the app exactly one self-introduction, and this is it: product chrome rather than the companion speaking, so it does not reintroduce the fault that took the app's name out of the opener on 1 September. The step label in the onboarding progress header still reads "Before we start" — see `onboarding-progress.ts`.)*
...
9. **Technical target-setting**, once the emotional goal is established:
   > "There are a couple of ways to measure your body fat. Bioimpedance scales give a useful estimate — especially over time. A single reading carries a wide margin of error compared to a DEXA scan — the trend over time matters far more than any individual number [R3]. Waist circumference is also a good marker, and if there's a particular area you'd personally like to track, we can add that too."
...
> **NOTHING IN THE APP EVER SETS THESE (found 2026-09-09). This is the gap, and it is load-bearing.** `fat_focus_state` and `muscle_focus_state` are **read in four places and written in zero** — the Overview's calorie bar, `ask-unflump`'s new day-state block, and the two target calculators. The onboarding goals step (`goals.tsx`, 206 lines) advances the step and persists **nothing** about the goal. The two rows in the live database that carry values were set by hand.
>
> **Both stored values are `maintain`, which is identical to the code's fallback**, and that is precisely why this has never been visible. `asFocus()` on both sides maps null to `maintain`, so the wrong answer and the right answer have coincided since 17 August.
>
> **What it actually means:** every calorie target the app has ever shown is a **maintenance** target, whatever the person wants. All nine combinations are supported by the matrix and exactly one is reachable. The deficit branch, the surplus branch and the recomposition framing are live, parity-checked code that **no user can currently reach**.
>
> ~~**Deliberately not fixed on the spot.**~~ **DECIDED AND BUILT the same day.** The mechanism is a real design decision rather than a missing line: this app captures almost everything conversationally, but a Focus state drives a number on a bar, and “the model decided you were cutting” is a different proposition from it remembering a preference. Whether it is extracted from the onboarding goal conversation, offered as an explicit choice, or inferred and then confirmed, was Ruth's call, and she chose **inferred, then confirmed**.
>
> **How it works (`app/lib/focus-states.ts`, 2026-09-09).** Selodía notices a direction in what somebody says, **offers** to set it, and changes nothing until they agree. Three new classify fields carry it: `proposedFatFocus` and `proposedMuscleFocus` when a direction is expressed that differs from the stored state, and `focusAnswer` when a later message answers the offer. A proposal that would not change anything is discarded rather than asked about.
>
> **THE OFFER IS STORED, NOT REMEMBERED**, in `pending_fat_focus` / `pending_muscle_focus` / `pending_focus_asked_at`. The alternative is trusting the model to recall across turns what it proposed, and a confirmation applied against a misremembered proposal sets somebody's calorie target to something nobody chose. The database holds the offer; the model only reports whether the answer was yes. Same split as the allergy gate and the goal check — **the model observes, the app decides.**
>
> **An unanswered offer expires after seven days.** Somebody who ignored it has effectively said “not now”, and treating a fortnight-old “shall I set you to reduce?” as live would apply it on the strength of a yes that answered something else entirely. **A ‘no’ clears the offer and changes nothing** — declining a suggested deficit is not the same as asking to hold steady.
>
> **`fat_focus_since` and `muscle_focus_since` are stamped only when the value actually changes**, which is what item 24 needs: re-confirming a state somebody is already in must not restart a nine-week consolidation clock they are eight weeks into.
>
> **The confirmation line is written by the app, not the model**, for the same reason the save confirmations are — the model composed its reply before any of this ran, so it cannot honestly claim the change was made. Verified by `scripts/probe-focus-capture.mjs`: 24 checks, all passing, most of them asserting that something does NOT happen.
...
**That comparability holds to about 30% body fat and breaks above it (measured 2026-09-03).** The ranges overlap at 55 kg/26.4% (−10%), 60 kg/22% (−5%) and 70 kg/30% (−14%), but at 85 kg/38% the lean path gives 105-126 g against the fallback's 136-170 g: **no overlap, −25%**. This is inherent rather than a tuning error — the fallback cannot see body fat, so the two formulas diverge exactly as body fat rises. The lean-mass figure is the better one; what the divergence really shows is that **bodyweight × 1.6-2.0 over-targets at higher body fat**, because the literature's bodyweight bands assume roughly typical composition. Worth carrying to expert review as a question about the fallback, not about the lean-mass path.
...
**Three layers, three different scopes — and they must never be substituted for one another (2026-08-27).** Protein quality is now assessed at three distinct scopes, and the distinction is the whole point:

| Layer | Scope | Where it renders | Data it reasons over |
|---|---|---|---|
| **Per-item flag** | one ingredient | the itemised breakdown card (item 13), as a tag beside the row | `food_items.protein_source` |
| **Meal-level completeness** | one logged meal | the chat breakdown table, as one line under it | `food_items.amino_profile` |
| **Day-level nudge** | today's whole intake | the Overview's food-intake card | `food_logs.protein_source` |

**A per-item flag must never be promoted into meal-level or day-level prose.** This is a real rule, not a style preference, and it was learned from a live defect. A tag beside an item is scoped *to that item*: "pair it" next to chia is true whatever else is on the plate. Written as a sentence underneath the table, the identical classification silently becomes a claim about the **whole meal** — and on 2026-08-27 that produced a line telling Ruth to add dairy to a breakfast that visibly contained yoghurt in the row above. Nothing had checked the rest of the meal, because a per-item flag never does. Anything that reads as a statement about a meal or a day must re-reason at that scope, over data adequate to it.

**Why `amino_profile` had to exist at all.** `protein_source` (animal/plant/collagen) *cannot* express complementarity, so no amount of meal-level logic over it could have fixed the defect. Complementarity is about **which** essential amino acid is limiting: legumes are methionine-limited, grains and seeds lysine-limited, collagen very low in tryptophan. Lentils + rice is complete; lentils + chickpeas is not — and both are "plant, plant". So `amino_profile` (`complete` / `limiting_lysine` / `limiting_methionine` / `limiting_tryptophan` / null) is classified by the parse LLM at log time on both `food_items` and `food_logs`, coerced valid-or-null. This is the principled log-time capture of Part Two principle 13 and the sodium precedent: the **model judges the food**, which it reliably knows; the **completeness reasoning stays deterministic code** (`mealAminoAssessment`, pure and node-tested), which is the same split the safety state machine draws.

**Three honesty rules the meal-level layer holds to**, each of which a naive fix would have broken:
- **A protein floor (15g per meal).** Below it the layer says nothing at all. The meal that exposed the bug carried ~6.7g of protein across a banana, chia, a spoon of yoghurt and blueberries — the honest assessment of that is silence, not advice. (The day-level nudge holds the same line at 20g, for the same reason.)
...
>
> **Does building muscle raise your BMR?** Yes — but modestly. Research puts it at roughly 10-13 kcal a day for every kilogram of muscle gained. [R1]

The "10-13 kcal/kg/day" figure is the resting metabolic rate of skeletal muscle [R1] [Resources: muscle resting metabolic rate].
...
**Correcting a reading — ask when it's ambiguous, never guess (decided 2026-08-28).** A correction stated as a bare number ("no, that was 26.4") can fit more than one of the three scale readings on the row it targets: 26.4 is a credible body fat percentage *and* a credible weight in kilograms, and on a row holding both there is no honest way to choose. **Unflump asks which reading is meant and writes nothing until it is told** — naming the candidates ("is 26.4 your weight or body fat?") rather than asking a bare "which one?", since the person cannot see the row and should not have to remember what was on it. This is the same **ask, don't assume** discipline the safety classification and the food-clarification loop already follow, applied to the case where guessing wrong is *invisible*: it rewrites a number the person never mentioned, and nothing on screen says it happened. Scoped to corrections only — a fresh log has no row to be confused with, so a bare number there is an ordinary weigh-in and asking would be nagging. **A correction is also a patch, never a replacement:** it changes only the readings actually restated and leaves every other field on that row untouched. *(Both halves found and fixed 2026-08-28: the update path had been writing all three fields, so correcting a weight silently deleted the body fat and muscle beside it.)*
...
**Refined 2026-08-31 when this was built (item 29).** The test for whether an output belongs in this store is sharper than "is it per-entry": it is **whether the output's own inputs can change retroactively**. Only then is there a difference between what was said and what would be said now, and only then is a point-in-time record worth keeping. The body-measurement interpretation passes that test. The per-item protein-quality flag does not — it is a deterministic function of `protein_source`, persisted at log time and never changed, so it is computed on read and deliberately not stored. A per-entry output whose inputs are frozen is not a diary entry; it is a pure function, and storing its result creates a second copy that can only drift.
...
**Built the same day, for food: 1 and 2.** `food-history-view.tsx` is the food log week by week, on its own screen (`body/food-history`, titled "Food log") one level in from Food, reached by an "Earlier weeks" link under today's log. It is addressed by a week and never by "today", so the stepper moves one value and everything re-reads from it; **Next stops at the current week**, because a stepper that walks into empty future weeks invites the question of whether something failed to load. Each day shows its entries and its own total, each entry opens the same "What's In Here" breakdown card as today's log, and the discussed marker carries across. The screen takes an optional `?week=` param through `parseWeekStartParam`, so a later link from a roundup into a particular week needs no new path.

**3 is built too, for food, the same day.** "Select them to discuss further in chat" is the discuss-card's INTERACTIVE half (build item 30), which had never been built on any card: the food breakdown card, the reading card and the Almanac's exercise detail each held it back in their own comments, deliberately, because rendering a dead control is what principle 8 forbids. It turned out the server half was already finished - `ask-selodia` accepts `entryId` and `entryType`, validates the type, writes them onto the turn, and carries the thread's tag forward until the model says the topic has moved on - so what was missing was only the button and two fields from the phone.

**"Ask about this"** now sits at the foot of the food breakdown card, wherever that card is opened from: today's log or any week in the history. It closes the card, opens Chat with the entry named in the composer (`About my "Chicken and lentil salad" log: `), and carries the entry's id and type on that one turn. The tag is then dropped on the phone rather than held, because the server is the one that continues it - two places holding the same fact is how they drift.

**The card image is deliberately still unbuilt.** Item 30 also describes posting a picture of the card into the thread, and that needs a view-capture library nobody has installed. Half-building it would have meant a button that sometimes posts an image and sometimes does not. The read-only half of that (history rendering `image_path`) has been there all along, waiting.

**The other two cards still hold the button.** The reading card and the Almanac's exercise detail say why in their own comments; an exercise is not a valid `discuss_entry_type` at all, because it lives inside a plan's JSONB rather than being a row with an id.

**Activity and measurements have no history view either.** Ruth's rule was "all logs", and this is the food one. The same shape ports to both.

**It opens where the data is, and the way back is never conditional (2026-09-16, within the hour).** Ruth opened the new screen and reported *"there is nothing at all in Food log"*. The screen was working: it opened on the CURRENT week, which held nothing, while the seven days she had just caught up sat in the week before. Seven rows of "Nothing logged" is indistinguishable from a screen that failed to load. Two rules from her, both now held in code: *"It should always be navigateable to past weeks, even if the current week is blank"* - the "Earlier weeks" link renders whether or not today has entries, and the arrows are always live, with only Next disabled and only at the current week - and the screen now opens on the most recent week that actually holds entries, unless a `?week=` was asked for, because a link into a particular week is an instruction rather than a suggestion.

**"This was scoped already", and she is right.** `food-week-view.tsx` was built weeks earlier - a week-by-week table, addressed by a week start, written to be stepped - and then parked for a future Measurements port, while the Food segment was written as today-only. So the capability existed in the repo, complete and unreachable, and reads to the person using the app exactly as though it had never been built. **Parking a built thing is not neutral: a capability with no route is invisible, and the spec line that parked it is the only place anyone would ever learn it exists.**

**Found while diagnosing, and older than any of this: the weekly average under today's log has always covered the wrong seven days.** `weekRange` takes a week START and does not snap to a Monday; `food-today-view.tsx` passed it `new Date()`, so the "Avg ... kcal" line was computed over today plus the next six days - a window mostly in the future - and vanished entirely whenever today was empty, which is how it was noticed. Fixed to `currentWeekStart()`. Every other caller already passed a real week start.

**Two ways back, deliberately, until one wins (Ruth, 2026-09-16).** Seeing it work, she said the link was not the shape a phone user expects: *"It would be much more in keeping with normal app behaviour if the title 'Today' just had an arrow to the left which took you to earlier weeks. Maybe leave 'earlier weeks' button for now but place the arrow button too and I can test over time what's best."* So the Food segment now carries both: a back chevron beside the **Today** heading, which is where a thumb goes, and the explicit **Earlier weeks** link below the log, which cannot be missed by someone who has never thought about it. **This is a decision deliberately deferred, not a design**: one of the two comes out once she knows which she uses, because a screen with two controls doing the same thing is an unmade decision left on display.

**Activity has the same log, same shapes (Ruth, 2026-09-16: "let's roll out the weekly back scrolling to Activity tab too").** `activity-history-view.tsx` on `body/activity-history`, titled "Activity log", reached by the arrow beside **Recent activity** and by the **Earlier weeks** link below the list - both unconditional, both to the same screen. It opens on the most recent week holding sessions, steps a week at a time, stops Next at the current week, and totals each day's minutes and the week's. **Two differences from food, both because activity is a different thing:** no eye icon, because there is no activity detail card to open - the breakdown card is a `food_items` table and a session has no items - and daily tracker totals are filtered out by `withoutDailySummaries`, the same rule the Activity segment already applies, because a whole-day step total is not a session anybody did.

**The thread now shows what is being discussed (Ruth, 2026-09-16: "it did land in chat but no card appeared to let the user and chat know what is being discussed").** "Ask about this" tags the turn, and the entry now renders ON HER OWN TURN, above the reply, so the question and its subject sit together. It is the live itemised table read from `food_items`, never a picture of the card: **a screenshot is a second copy of the same facts, and it is the copy that goes stale.** The table returns nothing for an unitemised entry by design (an apple, a branded yoghurt), which would have reproduced the exact complaint, so it takes a `naming` flag: on a turn that is ABOUT the entry it renders the entry's name and date even with no table under it. Off everywhere else, where the reply has already said what was logged.

**EVERY DETAIL CARD HAS THE SAME WAY INTO CHAT (Ruth, 2026-09-16).** Her rule, on finding activity had no card at all: *"The eye card once open should always have a way to take it to 'Ask about this', same as throughout the rest of the app."* And the wording is hers: **"Ask about this", never "discuss"** - the database columns are named `discuss_entry_*` and the word stays there, out of sight.

- **Food** already had it. **Measurements** now has it on the reading card, where it had been held back since the card was written; it appears only when the day actually holds a reading, since the card also opens on empty days.
- **Activity had no detail card to hold a button.** `activity-detail-card.tsx` is that card: the session's type, when, duration, burn, distance and its log-time intensity tag, taken from the row the caller already holds rather than read back. No interpretation note, because item 29 writes those against body measurements only, and computing one live is what that item exists to prevent.
- **The thread shows the subject whatever kind it is.** Food keeps its live itemised table; `entry-summary-card.tsx` covers sessions and readings, which have no items and so no table. Read live, never posted as a picture: a screenshot is a second copy of the same facts and it is the copy that goes stale.

**"Ask about this" fills the composer; it does not send.** Ruth reported no card appearing, and nothing had been sent at all - the card rides on HER turn, so it cannot appear until the message goes. Worth stating because it looked like a failure and was a misunderstanding of what the button does.
...
**And the geometry settles it, which is Ruth's own argument (2026-09-16).** The worry recorded above was that the ring might read as a target once real petals appeared - the part-way week was mocked precisely to test that. Seeing it on her phone she gave the better reason for keeping it: *"the petals move from the outside in, so actually they are a starting point, not a target."* Each petal is pinned at the tip radius and grows INWARD, so a gauge's logic is inverted here: a gauge fills toward its edge, and this one empties away from it. **A circle that every petal moves off cannot be a thing to fill.** The ring marks where growth begins, and the bloom happens in the space it leaves behind.
...
**Status: BUILT 2026-09-09.** The rule is now in `ask-unflump`'s system prompt: a symptom — ache, soreness, stiffness, fatigue, low energy, bloating, poor sleep, feeling heavy — sends the model to the logged activity and food for the previous one to two days before it says anything, and it must name the session it found. If the log genuinely does not explain the symptom it says so and asks, rather than reaching for a generic cause: being told “nothing in your log obviously accounts for that” is useful, and being told something true of anybody is not.

**One correction to what this section previously said.** It stated that “what is missing is not data or a lookup; it is the rule”. The rule was indeed missing, but so was half the data: `ask-unflump` selected only `happened_at, activity_type, duration_min, kcal_burned` from `activity_logs`, so **`eccentric_load` and `intensity` were classified at log time (item 27) and then never read here**. The prompt could see *that* somebody trained but not whether it was the kind of training that makes legs hurt two days later — which is the whole question a symptom query asks. Both fields now reach the summary, appended only when they were actually classified so an older row reads as it always did. The original observation was right that the ingredients existed; it was wrong that they were on the table.
...
*Ruth's text, verbatim. The Insights brief it calls "above" is the next section of this document. Status on 2026-09-12: the Insights brief is reviewed and CONFIRMED (Ruth's answers follow its review). The Me brief is CONFIRMED too, and for Me it supersedes this note's line that the user writes and edits the document: the AI drafts from conversation and she confirms (Ruth, 2026-09-12). Whether the same holds for Movement is for its brief. The Movement brief is CONFIRMED as well (Ruth's answers follow its review), and its answers show the same rule as Me: the AI drafts from conversation and she confirms. **All three briefs are confirmed, and building can begin. Build order, decided by Ruth on 2026-09-12: Insights first.***
...
- **Movement and Me empty-state wording approved by Ruth, 2026-09-12.**
...
**Slice 3, the Sunday roundup, built 2026-09-16. NOT YET SEEN ON A DEVICE.** The roundup now has two jobs: the weekly reflection itself, and the living portrait at the top of Insights.

- **It reads the week directly.** The old route was built on stored daily summaries, which Ruth's answer 3 retired, so `app/api/weekly-roundup/route.ts` now reads the week's food, movement, measurements, what she agreed to keep (this week's symptoms, notes and insights) and what she actually said in chat. Every honesty rule around the numbers is unchanged and still in code, not the prompt: the 5-of-7 gate, missing days excluded rather than zeroed, and a direction only on three readings or more. `app/api/daily-roundup/route.ts` is deleted; the `daily_summaries` table stays for export and deletion, holding the rows it always had.
- **Where it lands (Ruth, 2026-09-16): both.** A permanent Almanac entry tagged Roundup, and the same text posted into the chat thread.
- **The witness statements ride on the entry.** Two or three, covering the last 6 weeks, written by the same model call from this week plus the previous six weeks of roundups, stored on the roundup entry as plumbing keys. The portrait reads the newest roundup, so a statement can never drift from the week that produced it, and somebody with no roundups has no statements rather than a default set. `mobile/src/lib/almanac-content.ts` now hides any key beginning with two underscores, so a card shows the roundup and not its plumbing.
- **How it fires (Ruth, 2026-09-16).** There is no scheduler and no remote push in this project, so a server job would have been new infrastructure and a Vercel production change. Instead: a **local Sunday-evening notification at 21:00**, riding on the same reminder yes as the daily ones and cancelled with them, and a **launch check** (`use-weekly-roundup.ts`) that asks the server for the week's roundup when the most recent complete week has none. The week is named by the phone, in local time, because "Sunday evening" is local and a serverless function runs in UTC; the server validates what it is handed and refuses a week that has not finished.
- **One roundup per week, and only the most recent week.** The entry carries its week, so a second call returns the one already written. Someone away for a fortnight gets last week's roundup, not two: a reflection handed over ten days late alongside a fresher one is a backlog.
- **A week with nothing in it gets nothing, silently**, which is Part Nine's rule for an empty day applied to a week. It matters most for somebody who installed the app on Friday: without it, their first Sunday would ask how their logging had gone before they had done any. A thin week still gets the "how have you been finding the logging" question, but **once per week** rather than once per launch, which without a guard is what the launch check would have produced.
- **Probe:** `scripts/probe-roundup-week.mjs`, 32 checks, all passing. Typecheck and lint clean on both sides.
- **New app wording, awaiting Ruth's approval:** the Sunday notification reads "Ready to look back on the week?"
...
- **Found on the first voice test, 2026-09-12: an offer can be stored without being asked.** "I had sore hips after ballet last week too" stored a symptom offer, but the spoken reply contained no question, so an offer sat waiting for 48 hours for a question she never heard, and a later yes to something else could be read as its answer. **Fixed the same evening, on Ruth's choice:** the app writes the question itself, as it writes the confirmation, and only once the offer is actually stored. Approved wording: "Want me to keep that in your Almanac?" The model is told not to ask it, and if it asks about the Almanac anyway the app's line is left off, so the question is asked once. The unasked hips offer was cleared at Ruth's request.
...
3. **Supplements: record, never prescribe.** Selodía records what she has decided and why, never suggests a dose, and prompts a check with a GP or pharmacist when health conditions, medication, pregnancy or breastfeeding are relevant. Added to the Safety Architecture as §9; breastfeeding confirmed later the same day.
4. **Nutrition shows the targets the app already holds**, not a copy. One source of truth, no drift.
5. **No ticks. Status in words only:** Taking, Ordered, Dietary source, As needed, Paused. **Active is kept as well**, for recurring commitments such as a weekly call (Ruth, 2026-09-12).
...
*Ruth's answers to the thirteen decisions above, plus decisions from the design chat. They supersede any conflicting detail in the Movement brief. With them, all three Almanac briefs are confirmed.*

1. **Only "Inner Thigh Tone" moves**, filed under the goal name she gave it, "inner thigh tone". The two seeded test entries are deleted. No renaming and no taxonomy: goals are always named by her, never by the app. **Done 2026-09-12:** the seeded test data is gone. That is three entries, not two: the two test plans and a test insight, "Late-luteal weight pattern". With them went the test plan's completion, its two working weights (Ruth's own testing, one of them logged that afternoon) and the activity row that test completion had written into her activity history. Every deleted row was returned by the delete and kept as a JSON backup for the session. "Inner Thigh Toning Routine" keeps the goal it was saved with, "tone and strengthen inner thighs": Ruth, asked whether to change it to her shorthand, said it should stay whatever the user wanted at the time it was made.
2. **Ticks are optional: a navigation aid during a session, not the record of completion.** Training history in Insights comes from what she tells the AI. Three ways to log a completed workout: **voice**, the primary method; **tick and submit**, ticking exercises in the plan, picking a date (a date picker is required, since yesterday's session may be logged today) and submitting without speaking, for public situations; and **chat text**. *Build consequence: today every tick writes an append-only completion row through `/api/log-workout-completion`. That changes. Ticks alone record nothing; a submit with a date writes the session.*
3. **The Never list informs; it does not block.** The AI will not suggest contraindicated movements by default, but if she says she is doing something on the Never list, it notes the context and respects her decision. No gates, no warnings, no refusals: she decides, not the app. *Recorded as deliberate, against Code's recommendation of a code-level gate, so that it is not "fixed" later. It follows the Language Rules: autonomy stays with the person.*
4. **Letters are not stored.** The AI extracts the constraints and drafts them in plain language, she confirms, and the letter itself is discarded. NHS letters carry a name, NHS number, address and GP surgery, and there is no benefit in keeping any of it. *Build consequence: nothing to store, delete or export. But each letter is still sent to Anthropic to be read, so the privacy policy's list of what Anthropic processes must include documents, and Google's Data Safety still counts it as collected (it leaves the device), declared as processed ephemerally.*
5. **Any format the AI can read reliably:** PDF, photo or anything else. If it cannot extract the content reliably, it tells her so clearly rather than guess or fail silently. *Needs a document picker (`expo-document-picker`) beside the existing photo picker, so a new build.*
6. **Movement analytics live in the Movement tab**, in their own view, following the same pattern as domain analytics in Insights.
7. **Daily movement has four sources:** automatic step data from the phone's health platform (Health Connect or Samsung Health on Android, Apple Health on iOS), replacing the Samsung Health screenshot workaround; a **quick log from saved classes**, one tap to confirm with an optional note, contextual to the day and her My Week baseline; **uploads from third-party apps** such as Strava, uploaded or connected, with the AI extracting the session; and **chat**, by voice or text, for anything else. *See the Health Connect answer below. A Strava connection would be a new third-party processor, with the same privacy and Data Safety consequences as letters. One tension to settle at build time: with My Week holding rhythm rather than days (answer 9), "expected based on the day" has to come from rhythm, for example a once-a-week class not yet logged this week, and it must never read as behind.* **Addition, 2026-09-12 (Ruth, after logging "Gym" on her phone): saved workouts appear as quick taps wherever movement is logged, beside the saved classes**, because a generic "gym" could hold anything. Tap the workout, confirm, optionally add a note. **The evidence came with it.** The "gym session" she logged that evening (45 minutes, intense) was written with all six flower coverage values empty, so it added nothing to the flower, while the yoga and the run logged beside it credited every petal. A workout completion does no better today: the test completion deleted the same day had written an activity row with no coverage at all. So a quick-tapped workout only helps the flower if the build works out its coverage from the plan's own exercises, never from its title. **Recorded as a requirement of the Movement build.** It is the same gap as the flower's known unclassified rows (Part Eight), met from the other side.
...
**Me, the two remaining items:** **Active is kept** as a status, for recurring commitments such as a weekly call; and **breastfeeding joins pregnancy** in the supplement rule (Safety Architecture, section 9).

**Build order: Insights first.** Code sees no reason to change it, with one note for the build: the conversational save (the AI recognises the moment, offers, she confirms, the entry is tagged) is the same mechanism Insights, Me and Movement all need. It is built once, in Insights, in a form the other two reuse.

**Ruth's technical question: does Health Connect integration exist?** Half of it. The native module (`react-native-health-connect`) is in the build, the step permissions are declared in `app.json`, and the permission request exists (`step-permission.ts`, fixed 9 September, not yet verified on a device). What does not exist is reading: nothing calls `readRecords`, so no step has ever been read (Part Four). It needs building: a reader for daily steps, somewhere to keep daily totals, a way to change the permission answer after onboarding, and verification on a phone. On her Samsung, Samsung Health also has to be set to share with Health Connect. About half a day, and because the native module is already linked, it may not need a new build. Apple Health on iOS is in the same position.
...
**Standing correction — posterior-chain focus was never a product requirement (2026-08-19).** Posterior-chain work runs through this document's examples: the "Posterior-chain training plan" Almanac entry, the Romanian-deadlift safety note, and a large posterior-chain group in the coverage workbook. **That is Ruth's own training context — she is the design-time example user — and it is not, and never was, a product focus for Unflump.** No product requirement, coverage bar, acceptance test, or build assumption may rest on it, and any earlier wording that read as one is superseded. Real users arrive with the full range of goals, which is why the library is judged on **breadth across every movement pattern**, never depth within one — and that is how the phased-rollout decision below was actually validated. Posterior-chain references that remain are **illustrations of a mechanic, labelled as such**. *Verified 2026-08-21: no application code, database schema, or migration carries a posterior-chain assumption — at that date there was no exercise database of any kind in the product, so nothing had been built around it.* **Updated 2026-08-28:** an exercise **classification manifest** now exists (see *Movement classification manifest* below). The verification above still holds — the manifest carries no posterior-chain assumption, and its muscle vocabulary is a flat anatomical list with no pattern privileged over another — but the "no exercise database" clause is no longer literally true and must not be quoted as though it were.

**Exercise demonstration content:** for movements where form matters (e.g. a dead bug, or anything with a timing/sequencing component that a static image cannot convey), the Almanac needs visual demonstration. Prefer calm, animated demonstrations over real video — this is consistent with the brand's calmer, less intrusive tone, and it also sidesteps body-representation concerns that real-video libraries raise. AI-generated video/animation is explicitly rejected for this purpose: exercise form is safety-relevant, not cosmetic, and current AI video generation can produce subtly incorrect movement, risking genuinely bad form rather than a minor inaccuracy. Use a licensed animation library instead.

> **THE 19 AUGUST RULING STANDS IN FULL. Nothing is AI-generated.** It was briefly reopened on 2026-09-16 and withdrawn the same hour, and both halves are recorded because the reasoning matters more than the outcome. **What was proposed:** still illustrations, for a named shortlist of fifteen movements only, with Ruth's sign-off on every image — on the argument that fifteen is small enough to check one at a time, where a 770-clip library never was. It was priced (about 15p a movement) and not run. **Why she withdrew it:** she chose a licensed SVG illustration library instead — a source with a commercial licence beats a generated image that a human then has to vet, because the vetting is the expensive part and the failure mode is silent. A plausible-but-wrong depiction of a deep end-range hip or knee position is the one error nobody catches by glancing. **The standing direction is therefore: find licensed or openly-licensed artwork, never generate it.** The paragraph below is kept only as the record of a decision considered and declined.
>
> *(Historic, withdrawn 2026-09-16.)* The ruling above stands for video and animation, and for generating a library. It does not stand for the fifteen specific movements the licensed library cannot cover. **What changed:** the 19 August ruling was made about generating a LIBRARY, where nobody can check 770 depictions and the tail risk is unbounded. The situation now is fifteen named movements — butterfly stretch, frog stretch, seated straddle, wide-leg forward fold, wall-supported middle splits, cossack squat, side lunge stretch, sumo squat, curtsy lunge and the rest of the unresolved set — which is small enough to check one at a time. **The condition is the whole of the decision, and it is not optional: every image gets Ruth's explicit sign-off before it ships.** Not spot-checked, not sampled. She does splits training and will spot a wrong hip or knee position faster than any automated check, and "I'm not certain this is right" is a reject rather than a maybe. **Why the condition carries the safety argument rather than the generation doing it:** a plausible-but-wrong depiction of a deep end-range position is exactly the failure the original ruling feared, and nothing about the generator has changed - what changed is that a human who trains these movements now stands between the image and the person following it. Anything generated and unchecked must not reach a build. **Also recorded: the cheaper route was tried first.** Exercise Animatic's licence includes future standard releases at no additional cost, so asking the vendor what female flexibility content exists or is planned costs nothing and may remove the need entirely (see the movement coverage report, 16 September 2026). The Almanac provides individual movement building blocks (a squat, a stretch, a pose) that Unflump sequences into a bespoke plan for a specific user's goal — it does not need to license every possible pre-assembled routine, consistent with the Almanac's bespoke-plan philosophy generally. **Coverage is every single exercise, no exceptions** — framed as a competitive *floor*, not a nice-to-have (The Gym Group's app is the bar): the cost of a gap is a user leaving Unflump to find the answer elsewhere, which is the opposite of what this feature is for. *(Phasing, 2026-08-19: this exhaustive floor is retained as the longer-term target, not the first-release bar — the first release ships confident movement-**pattern** coverage from what the library actually contains. See the phased-rollout decision below, and the standing correction retiring the 750-item probe list.)* Sourcing is **licensed animated content, not self-recorded**. *(The earlier instruction to locate and reuse prior licensing research is superseded — the research is done and the vendor is chosen; see the purchase decision below.)*

**Phased-rollout decision — pattern coverage now, exhaustive depth as the retained target (2026-08-19).** The **first product release** ships the movement-pattern coverage confirmed below, accepting a close substitution where an exact movement isn't available (same precedent as the deadlift-for-RDL decision). This is the **real, honest scope for the first release — not a temporary placeholder for a small friendly audience, and not a permanent redefinition of the standard either (there is no informal friends round — see Distribution).** It was validated on **movement-pattern coverage**, not raw count, per the product philosophy that *program coherence matters more than exact exercise identity* — especially for beginners, who benefit from Unflump making one confident, complete choice rather than a menu.
...
**What the licence actually permits — read in full 2026-09-09, and it is a real document.** For three weeks the only record of the terms was prose in the purchase email thread, and the spec said only that licensing was “settled”. It is: Exercise Animatic publishes a **Lifetime Non-Exclusive Business-to-Business Licence (N-EB2BL)**, with the operative text in **section 8 of its Terms of Service**. Licensor is **N.C.A. Health & Wellness LTD**. Both documents are captured verbatim, dated, and saved in Drive at `Selodia App Project Master Folder/Licences & Legals/Exercise Animatic/` — captured rather than linked, because a licence you can only read by visiting a website is one that can change without telling you. That folder also holds the vendor's email statements and a plain-English `Read me first`.

    **Permitted, explicitly and by name:** commercial iOS and Android apps, including free, paid and subscription; websites, fitness software and membership platforms; monetising a product that incorporates the content; and “add your logo, replace green-screen backgrounds, resize, crop, compress, transcode and adapt”, plus “combining the content with your own software, graphics, text or user interface” (ToS 8.2). **The 770-clip batch recolour is covered**, and was covered before it was run — the vendor said so twice by email in August, and the published terms agree. Adapted clips remain subject to the licence; the recolour transfers no ownership.

    **From the email thread, and nowhere in the published terms: no per-user, per-view or royalty fees, ever, however large the app gets** — plus future standard releases at no additional cost. That is the most commercially important term of the whole purchase and it exists only as a sentence in an email, which is why the email is saved beside the licence rather than left in a mailbox. Worth getting into the same document as everything else at the point real money depends on it; not worth chasing now.

    **Two clauses govern how item 36 gets built — not whether.** These are build constraints, recorded here once so they are not rediscovered:

    **(1) Clips are streamed, never made downloadable.** ToS 8.4 prohibits “allowing end users to download or extract the original Exercise Animatic files from your product or service”, and the licence page separately prohibits sharing or providing access to the raw files outside the business or an authorised project team. This rules out a public bucket, a permanent or guessable file URL, any save/export control, and any download affordance. It points squarely at **a private Supabase bucket with short-lived signed URLs**, which is therefore not merely the convenient choice but the one the licence asks for: no URL exists until an authenticated person opens a specific exercise, and it expires within minutes. The clause is about what the product *offers*, not about what someone with developer tools could capture off a video stream — nothing short of studio DRM prevents that, and no fitness app on the market has it.

    **(2) No clip, frame or thumbnail is ever sent to a language model.** ToS 8.3 explicitly *permits* AI-powered applications — content may be used in platforms with AI features “provided the Exercise Animatic Content is displayed only as licensed visual media”, so Selodía qualifies without qualification. What is prohibited is *supplying* the content to a model for training, fine-tuning, dataset creation or generating output. **Standing rule:** exercise metadata is derived from filenames, the vendor manifest and human review. Never by having a model watch a clip.

    **The classification manifest was built the prohibited way, and this note first claimed otherwise (corrected 2026-09-09, hours after it was written).** `classify.mjs` extracts **three JPEG frames from every one of the 770 original clips** and sends all 2,310 to Sonnet as base64 images, so the model can read the vendor's red/orange muscle highlight off the frame. Every row in `movement-classification.json` carries an `ai_confidence` value, which is the proof that all 770 went through the vision call rather than only the 393 marked `ai_classified` — those 393 took muscles and equipment from it; the other 377 took `movement_pattern`, because the vendor sheet has no such column. A 50-clip validation run preceded it. This happened **2026-08-28, twelve days before anyone read the licence**, which is the entire argument for reading a licence before building against it rather than after.

    ToS 8.3 forbids content being “uploaded, supplied, **analysed**, or otherwise used to … generate output from” a model. The script's own header reasons that reading the vendor's own highlight is “closer to reading their annotation than to inferring anatomy” — a fair distinction in spirit, and one the word *analysed* leaves no room for. On the plain text this is a breach, and ToS 8.4 lets the vendor suspend a licence for a material one. Against that: a single private pass, nothing redistributed, nothing used for training or fine-tuning, and what it produced is our own factual categorisation rather than any reproduction of their content.

    **The manifest itself stays.** The breach was in the method, not the artefact — deleting 770 rows of real work would un-analyse nothing. What stops is the method: no re-run of `classify.mjs`, no further frame extraction, and any future gap filled by filename, vendor sheet or human eyes.

    **Decision, Ruth, 2026-09-09: leave everything as is and keep going.** Nothing is deleted, nothing is disclosed to the vendor for now, and the build carries on. The reasoning, recorded because it is the kind that gets forgotten and second-guessed later: **there was no bad faith at any point.** The library was bought on 19 August; the vendor had no muscle metadata, which is the thing that would have made it usable; filling that gap was treated as ordinary daily build work by someone who had not read a licence she had no particular reason to think existed as a formal document. The moment it was understood, it was written down rather than tidied away — which is what this paragraph is.

    **And the strategic read that goes with it:** the licensed animatics are a **starting point for Selodía, not its answer.** A library this constrained — no metadata, no custom production, and terms strict enough to shape the architecture around it — is a floor to get the product standing on, not the thing the product is eventually made of. The route forward is either a better source or a commissioned relationship with Exercise Animatic once there is funding to make that a real conversation. Which also reframes the disclosure question: the natural time to raise it is inside a commercial conversation about producing what Selodía actually needs, not as a standalone confession about a one-off classification pass.

    **Also prohibited, for completeness:** selling, sublicensing or redistributing the clips as standalone assets; granting reseller, franchise or redistribution rights onward; building a competing exercise library or stock-content service; distribution through stock-media sites or asset marketplaces; NFTs or blockchain assets; television, cable or satellite broadcast without prior written approval (8.5 confirms this does **not** touch apps, websites, social media or online courses); and claiming ownership or authorship. All copyright — **including in the vendor's own metadata** — remains with N.C.A. Health & Wellness LTD.

    **One thing that will matter later and is easy to forget (ToS 8.6):** a **share sale keeps the licence**, because the legal entity is unchanged. A transfer of the licensed assets to a **different legal entity does not automatically carry the licence** — the vendor asks to be contacted in advance and says approval will not be unreasonably withheld on a genuine sale. Relevant the moment the Companies House structure changes or the app is sold.

    **New releases are added automatically, every week (Ruth, 2026-09-17: "The library should grow automatically every month without me touching anything"; moved from monthly to weekly the same day, because the vendor releases most weeks and a clip should not sit unused for a month).** The Ultimate Bundle includes every future Exercise Animatic release at no extra cost, and the vendor is filling the library with female-led clips one muscle group at a time: Abdominals on 27 August, Back on 3 September, Biceps on 9 September, Functional and Cardio on 15 September 2026. Those four releases added 149 female vertical clips, and the vendor also re-cut 265 existing files to shorter loops (average 19 s down to 5 s, same resolution). Nothing noticed any of it until someone looked, so the looking is now a scheduled job.

    **The job.** `.github/workflows/monthly-animatic-sync.yml` runs `scripts/monthly-animatic-sync.mjs` at 09:00 UTC every Friday on GitHub Actions (the vendor's releases have landed Tuesday to Thursday), independent of the laptop. It can also be run by hand from the Actions tab, with a dry-run option that reports and changes nothing. Each run:

    1. **Backs up everything, male and female, every format.** It compares the vendor's shared folder (a read-only mount that could be withdrawn) with **`Selodia Team Folder`** in Selodía's own Dropbox team space and copies every new file across on Dropbox's servers, so nothing touches the laptop's disk. A file the vendor has replaced has its earlier version moved to `Superseded by vendor re-release/<date> versions` first. Nothing is ever deleted. The backup was completed and verified file by file on 17 September 2026: 20,600 vendor files, 137.5 GB, none missing, none differing in size. The vendor renamed "1200+ GREEN SCREEN VIDEOS" to "1500+"; the backup keeps the original folder name and new green screen files go into it.
    2. **Adds new female vertical clips to the app's library.** `scripts/process-animatic-release.mjs` recolours each clip with the locked treatment (`scripts/assets/selodia-recolour-locked.cube`, sha256 aded0bbd…, same geometry as the August batch), labels it, uploads it to the private `movement-demos` bucket and writes its `movement_assets` row in the same step. **Every new clip becomes a library item whether or not any saved plan uses it yet.**
    3. **Replaces clips the vendor has re-released.** The new version overwrites the one users see (Ruth, 17 September, after comparing them: they "all look the same"). The earlier version stays in Superseded in the backup.
    4. **Re-links every saved plan.** `public.reresolve_plan_demo_refs()` (service role only) re-runs `resolve_movement_refs` over every plan's exercises and rewrites only the references that changed. `demoRef` is frozen at save time, so without this a plan saved before its clip existed would never see it. Idempotent: a second run changes nothing.
    5. **Reports.** A summary on the run's GitHub page; any error fails the run, and GitHub emails the repository owner.

    **Labels come from text, never from the clip (ToS 8.3).** `scripts/lib/animatic-labels.mjs` takes muscles and equipment from the vendor's metadata sheet (`2000+ EXERCISE METADATA.xlsx`) and the movement pattern from fixed rules over the clip's name and vendor folder. No clip, frame or thumbnail is sent to any model. The vendor appends a `_Female` row for each new clip but fills the anatomy in on the unmarked row of the same exercise (0 of 149 female rows had muscles, 134 of their unmarked twins did), so the first row that carries anatomy is used. Rows are written with `human_verified = false` and `ai_confidence` set to `rules+vendor_sheet` or `rules_only`, which records where the label came from rather than claiming an AI confidence, so a later review can find exactly these. They go live immediately; the library growing on its own was the point. Measured against the 770 August labels, the pattern rules agree 86% of the time. Those August labels were themselves produced by sending frames to a model, which this section already records as a licence breach, and relabelling them is a separate job.

    **Credentials.** A Dropbox app, "Selodia Library Sync" (scoped access, Full Dropbox, team member file access, `members.read`), authorised once with a refresh token. The job acts as the team's member and sets the team root namespace on every call. Five GitHub repository secrets: `DROPBOX_APP_KEY`, `DROPBOX_APP_SECRET`, `DROPBOX_REFRESH_TOKEN`, `NEXT_PUBLIC_SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`. They were set up without ever passing through chat or a terminal: `connect-dropbox.cmd` and `github-secrets.cmd` in the project root each open a page served only to the laptop (127.0.0.1) with ordinary paste boxes and copy buttons.

    **What stays human.** The monthly calendar reminder on the 17th is now a prompt to look back over the month's run summaries, not to do the work. Choosing an **alias** (a plan's name for a movement that the vendor calls something else) stays a human decision, because the matcher is deliberately exact and a confidently wrong demonstration is a form fault. Pull Up Normal Grip, released 3 September, closed the first gap this way: "Pull-Ups" was aliased to it on 17 September and two saved plans picked it up.

    **No further vendor contact is needed before building item 36.** The published terms address the delivery question directly enough that signed URLs are clearly compliant; asking a vendor to bless a specific storage architecture invites a slower and more cautious answer than the terms already give.
...
**Movement classification manifest — the product's first exercise database (built 2026-08-28).** Every one of the 770 female vertical clips now carries structured classification: **primary muscles, secondary muscles, equipment, and movement pattern**, against a closed vocabulary of 20 anatomical regions and 12 movement patterns. It is **asset-prep metadata, not a user-facing feature** — static reference data describing licensed content, identical for every user, regenerated only when the library changes. It is emphatically *not* the exercise-picker the "no dead pages, no pre-built empty sections" principle exists to prevent, and nothing in the app reads it yet.

**Provenance is permanent and visible on every record**, because the data has two very different origins and the difference must never be lost: `vendor_female_row` (313), `vendor_unsuffixed_row` (60, the same movement's unsuffixed row via gender-stripped matching), `vendor_fuzzy_variant` (4, morphological name variants such as Bicep/Biceps), and `ai_classified` (393). A `human_verified` boolean carries the top-level split — **377 human-authored (49%) against 393 AI-classified (51%)**. Vendor anatomy always wins where it exists; AI fills only the genuine remainder. **`movement_pattern` is AI-derived for all 770** — the vendor sheet has no such column, so there was nothing to inherit for any clip.

**Why AI classification is honest here, and where its limits are.** The vendor draws the activated muscles onto each animation in red/orange, so the classifier reads *their own annotation of that specific clip* from three frames (15/50/85% of duration, taken from the original masters — the recolour deliberately mutes that highlight). Validated against 45 vendor-labelled clips: **45/45 shared at least one primary muscle, 58% exact set match, 73% recall**. The single apparent miss was the model being right and the vendor wrong — on *Abdominal Stretch* the highlight genuinely sits on the lumbar region while the vendor had labelled it from the exercise name. **So vendor data is the better default but is not infallible ground truth**, and the recall figure measures agreement, not accuracy. The classifier under-calls rather than mis-calls: it names fewer primaries than the vendor on compound lifts. Cost was £3.52 all in.

**Two sources deliberately rejected.** (1) **Borrowing across equipment variants** — "Kettlebell Upright Row" from "Barbell upright row" — was rejected: near-identical anatomy does not make it that exercise's record, and attributing one movement's data to another is inference dressed as vendor fact. (2) **A reputable-web-source backup layer** was piloted and rejected on evidence: two of three trial exercises returned muscle lists that reputable sites did not actually state, generated by inference and returned *with citations attached*. The failures correlated with the gap — the web documents common exercises well and vendor-specific band, calisthenics and yoga movements not at all, which is exactly where the missing data is. A frame the vendor annotated beats a web page about a similarly-named exercise.

**Instructions and safety copy are excluded by rule.** The manifest carries no step-by-step form guidance and no safety notes. The same reasoning that rejected AI-generated exercise *video* — form is safety-relevant, not cosmetic — bars AI-written form *instruction*. The "Before you start" card stays exercise-specific and human-authored, and the vendor's own "Exercise Tips" are a source at most, never that card's content.
...
**Movement-demo slot ratio — 4:5 portrait (decided 2026-08-19), not 1:1 square.** The program content is predominantly standing / strength movements, where the taller ratio shows the figure **larger and more legibly** (in-app test: roughly 2× the size it reads at in a 16:9 slot). Wide / floor exercises (the minority) are handled by **fitting the subject into the fixed slot and padding with cream** — invisible, because the recoloured background and the slot are both cream — so nothing is ever cropped and a squarer 1:1 default isn't needed to accommodate them. Source from **vertical masters** (higher resolution on the figure than the horizontal versions). **Status: DONE, batch complete (corrected 2026-09-09).** **770 recoloured clips exist**, in `Build Specs/Excersise Animatics/Recoloured 4x5 female vertical/`, foldered by muscle group — one for every female-marked movement in the library. Counted on disk, not recalled.

*The paragraph below is the position as of 2026-08-19 and is kept because its reasoning still governs any future re-run:* proof of concept on one clip only; batch across the full library was **blocked** on two vendor questions — (1) are all exercises available as vertical masters, and (2) is the source background consistent library-wide (new samples are on white; one older clip came on green screen). No recolour was applied at scale until both are answered.
...
- The **exercise detail** view holds, in order:
  ...
  - a **working-weight control — a slider defaulting to 1kg increments, with the number always directly editable so any value can simply be typed over it (settled 2026-08-21).** Not a fixed jump, and deliberately not a predefined per-exercise increment either: real equipment lands on genuinely odd numbers — Smith machines, oddly-weighted bars, whatever plates a particular gym happens to own — and no increment chosen in advance can anticipate those. Letting the actual value always be typed handles that better than guessing the right step per movement. It **appends to an append-only working-weight history** (the plan displays *current = latest*; never overwritten in place, since progressive overload depends on keeping the history). This is **data-logging, not plan-editing**, so it doesn't touch the Almanac's chat-only editing rule. See build item 35 for the full data model.
...
**Conversational substitution — resolved 2026-08-21 (build item 46).** Because the first release ships a *subset* of movements, a conversation can legitimately arrive at a movement with no asset, and two rules meet there: no picker exists to steer someone toward what's available, and the fourth-wall rule forbids saying anything is missing. **The resolution reframes the whole interaction around *function* rather than name-matching.** When a requested exercise isn't in the library, Unflump:

1. **explains the muscle group and movement pattern that exercise targets** — useful in its own right, and squarely the body-literacy mission rather than a workaround;
2. then **either suggests genuine functional alternatives directly, or asks what specific outcome the person is after** and suggests from there.

**It never references the library's limits, in any wording** — no "we don't have that", no "that one isn't available", nothing that implies a catalogue with edges. This is not a cover story: what someone actually needs is the *training effect*, and a movement that delivers it is a real answer, not a substitute for a better one. Naming the pattern is genuinely more useful than name-matching would have been, which is what makes the reframe honest rather than evasive.

**Prerequisite, confirmed real (2026-08-21):** this needs per-movement **muscle-group and movement-pattern metadata**, and it does not currently exist in usable form. The delivered archives carry only coarse muscle-group *folders* (Abdominals, Back, Biceps, Calisthenics-Cardio-Plyo-Functional, Chest, Forearms, Legs, Powerlifting, Shoulders, Stretching-Mobility, Triceps, Yoga) — no movement-pattern axis at all. The coverage workbook's pattern categories look like the missing piece but are **not usable as product data**: it says so itself, having been built by *keyword matching against filenames*, explicitly "not their official muscle-tagging data, which wasn't provided" and to be treated as "a directional shape of coverage, not an authoritative count". Leaning on it would re-import exactly the closed-keyword-list problem Part Two principle 13 rules out, and that the DOMS flagger was rebuilt to escape. See build item 46 for the three real options.

**No contradiction with "no exercise database" above.** That rule forbids a **user-facing** picker, search box, or browsable catalogue — the closed menu the conversation-first philosophy exists to reject. An **internal** metadata store that only Unflump reasons over is a different thing entirely: the person still only ever reaches a movement through conversation. Stated explicitly so the two rules are not read as being in conflict.
...
**Capture — resolved 2026-08-21 (build item 42):** allergies are captured **conversationally, whenever they naturally come up** — never via a dedicated onboarding screen or a form to fill in. This is consistent with the one-text-box principle throughout, and it avoids front-loading a medical questionnaire onto someone who came here to understand their body. An allergy mentioned in passing while logging dinner is captured exactly as reliably as one volunteered at signup.

**Strictness — resolved 2026-08-21:** once disclosed, an allergy becomes a **hard, permanent exclusion — never suggested again.** This is categorically different from an ordinary preference or dislike, which can soften, be revisited, or be offered occasionally. The two must not share a storage mechanism or a code path: a dislike that leaks into the allergy filter makes Unflump needlessly restrictive, and an allergy that leaks into preference handling makes it dangerous. Only the allergy side is a safety constraint, so only it gets the guarantee.

**Mechanism, required:** its **own explicit storage**, and a **genuine filter check on every food suggestion** — an actual gate in the suggestion path, not an instruction in a prompt asking the model to remember. Prompt-level care is not a safety guarantee: the whole reason this is structured rather than conversational is that a long session can truncate context, and a model can be talked around. Folding allergies into general conversational memory alongside softer preferences would defeat the point of separating them. Note this is a *different* mechanism from Health Context (above), which shapes guidance through prompt injection — health context tilts priorities, an allergy hard-excludes.
...
**Real backend work (build item 31):** a way to log water intake — likely reusing the existing silent-logging pattern already built for food/activity, and overlapping the zero-calorie-drinks path (build item 26), since water is already recognised there — plus a sensible daily target. **BUILT — corrected 2026-09-09.** Both routes in exist: the Overview quick-tap and conversational logging through `hydration-logging.ts`. See build item 31 for the file-level detail and for which part of the original description was superseded.
...
**THE REMINDER FLOW ACTED ONCE AND NEVER AGAIN. Fixed 2026-09-12 (`3732881`) and VERIFIED ON A DEVICE the same afternoon.** `push_tokens` had held zero rows, ever, and the causes were three. Two were fixed on 10 September: the missing EAS `projectId`, and no Firebase config at all. The third would have survived both. On Android 13 and later the permission prompt does not appear until a notification channel exists, and registration asked first and created the channel afterwards. So the prompt never showed, permission came back not granted, and registration returned null silently.
...
**Verified on Ruth's phone, 2026-09-12, preview build `1291e191`.** The OS prompt appeared once on launch. The first push token in the project's history was saved at 16:26. A test push sent through Expo's service returned ok on both the ticket and the delivery receipt, and it was seen in the notification shade. One detail from the logs is worth keeping: the first launch read the reminder setting 0.3 seconds after sign-in, which is how it is known that the restore runs on sign-in as well as on a cold start. The locally scheduled reminders are the second check, first due at 20:00 that evening.

**Two gaps noted 2026-09-12, left as is:** there is no way in the app to change a reminder answer once it is given (Ruth's own "no" from 27 August had to be switched in the database to run the test), and `saveReminderChoice` in `notifications.ts` exists with no caller.
...
> **KNOWN DIVERGENCE, rewritten 2026-09-08 — half of it was fixed the day after it was written, and the note never caught up.** As found on 2026-09-02, `use-theme.ts` read `useColorScheme()`, so the app followed the phone and flipped with it — which the rule below forbids. **That is no longer true.** On 2026-09-03 dark mode was withdrawn (see Part Five, "Dark Mode — withdrawn") and `use-theme.ts` now returns `Colors.light` unconditionally, ignoring the system setting entirely. Verified 2026-09-08.
>
> **So the app no longer flips at sunset, and it does not honour a user choice either.** The rule below has two halves — *persistent* and *chosen* — and the current state satisfies the first by removing the second. A phone in dark mode gets a cream screen, which Part Five records as a deliberate, accepted cost rather than an oversight.
>
> **What remains genuinely outstanding** is the control itself: a persisted preference (a `user_profile` column, matching the three existing "shown once" flags), a provider in place of the bare hook, and a setting on the Settings screen that has existed since item 41 shipped on 2026-08-31. Not started.
>
> **The lesson is kept because it recurred.** The original deviation was documented with a sound reason — no settings screen existed to hang a preference on — and that reason expired without anything re-checking it. This note then made the same mistake one rung up: it described a state that was corrected the following day and stood wrong for five. **A deviation note needs a re-check trigger, not just a reason.**
...
**CHARCOAL IS A TEXT COLOUR ONLY, never a background and never a brand element (added 2026-09-10).** Stated by Ruth on seeing a pitch deck that used charcoal and forest grounds across four of eleven slides. The palette reads as a dark-neutral set if you only look at the hex values, and charcoal is the obvious candidate for a dramatic ground, which is exactly the mistake. **Selodía's brand is light.** Grounds are cream and sand; terracotta and sage carry emphasis; forest is an accent, not a canvas. Anything produced in the brand, decks and marketing assets included, sits on cream or sand unless there is a specific reason not to.

**Two practical notes that follow from it.** Terracotta at its literal `#C97458` carries only 3.10:1 as small text on cream, under the 4.5:1 AA floor, so headings and small text use a darkened terracotta around `#834B39` rather than the brand value itself. And sage is a **line and fill colour, never text** at 2.28:1 on cream. Both are already enforced in `mobile/src/constants`; this records them where the palette is defined, because the deck reached for the literal values and had to be corrected.
...
*(Asset caveat: the render used the terracotta master recoloured to sand `#E9D6C2` in the SVG, because the asset pack has no sand variant — `selodia-mark-reverse-on-terracotta.svg` exists but its exact fill was not verified. The shape is identical and the result stands, and **Logo Asset Pack v2.0 now ships real sand lockups on the `#834B39` ground**, so the shipped dark lockup no longer depends on a recolour.)*

> **Logo Asset Pack v2.0 is complete (2026-09-02), and v1.0 is archived.** 190 files in `Build Specs/Branding/Logo Asset Pack v2.0/`. The newly drawn work is 31 SVGs, 93 PNGs at 512, 1024 and 2048 on the longest edge and 31 PDFs, organised into `stacked-lockups/`, `horizontal-lockups/` and `wordmark-only/`, each holding the SVGs plus `png-512/`, `png-1024/`, `png-2048/` and `pdf/`. Alongside them, `mark-only/` carries the 34 Seed Mark assets forward unchanged, in `svg/`, `png/`, `pdf/`, `animated/` and `jpg/`.
>
> Every SVG is fully outlined. No file contains a `<text>` element or a font reference, and no PDF embeds or subsets Comfortaa, so a vendor without Comfortaa Light installed still sees the real wordmark. Spacing went through HarfBuzz rather than raw advance widths, because Comfortaa carries its kerning in GPOS and the difference is 30 units per 1000 across this word: the files match what the browser renders on selodia.app.
>
> Geometry is measured from ink rather than from bounding boxes. The Seed Mark master carries a 92-unit inset on all four sides, which is why v1.0 ended up with 85px above the mark against 49px below the wordmark. v2.0 pads the ink, giving even 48px margins, with the wordmark at 111% of the mark's ink width and the same wordmark size in both the stacked and horizontal lockups.
>
> Every variant exists with its ground baked in and again as `-transparent`. Transparent files are named for their ink alone, so colourways differing only by ground resolve to one file: there are 5 transparent lockups to 6 solid, and 4 transparent wordmarks to 5 solid. The pack's README says which name to reach for.
>
> **Nothing in the Branding folder draws the old wordmark any more (2026-09-02).** `Logo Asset Pack v1.0/`, both stale ZIP archives, and the Exercise Animatic kit's two lockup subfolders and two lockup PDFs were all moved to `Selodia archive/Logo Asset Pack v1.0 — superseded/`. There is no longer a wrong file to reach for, so the warning that stood here is gone rather than narrowed.
>
> Two things worth keeping from how that went. Archiving v1.0 wholesale briefly took the only copies of several **current** Seed Mark assets with it, because v1.0 held the mark as well as the old lockups: sage, forest, cream, both reverse variants, the three animated breathing SVGs and the 16 to 128px sizes existed nowhere else. Those were never superseded and now live in `Logo Asset Pack v2.0/mark-only/`. And the animatics kit is now a **mark-only** kit: its `README.txt` was rewritten to say so and to send anyone wanting a lockup to v2.0, because a vendor-facing manifest listing folders that no longer exist is its own kind of wrong file.
...
**Also note:** forest is deliberately absent from the app's colour tokens — nothing in the product renders it, and adding a token before something needs it would be colour nothing draws.

> *Updated 2026-09-02.* This previously said sage AND forest were absent. **Sage became a real token on 2026-08-31**, in both light and dark, when the Almanac empty-state illustration needed a line colour (`mobile/src/constants/theme.ts`). That is the stated rule working exactly as written — the token arrived when something drew with it — so only the status sentence was stale, not the principle. Forest is still unused and still absent. The accessibility note below still applies and is still recorded at the token: sage is a line and fill colour, never type.
...
> **Status, 2026-09-02: the asset exists, the app does not use it.** The breathing pulse is a real, finished brand asset — `animated/unflump-mark-breathing-*.svg` in the Logo Asset Pack, scale 0.985→1.015 over 3.4s ease-in-out on the whole mark, with its own `prefers-reduced-motion` guard — and it is live on the selodia.app landing page. Inside the app, `mobile/src/components/animated-icon.tsx` is a splash *reveal* (a 600ms scale-down and fade on launch), which is a different moment and not this. No loading state in the app currently breathes. Described here, not built there.
...
### The Me tab, built — 19 September 2026

The brief was confirmed on 12 September; nothing could write to it until today.

**Adding.** Me is the fourth type on the conversational save — which is what `pending-save.ts` was built for: *"Built once, here, so Me and Movement can reuse it."* A card needs a **section** (open, created by its first card arriving) and a **why**. The why is required: a name with no reason is a checklist item, and Me is not a checklist. Status is optional and is one of Ruth's six words — Taking, Ordered, Dietary source, As needed, Active, Paused — never a tick.

**Ask the one question (Ruth, the same morning):** the first build refused silently when no reason was given, and she called it *"too strict. Perhaps there should just be a question."* Her flow is now the behaviour: *"I've started taking D3"* → *"is that for the winter dips, or something else? And is it daily?"* → the answer → the offer. One question, never an interrogation, dropped if brushed off. The offer names the tab: *"Want me to keep that in your Me tab?"*

**Changing.** *"I've stopped taking magnesium, it wasn't helping"* moves the card to Paused. **No offer first** — saying it is the instruction. **Nothing deleted; the history is appended**, so a card can say taken from October, paused in March because it wasn't helping, taken again in November. **It finds the card or it asks**: "skincare" matching both the morning and the evening routine is a question, never a guess.

**The view is a document, not a feed** — sections in the brief's order, name and status on the surface, tap for why, when, and how it has changed.

**Export.** One tap opens the protocol in the phone's browser, everything laid open, set in the app's type, with print, save-as-PDF and share from the browser itself — so no native module and no rebuild. It reaches a browser with no session without widening the service role (kept to one job: licensed demo clips): the page is rendered with her own session, stored fifteen minutes in `me_exports`, and fetched by an unguessable id through one `SECURITY DEFINER` function that can do nothing else. Verified against the database: a live id returns the page, an expired or unknown one returns nothing, the table cannot be listed.

Probes: `probe-me-save.mjs` (21), `probe-save-offer.mjs` (10), `probe-me-update.mjs` (16).

### One plan saved three times is one plan

The same write-level guard as the food, on the Almanac: same kind, same title ignoring case and punctuation, within ten minutes, is the same entry, and the newer content **updates** the older rather than being dropped — because the Full-Body pair showed the second save can be a correction — keeping the older row's id so recorded workouts stay attached. `probe-almanac-same-entry.mjs`, 6 checks.

**The existing duplicates, and a boundary that held.** Ruth authorised merging her duplicate plans. The backup taken first showed they belonged to **two accounts**: hers held only the Full-Body pair, and the three Bent-Over Row and two Side Splits copies belong to a second account created 1 September with no name set. Only her pair was merged — all 9 sessions and the working weight onto the corrected plan, then the empty copy removed. **The other account is untouched**, pending her confirmation that it is hers.
...
3. **~~Save to the Me tab is broken~~ — built 19 September 2026.** A skincare routine saved into Insights instead of Me, twice. **It was never a routing bug: there had never been a pathway.** `insights.ts` sorts an entry into Me when its kind is `me`, and its own comment said *"Nothing carries it yet, so today every Me list is empty."* Anything offered to Me had exactly one place to land. See **The Me tab, built** below.
4. **~~The insight card has no hierarchy~~ — fixed 19 September 2026.** The title and the preview were both 14pt. Now the Movement library's measure: the name in the serif at 21, the kind small, muted and spaced above it, the first line beneath at a readable size. *Original report:* Title, category label and body all render at the same weight. Title large, category small and muted, body readable.
5. **~~Flower petals are not tappable in the Almanac~~ — fixed 19 September 2026.** The Almanac drew the same flower as Today and never passed it Today's handler. It now opens the same detail screen; that screen lives under Today, so the tap crosses to that tab and back returns there — one screen rather than two copies that drift. *Original report:* Tapping Strength or Cardio should show what fed that petal; it does nothing. *(The Today flower does have working targets — see Part Eight — so this is the Almanac copy of the control.)*
6. **"Ask about this" does not carry the context card — partly fixed 19 September 2026.** The opening turn on 18 September worked on the server: the right entry was tagged and the reply named the oxtail dinner. What the data showed instead was the tag **never letting go**: it rode 38 messages, all evening, through subjects that had nothing to do with the dinner. The second time (a pizza did the same for two hours on 16 September), and the same structural cause: the 45-minute silence rule measured from the previous tagged message, which every carried message is, so a continuing conversation reset its clock each turn. A six-message limit was tried the same morning and **reversed by Ruth the same day**: *"The anchor card is not just UI - it preserves the context of the conversation. Without it, the transcript quickly becomes difficult to understand because references like 'it', 'that meal' or 'the protein' lose their meaning... Please don't remove it after an arbitrary number of messages."* A discussion now ends only when **she opens another "Ask about this"**, **she closes it herself** (a quiet line above the message box names the topic, with a Close), or **Selodía is confident the conversation has genuinely moved on** — with logging something new and a 45-minute silence kept as forms of the third, since neither counts messages. **The real fault was that Selodía never judged:** every carried turn was handed the opening instructions ("they tapped Ask about this"), so forty messages in it was still being told the card had just been tapped. Carried turns now get their own instructions asking for that judgement, with a high bar, because the anchor is what keeps a transcript readable months later. The missing visible card could not be reproduced from the data and needs one test on the phone. *Original report:* The eye on a food entry opens Chat with a generic "Sure, what would you like to know?". The data partly arrives (the model knows the meal) but the visible card does not.
7. **~~Voice does not route to headphones~~ — fixed 19 September 2026, needs a phone test.** Not this app's fault: the ElevenLabs React Native SDK (1.2.26) configures every call with `android.preferredOutputList: ["speaker"]`, a one-entry list, so Android always chose the loudspeaker. LiveKit's own order — Bluetooth, wired headset, speaker — is restored by wrapping `AudioSession.configureAudio` once at the voice provider (`voice-audio-route.ts`), without patching the library. The earpiece is deliberately left out. *Original report:* Audio comes out of the phone speaker with headphones connected.
8. **~~Photo logging inaccurate~~ — fixed 19 September 2026.** Three causes. The photo path used the smallest model, and plated food is where that shows — it now uses **Sonnet, for photos only** (flagged: roughly a penny a photo rather than a third of one). It went straight to macros, so a wrong identification arrived as confident as a right one — it now **names each item first**, checks look-alikes, and marks what it cannot tell apart as uncertain. And what it saw was never kept — every photo entry read "(photo upload)", so a misreading was invisible and uncorrectable; entries now read **"Photo: cheese omelette, rocket…"**. *Original report:* An omelette was read as a baked potato.
9. **~~The insight trigger misfires~~ — fixed 19 September 2026.** Her own entries showed both halves: knee pain saved as two symptoms on consecutive days (surface), and a conversation working out why sugar makes her face puffy filed as a *symptom* rather than an insight (missed), saved twice. The model had never been shown what her Insights already held, so it could neither avoid repeats nor notice recurrence — her newest fifteen are now in context. The rules were rewritten: a symptom is a single **new** observation; an insight is a **pattern** — if it has a WHEN in it, it is an insight; the analytical conversation is the moment to offer one; recurrence is named, not re-offered, and repeated recurrence is itself the insight. *Found on the way:* two examples in the shared prompt were specific to Ruth or read as facts about the user (her friend's name; an invented ferritin result). Both made generic. *Original report:* It fires on surface symptom observations and misses genuinely analytical conversations. It needs better judgement about what is insight-worthy.
10. **~~Micronutrients are refused rather than offered~~ — fixed 19 September 2026.** Asked about iron, Selodía now answers, says plainly it does not measure micronutrients in numbers, and offers to watch for it in the food logged. On a yes the interest is stored in durable context as `watching`, and the nutrient pass treats it with the same standing as a health marker. **It never puts a number on it**: there are no iron figures in the app, so it speaks in foods and patterns, never milligrams. *Original report:* Asked about vitamins or minerals, the app says it cannot track that and stops. It should offer to start: *"I don't currently track micronutrients in detail, but if iron or vitamin C is something you want to monitor, I can start noting it. Want me to?"* Closing the door contradicts the proposition that anything can be brought here.
11. **~~Fabricated context on the measurement screen~~ — fixed 19 September 2026.** The actual message, from the weigh-in photo reply: *"Weight's up 0.3 kg since yesterday, but yesterday was on the salty side and you had a hard session a day or two ago"* — directly beneath a facts block reading **down 0.1 vs 2 days ago**. Two inventions: causes that route is never shown, and a number contradicting the line above. The prompt already forbade invented comparisons and did not hold, so the reply is now **checked in code before posting**: a weigh-in read that names a cause it cannot know (salt, water, training, cycle, sleep, carbs, alcohol, hedged or not) or a measured number it was never given is dropped, and the facts and interpretation go out alone. `scripts/probe-ack-invented-context.mjs`, 15 checks, the first being her real message. *Original report:* A comment on a weight change invented reasons — "yesterday was salty", "you had a hard session a day or two ago" — with nothing logged to support either. The witness principle applies: reference only what is there, and never invent context to sound reassuring.
12. **~~Tab labels still truncate~~ — fixed 18 September 2026.** "Foo", "Activit", "Measurement" in the Log tab, and "Movemen" in the Almanac, reported after the shared strip landed — so six rebuilds had not fixed it. **The cause was the typography pass, not the strip.** `segmented-tabs.tsx` had a rule written into it from the earlier attempts: keep the label on the system face, because Android measures in the system face and drawing another makes the word wider than the box it was given. It kept that promise — until every body style moved to Manrope, which quietly put a custom face back into this control and brought the gap back with it. **The fix does not depend on the measurement being right:** six points of padding INSIDE the text box, so the box is wider than the glyphs need whichever face measures it. The label also drops to 13, and the strip may sit past the page margin — three labels can be wider than the text column, and a switch is a control, not a sentence.
13. **~~Settings is only reachable from Chat~~ — fixed 19 September 2026.** The same quiet word on every screen, from one `SettingsLink` component, placed so no heading moves by a point: in the top margin on Log and Almanac (dropping by the status bar's height on Log, whose content runs under it), and at the end of the date line on Today, because "Good afternoon," at 50pt runs under a top-right corner.
14. **~~The Body card truncates on Today~~ — fixed 19 September 2026.** "38.5 kg m…", "27.7 % bo…". The clipping was designed in — the unit was set to shrink and clip first — and "38.5 kg muscle" needs about 93 points in a square with about 78. Now **56.5 kg / 38.5 muscle / 27.7% fat**, and `numberOfLines` is gone from the unit so a narrower phone wraps rather than cutting a word. The protein line had the same fault unreported and gets the same fix: **111g protein**.
...
### Found on device, 19 September 2026, and fixed the same day

All server fixes are live; the phone fixes shipped as EAS updates. **None is verified on her phone yet**, and the voice ones can only be proved by a real call.

18. **One day's food, spoken once, went in as five rows.** ElevenLabs resends a turn, longer, while she is still talking, and the only thing stopping each version logging was a line in the prompt. Breakfast went in twice (407 and 463 kcal) and the mango three times. **Fixed at the write** (`app/lib/voice-supersede.ts`), and **rebuilt the same afternoon after an independent review** found the first version could delete food she ate: it cleared every food row created since the earlier turn, from any source, before the new parse had saved. Now every row a voice turn writes carries that turn (`food_logs.source_turn_id`), and after a turn has saved, rows from earlier versions of the SAME sentence are removed - or its own rows, if a fuller version already saved. Whichever finishes second tidies, so a failed later turn loses nothing, and typed or photographed logs are never touched. The prompt for a continued turn had also said "do not log again" while the code needed it to log the whole sentence; it now says the latter. The first version did work once on device (a two-part sentence went in as one row); the rebuilt one is unproven.
19. **The same turn arrived twice in the same second**, so both copies ran: one corrected an entry and the other logged it again as new. The adapter's replay check reads the thread before the turn is written, so two simultaneous requests both pass it. Now checked straight after the turn is written: a later copy with no reply between it and the first writes nothing and answers with the first copy's reply.
20. **A food correction replaced the entry instead of correcting it.** "The chocolate caramel was just one tiny caramel the size of a Malteser" was parsed on its own and written over the entry, and the candied orange logged with it vanished. The model now sees the entry as it stood plus the correction, and returns the corrected whole.
21. **Food memory handed back the number being corrected.** Memory (Part Eleven, v2) offered the 240 kcal chocolate from twenty minutes earlier to the re-log that was meant to fix it. Memory now ignores anything younger than twelve hours: consistency is a between-days problem.
22. **After a voice call, nothing showed in chat or the log until the app restarted.** Two causes. The call is a modal over Chat, so closing it never triggered Chat's re-read; it now re-reads as the call ends and six seconds later. And the Food, Activity and Measurements views loaded once in a tab that stays mounted; they now re-read on arrival and again at 4 and 10 seconds, because a spoken meal is saved a few seconds after the reply and she went straight to the log to look (`hooks/use-focus-reload.ts`). A realtime subscription would be the complete answer; it needs the database's realtime publication, which is not switched on.
23. **The voice-note mic was recording after a call.** The whole call screen is one "tap to end" target and it fades; a second tap during the fade most likely landed on the mic beneath. The mic now ignores taps for 1.5 seconds after a call ends.
24. **Step tracking could be turned on in Settings but not off.** Now it can: the preference stops the reading everywhere (the Today panel used to read regardless), and on Android the permission is handed back to Health Connect. iOS cannot give a permission back, so it says where to remove it. Steps already recorded stay.
25. **Consent choices could not be changed after onboarding.** Settings now has "Your choices": the updates and research boxes can be changed at any time, each change a new consent record, and withdrawing core consent is said plainly to mean deleting the account (UK GDPR Article 7(3)).
26. **"Activit", "Measurement / s" and "Setting" came back.** The segmented control's comment promised "Measurements" room to spare in a third of the track; at 88% of her screen a third is about 100 points, the segment padding left 76, and the word needs about 88 - more with a larger phone font. Padding cut to 4, and the label may now shrink to fit (never below three quarters) with font scaling capped at 1.3 for this control. The Settings link lost its last letter to Android measuring in one face and drawing in a wider one: the s was wrapped onto a hidden second line, not clipped, so padding did nothing; the corner link now has a right-aligned box wider than the word. **Confirmed on her phone the same evening:** "Activity", "Measurements" and "Settings" all whole.

### Settings, restructured as a hub (20 September 2026)

Her brief, after a long look at the information architecture: *"The issue isn't the design - it feels too small because we're treating it as a single page rather than the entry point into all account-level configuration."* Three concepts stay separate and keep their own mental model: **Chat** is conversation, the **Almanac** is her accumulated knowledge, and **Settings** is how Selodía works.

**Settings is now a navigation hub** (`mobile/src/app/settings/`), each row its own page: Profile, Connected devices, Notifications, Privacy, Data and export, Help and support, About. The test it has to keep passing is hers - when a feature arrives, where it belongs should be obvious.

**Profile is new.** Name, email, date of birth, biological sex, height and everyday activity, each editable in place; every one of those was asked for in onboarding and then never shown again, so a wrong answer could only be fixed by conversation. Goals are shown and not edited here: they are agreed in conversation where the reasoning is, and a dropdown would become the place people pick a goal without that conversation.

**What was left out, and why.** *Appearance* - the app is light-only by decision and text size follows the phone, so the page would hold a heading and nothing else. *Garmin, Oura, Fitbit and smart-scale rows* - there is no integration, and "Not connected" beside something that cannot be connected is a promise the app has not kept; the page says so in words instead. *FAQ, release notes, licences* - not written. *Quiet hours and insight notifications* - nothing to schedule. Principle 8 forbids a control that does nothing, and it applies to a page as much as to a switch.

**The Almanac's Me tab stays where it is**, at her instruction: *"Please do not move the Almanac 'Me' page into Settings ... it's my externalised memory"*, where Settings is configuration. Two different purposes, two different homes.

**Also built with it:** daily reminders can be turned off, not only accepted once - the same fault step tracking had, and the same fix.
...
### Reading a letter she was given (20 September 2026)

She tried it, and the app refused her twice. Her own account is the brief:

> *"I wanted to upload the consultant letter with the mri results and the letter so it could extract it and add summary to Me tab. This is how I dealt with it in real life with Claude and I didnt understand the letter fully tbh. This is something selodia needs to be able to do to help ppl."*

The capability had been scoped since Part Twelve (*"User uploads a PDF, AI extracts the relevant constraints in plain language"*) and parked behind a native module. The photo half needed no module at all: `expo-image-picker` was already in the build and a classifier already routed a photo to the right parser, so a medical document became a **fifth kind of image** and shipped over the air the same evening.

**What is kept, and why it is more than a summary.** Asked whether the document itself should be stored, she chose the extracted record only — with a condition that shaped the whole design:

> *"it needs to keep the information that makes the letter not needed, so insurance numbers and all of the details that can make the thread verifiable on a phone call or visit to a clinic. Otherwise its not helpful. You should be able to ask selodia: I've been having wierd symptoms in x again, can you remind me how to get seen?"*

So this is not reading comprehension. The record has to **stand in for the letter at a reception desk**, and the card is written in the order of that phone call: who and where, what to quote, what was found, what it means, what was agreed, how to get back in. A record that explains the findings beautifully and loses the hospital number has failed at the thing she asked for. Nothing is stored of the pages themselves; the bytes are read and gone when the request ends, which is what the report's cover already tells clinicians.

**Three kinds of content, kept apart on purpose.** What the letter **says**, in its own words. What the terms **mean**, labelled every time as an explanation of the vocabulary and never a new opinion — because she asked for this feature precisely because she could not follow her own letter, and the moment an explanation stops being labelled it starts being mistaken for what the clinician wrote. And what to **quote**, transcribed exactly. The model may not diagnose, may not give a prognosis, may not advise, and may not resolve a contradiction in the document — it reports both parts.

**The numbers are read twice, and this is the interesting part.** Every other guard in this app checks an output against data the app already holds. Here there is no such data: the only record of what the letter says is the reading itself, so the only possible check is *another reading*. Two calls that cannot see each other transcribe the page, and a reference is marked confirmed only where both agree, comparing on digits while treating spacing, punctuation and case as the noise they are.

A number that differs is **kept and marked to check**, not dropped — a half-read hospital number she can verify against the letter beats a blank, and she still has the letter at the moment the card is made, which is exactly when the mark is useful. But it is never offered **twice**: a review of the first version showed both readings being printed, which turns a record into a puzzle to solve at a reception desk. One value, marked, is the honest form of "I am not certain"; two values is the app declining to answer while looking like it did.

**It offers, it does not save.** Through `pending-save.ts`, whose own note says why: *"the database holds the offer, the model only reports whether the answer was yes, and the app saves exactly what was offered."* That matters more here than anywhere it has been used, because what gets written is a hospital number she will read aloud on the telephone. The offer is also written to the thread — found while checking the path end to end, the offer lived only in screen state while the pending save outlived a restart, so she could have returned to an empty thread and later agreed to a record she could no longer see.

**Pages are gathered before anything is read**, which makes a document the one thing in this app not handled the moment it arrives. A consultant letter is three sides and the number is on the first while the plan is on the last; reading each photograph as it came would produce three records of one letter, each missing what the others had. Asked how multi-page should work she declined the incremental option — *"Lets just get those right the first time. Please make it so it can accept all photo formats and pdf"* — so the tray takes a photograph, several gallery images at once, or a PDF, and reads them together. Photo quality is **1, not the 0.6** every other path uses: a plate of food survives compression and small printed type does not. A letter photographed on the Today quick-log bar is carried to Chat rather than taken again, and dropped on sign-out.

**"All photo formats" has one honest exception.** The model reads PNG, JPEG, WebP and GIF, and not HEIC — which is what an iPhone writes by default. HEIC was briefly in the accepted list, which would have uploaded it to be refused upstream and reached her as a blank failure after a long wait: the exact shape of error the morning was spent removing. The camera and gallery hand over a converted JPEG, so it only arises for a `.heic` chosen as a file, and the app says so before the upload. Converting it needs a native image module, queued for the next build rather than papered over with a list that lies.

**PDF needs a native build**, because `expo-document-picker` is native code and an over-the-air update cannot add any. The option is therefore asked for at the moment the sheet is drawn rather than listed, so it appears only on a build that can honour it — principle 8, applied to a control that would otherwise work on one install and not another.

`probe-clinical-document.mjs`, 45 checks, most of them about the parts she will read out loud.
...
>
> **Phase 1: 32 of 48 built (67%) · 2 partial · 1 retired · 13 not built.** *(As reconciled on 31 August. Built since, the same day: 26 zero-calorie quick-tap, 41 export/settings/deletion, 20 Weekly Roundup, 14 habit window, 16 Then & Now — and 38 found already complete, its "still open" line stale. Then 45 onboarding first-log, 29 persisted interpretation notes, and 23 navigation Layer 1 (Tier A; the tab-icon half is closed by a platform constraint rather than outstanding). **Standing at 41 of 48 built, 1 partial, 1 retired, 5 not built** — and on 1 September item 41 went from complete-but-partial to genuinely complete, the auth shell included, which changes no count because it was already being counted as built. The partial is 42 (allergies): capture, storage and awareness built the same day, part (c) — the executed filter gate — left open and deliberately NOT counted as built, since the whole point of that part is that prompt-level care is not the guarantee.)*
>
...
>
> ~~**Partial:** **20** Weekly Roundup — there is no weekly route and no weekly output. **26** zero-calorie drinks — the *quick-tap shortcut* is not built.~~ **BOTH CORRECTED 2026-09-09; both were false.** **20**: `app/api/weekly-roundup/route.ts` exists and is 243 lines, alongside the `weeklyGate` / `WEEKLY_MIN_FULL_DAYS` machinery in `roundup-rules.ts` that this line correctly credited. **26**: the quick-tap is `mobile/src/components/hydration-quick-tap.tsx`, 180 lines, mounted in `overview-panel.tsx` and writing to `hydration_logs` with an undo. Ruth pointed out the hydration half from memory of using the app; the weekly half surfaced from the close-out check written the same afternoon, which is the argument for that check in one line.
>
...
>
> **Confirmed not built (rewritten 2026-09-08 — this line contradicted the paragraph directly above it).** As originally written on 31 August it listed ten items, and the update note two paragraphs up then recorded six of them as built the same day without amending this line. A reader scanning for status could land on either and be confident in the wrong one. **The list below is the reconciled one; the paragraph above is the count.**
>
> **Still not built — rewritten 2026-09-09, and it was wrong in three ways.** **24** Graduation moment and pause-mechanism triggers — `habit-window.ts` has the entry point the taper and Graduation are meant to call, so the mechanism exists and the triggers do not. · **25** navigation Layer 2, a fast-follow, inheriting Layer 1's tab-icon platform constraint; `spotlight-provider.tsx` names it as explicitly later. · **22** the Meal/Order Advisor — **omitted from this list entirely until now**, with nothing built for it. · **34** the voice note — also omitted, and easy to mistake for built because seven `voice-*` components exist; those are Part Eighteen's conversational agent, which item 34's own entry says explicitly does not replace it. · **46** conversational substitution — ~~blocked on per-movement metadata that does not exist in usable form~~ **that blocker is false and has been since 2026-08-28**: `movement_classification` carries all 770 clips with muscles, equipment and pattern, 377 human-verified. It is now held for a different and better reason — at 14% library coverage the substitution would fire on most exercises, which would mean steering people away from what they asked for to fit a video library. Waiting on item 49.
>
> **Built since that audit, and previously listed here in error:** **14** confidence/habit bars · **16** Then & Now · **20** Weekly Roundup · **23** navigation Layer 1, Tier A (the tab-icon half is closed by a platform constraint, not outstanding) · **26** zero-calorie quick-tap · **29** persisted interpretation notes — **complete; the display surface shipped 2026-09-09** as `reading-card.tsx`, recorded under item 30 · **41** data export / settings / deletion · **45** onboarding steps 6-7.
>
> ~~**Partial, and deliberately not counted as built:** **42** allergies — part (c), the executed filter gate, remains open and is hard-blocked on item 22.~~ **42 IS COMPLETE as of 2026-09-09.** Capture, storage, permanence and prompt-level awareness shipped 31 August; part (c) shipped 9 September as `allergy-gate.ts`, all four layers, verified by `scripts/probe-allergy-gate.mjs`. **The hard block on item 22 was backwards** and is the more useful half of this correction: the gate was waiting on the feature that would make the risk legible, while the risk itself was already live — Selodía answers “what should I have for dinner?” today, and `allergies.ts` said so in its own prompt block the whole time (“never suggest … in anything you propose”). `allergies.ts` still labels its block as awareness and explicitly not the guarantee, which remains correct: the guarantee is now the gate beside it.
>
...
10. Wire basic chat-based logging in the native app. Every food/activity/measurement-related step from here on assumes this baseline exists; nothing before this point in the build order actually builds it. Identified as a real gap during step 7, not assumed away — inserted here rather than discovered partway through step 11 later. Split into two, since one half needs a native module and the other doesn't:
    ...
    - **10b. Photo input.** `parse-body-measurement` is photo-only on the backend (no free-text path exists), so body-measurement logging belongs entirely here rather than to 10a. Needs `expo-image-picker` — batch for a later native rebuild alongside the Almanac's animation library and notifications, per the native-module milestone note above.
...
22. The Meal/Order Advisor — a standalone, reactive feature suggesting choices given remaining daily targets and a stated context; can be built whenever convenient once core logging is mature. **HARD DEPENDENCY on item 42, added 2026-08-31: this cannot ship without the allergy gate wired in as a structured filter — an executed check over the candidate suggestions, not an instruction in a prompt.** Item 42 built capture, storage and prompt-level awareness on 2026-08-31 and left part (c), the gate itself, open, because at that point there was no structured food-suggestion path in the app to gate. This item IS that path. The gate is therefore not a follow-up to the advisor and not a later hardening pass — it is part of the advisor's definition of done, and "the model was told about their allergies" does not satisfy it. **Dependency satisfied 2026-09-09:** item 42's gate is built and running on the chat path already, so item 22 inherits it working rather than having to wire it. What item 22 must not do is route around it — any new suggestion path has to return through `ask-unflump`'s reply assembly, or set `suggestsFood` and call `runAllergyGate` itself.

    **BUILT 2026-09-09, and NOT as a standalone feature.** The item describes “a standalone, reactive feature”; what shipped is a capability on the existing chat path, which is the shape the rest of the product already argues for — one text box, no dead pages (principle 8), and a suggestion screen would be a control somebody has to find before they can ask a question they can already ask. It also means the allergy gate is inherited rather than re-wired, exactly as the paragraph above requires.

    **The real blocker was not the advisor, it was blindness.** Until today `ask-unflump` carried the food already logged and **no target of any kind** — `protein_target_g` appeared in it exactly zero times, and `calculateCalorieTarget` existed only in `mobile/src/lib/calorie-target.ts`, where the Overview renders from it. So Selodía could see what somebody had eaten and had no idea whether it was a lot or a little **for them**. “Choices given remaining daily targets” is not buildable while the remaining targets are invisible, and the same blindness quietly limited every ordinary “how am I doing today?”.

    **`app/lib/daily-targets.ts`** is the fix: today's intake from `food_logs`, TDEE from the existing server primitives (a measured scale BMR preferred, the Mifflin-St Jeor estimate as fallback), the Fat×Muscle matrix, and the protein target — assembled into a `TODAY SO FAR` block stating what was eaten, what the target is and what is left. **Every field is independently nullable and nothing is fabricated when the data is absent**: with no derivable target the block says so explicitly and forbids stating, estimating or implying one, which is the failure Part Eight already recorded once when a bodyweight-derived protein figure was displayed as though measured. The day boundary is server-local midnight, **matching `daily-roundup` exactly**, because two day boundaries in one app eventually disagree in front of somebody.

    **The conduct rules are the actual work; the arithmetic was easy.** Say the number only when it earns its place — most of the time it should shape what is suggested without being said aloud, because reciting a macro budget at somebody deciding on dinner is the tracker register this app exists to avoid. Two or three real options in a sentence or two, never a numbered meal plan and never one correct answer handed down. **No food is good or bad**, nothing is a treat, a cheat, a reward, or something to earn or make up for; if what they want does not fit the numbers that is usually not worth mentioning, because a day is not a budget to balance to zero and somebody who wanted chips and got a lecture stops asking.

    **The matrix is now duplicated across the Next/Expo boundary**, deliberately — there is no shared package, and `body-metrics.ts` is already mirrored the same way. That duplication is guarded by `scripts/probe-target-parity.mjs`, which runs **both** implementations over 315 combinations and fails on any disagreement: 0 mismatched. This is pointedly the thing `onboarding-openers.ts` once *claimed* was keeping it honest when no such test existed. If the two ever drifted, somebody would see one target on the Overview and be advised against a different one in chat, with nothing on screen to explain the gap.
23. In-app navigation Layer 1 — **Tier A built 2026-08-31; the tab-icon half is closed by a platform constraint, not outstanding.** See the KNOWN PLATFORM CONSTRAINT under Part Six. What ships is the in-screen spotlight in full — dim, terracotta pulse, tap-anywhere dismiss, and the Phase B destination highlight with its anchored message — driven by a model-emitted `navigationTarget` validated against a registry, so an invented target is dropped rather than pointed at. What does not ship, and cannot, is pointing at a tab icon. **What shipped, in detail:** 18 registered targets across 7 files (Chat's Settings link, "+" and message box; the Body tab's four segment buttons (**superseded 2026-09-03**: the segments became routes, so `body.overview` was retired and the other three now point at the summary screen's three tappable headings); Overview's stat cards, calorie, protein and water/quick-tap; today's food entries; the Measurements week stepper and data link; the Almanac's category grouping and an opened category's entries; Settings' export and delete). A `SpotlightProvider` at the root — above the Stack, so a request survives the navigation it exists to follow — plus a `SpotlightTarget` wrapper that measures on demand and scrolls itself into view first, a `SpotlightScroll` context so a screen declares its scroller once instead of four components growing a prop, and a Modal overlay drawing the dim as four rectangles around the element rather than a mask. **Phase B is the common path, not the fallback.** A request whose route is a tab icon has no pointing stage at all, so it waits: Unflump answers in words, and the destination highlights when the person arrives — by tapping the tab, or a minute later of their own accord. The one route that completes the full Part Six journey end to end is Chat's Settings link, which is the only in-screen control that crosses screens. **`navigationTarget` is validated server-side** against the registry and dropped if unknown, with a second check client-side because the two packages hold separate lists. **Device verification outstanding:** cut-out alignment, the scroll-then-measure timing, and the pulse itself cannot be checked by typecheck or lint and need a real build. Everything fails safe — an unmeasurable target renders nothing rather than a ring at the origin — but "fails safe" is not "confirmed correct".
24. The Graduation moment and pause-mechanism trigger logic. **Investigated 2026-09-09 and NOT started, because it is blocked lower down than its own description suggests.** Part Eleven's step 2 is “consolidation begins once Fat Focus and Muscle Focus are both sustained at maintain”, sustained for nine weeks in step 3 — so the trigger needs both the current Focus states **and their history**. The app never sets the Focus states at all (see the note under Fat Focus / Muscle Focus), and nothing records when either last changed, so the trigger has neither input nor history. **What DOES exist:** `habit-window.ts` computes the nine-week window from `food_logs` and exposes `loadHabitWindow` as the single entry point both the taper and Graduation are meant to call, and `user_profile` already carries `food_paused_at`, `body_paused_at` and `activity_paused_at` for lite mode's per-area, reversible pausing. The mechanism and the storage are ready; the state the trigger reads is not.

    **STEPS 2–5 BUILT 2026-09-09, once Focus capture unblocked them** (`app/lib/graduation.ts`). `assessConsolidation` fires when both Focus states have been at maintain for **nine weeks**, expressed via `HABIT_WINDOW_DAYS` so the two nine-weeks in this app can never drift into different numbers. The offer is Part Eleven's step-4 wording, quoted verbatim and handed to the model rather than composed by it — “how do you feel about it?” is doing specific work a paraphrase would round off into a yes/no question. **Steps 6–10 are not built**: weekly check-ins, adaptive cadence and the two-year celebration need scheduled delivery and are a separate piece.

    **One condition was added to the spec's literal test, and it is flagged rather than slipped in.** Steps 2–3 say only “both sustained at maintain” for nine weeks. Taken literally, somebody who set maintain and then stopped using the app for two months qualifies — and would be told *“that's real, sustained work”*, which is the offer's own quoted wording. That is false praise, which Part Two forbids more strongly than it requires this trigger to fire. So the offer also needs them logging on **more days than not** across the window. Half is not a performance bar and sits far below the taper's strict 100%; it is the floor at which the sentence is true. Somebody below it has not been consolidating, they have been away.

    **An unstamped Focus state is not a long-standing one.** `fat_focus_since` only exists from 2026-09-09, so a state set before that — or by hand — has no known duration, and unknown must not be read as long enough. The clock starts when the app starts watching, which costs an existing user nine weeks and is the honest trade.

    **Asked once, and recorded as asked on the turn it goes out** — whatever the person then says, including nothing. An offer to stop using the app that reappears until it is answered is not an offer, it is a nudge with a question mark. **Lite mode** sets the three per-area `*_paused_at` columns plus `lite_mode_since`; they are timestamps rather than booleans so unpausing is setting one back to null and nothing is lost. Its standing prompt block says plainly that lite mode is a **designed, successful outcome, not a lapse**, and forbids nudging them to log or framing a return as getting back on track.

    **Verified** by `scripts/probe-consolidation.mjs` — 32 checks, all passing, most of them asserting the offer stays quiet: one day short, one state changed last week, already asked, unstamped, and logged-two-days-in-nine-weeks all correctly produce nothing.
25. In-app navigation Layer 2 — a fast-follow, not part of the initial build pass. **Inherits Layer 1's platform constraint (2026-08-31):** the self-playing demonstration is described as animating across the screen and performing the navigation, and any cross-tab version of that has the same problem — it cannot land on, or point at, a native tab icon. What remains available to it is the same in-screen scope Tier A has.
...
29. Persisted per-entry interpretation notes — persist interpretation-layer output as a **point-in-time note attached to the specific log entry it was generated for**, so historical browsing shows what Unflump actually said at the time, a diary record rather than a recalculated value. Deliberately distinct from the compute-on-read cycle-day decision; accepts that a retroactively-corrected input (e.g. a backdated period start) can leave an old note slightly inconsistent — the note still honestly reflects what was said. Only genuinely per-entry outputs are stored: the body-measurement reading interpretation (attaches to `body_measurements`) and per-item protein-quality flags (attach to `food_items`/`food_logs`); the day-level protein nudge and nutrient-depth flagging are *not* attached to a row (see Persisted Interpretation Notes for the full distinction). Storage must span entry types, not assume one table. Prerequisite for item 30. **Built 2026-08-31, measurement notes only — with one part deliberately not delivered and one deliberately declined.** The store is `interpretation_notes` (`20260831180000`): `entry_type` check-constrained to `body_measurement`/`food_log`/`activity_log`, `entry_id`, no foreign key — the same type+id shape `discuss_entry_id`/`discuss_entry_type` already uses, and for the same reason, since one FK column cannot point at three tables. Unique on `(user_id, entry_type, entry_id)`, and the write uses `ignoreDuplicates` so the **first** interpretation of a reading is kept and every later one is discarded: an upsert that merged would quietly replace history with the present, which is the exact failure the table exists to prevent. The write sits inside `loadLatestInterpretation()`, which is the right point precisely *because* it is not a log-time hook — it fires the first time a reading is genuinely interpreted for someone, on whichever of the three surfaces reaches it first, so the stored sentence is the one the person was actually shown. A null interpretation is not stored: recording "nothing was said" would make an empty note indistinguishable from an unvisited reading. **Not delivered: display.** The surface for browsing a past entry's note is the discuss-card (item 30 slice 4), still queued last behind on-device verification of the existing native modules — so today the notes accumulate correctly and the data export is the only place a person can read them back. That is expected: the value of a point-in-time record is entirely in having captured it at the time, and waiting for the viewer would permanently lose every note from the intervening period. **Declined: the per-item protein-quality flag, which this item originally listed as per-entry storage and which is now deliberately NOT persisted.** Point-in-time storage only means something where the inputs can change retroactively. The measurement note has that property — it reads `cycle_events` (backdatable), the trend across *later* readings, and activity/food windows, so recomputing it a month on genuinely says something different from what was shown. The protein flag does not: `perItemProteinFlag()` is a deterministic function of `food_items.protein_source`, which is already persisted at log time and never changes, so computing it on read in `food-breakdown-card.tsx` is stable forever and a stored copy could only ever drift from its own input. Persisting it would add a second source of truth and buy no history.
30. The "What's In Here" discuss-card system (item 13's deliberately-deferred sibling) — the overlay card for an entry surfaces its breakdown/reading plus any persisted interpretation notes (item 29), with an **"Ask about this"** button that carries the entry into chat; the conversation can go anywhere (a nutrient question, saving the entry as a recipe to the Almanac, a standalone idea). **Mechanism resolved 2026-08-21:** the card goes into the thread **as an image**, a shared visual reference both parties can see rather than structured data handed invisibly to the model, and Unflump opens with a simple **"What's on your mind?"** regardless of entry type — the card establishes *what* is being discussed, so the words only need to hand over the floor. See "The 'What's In Here' Discuss-Card". This is the interactive discuss layer — repost-to-chat, per-entry nested Q&A history, the has-discussion icon state — distinct from item 13's read-only breakdown *view*. **Slice 1 built 2026-08-21 (backend + storage + schema):** migration `…190000` adds `chat_messages.image_path`, `image_sent_to_model`, and a generic `discuss_entry_id` / `discuss_entry_type` pair — kept as type+id rather than a foreign key precisely because the mechanic spans food, activity and measurement entries living in *different tables*; only food is wired, and the check constraint names all three so the seam is real rather than retrofitted. Plus a **private** `discuss-cards` Storage bucket with per-user path-prefix policies (these render someone's own food log, which is health data). `app/lib/discuss-card.ts` owns upload, the pending-image lookup and the sent flag, degrading rather than throwing so a malformed card can never fail the underlying message. `ask-unflump` attaches the pending image to the newest user turn as an Anthropic vision block — reusing the shape `parse-food` already proved — and marks it consumed **only after the call succeeds**, so a failed call never burns the image's one chance to be seen. The flag is server-side deliberately: client-side tracking would be lost on an app restart mid-conversation. **Slices 2–3** (chat image rendering; per-entry history + icon state) ship over EAS Update. **Slice 4, the `react-native-view-shot` capture, stays last** — it is a *fifth* native module and must not stack ahead of the on-device verification of the current four. **Slice 2 built 2026-08-21 (chat image rendering only):** `chat-images.ts` exchanges stored paths for short-lived signed URLs in ONE batched call — the bucket is private, so there is no public URL to point an image at — and a path that fails to sign simply renders without its image rather than costing the thread its whole history. `ChatBubble` gained an image variant via `expo-image` (already a dependency, so no native module), using `contentFit: contain` because a captured card is a document rather than a photo: cropping to fill would cut off the very content the conversation is about. The history read now selects `image_path`. **The “Ask about this” button and the “What’s on your mind?” opener are deliberately NOT in this slice** — the capture that produces an image is slice 4, so shipping the button first would post a reference Unflump could not see, breaking the conversation rather than merely degrading it. They land with slice 4. **Slice 3 built 2026-08-21 (per-entry tagging + icon state):** a message carries `discuss_entry_id`/`discuss_entry_type` from the posting turn forward, resolved by the pure `resolveDiscussTag` — posting a card always wins (it is a deliberate act of changing the subject *to* that entry), otherwise the previous tag carries forward unless the model declares the topic has moved on via a new `discussTopicEnded` field on the classify tool it already calls. **No timer and no keyword matching:** a clock cannot tell a twenty-minute pause from a change of subject, and a keyword rule would re-import the closed-list problem principle 13 forbids. **Continue-by-default is deliberate, with its risk accepted rather than avoided** — silence keeps the tag, because requiring re-affirmation every turn would lose genuine follow-ups; the trade is a possible *stuck tag*, where one entry slowly absorbs a whole conversation. Known and monitorable (count consecutive messages sharing an id), not an oversight. Both halves of the exchange are tagged, so an entry's history reads as a conversation rather than a column of unanswered questions. The eye icon reads its state from **one indexed query per view**, never one per row; permanence needs no logic, since messages are never deleted so the set only grows. The accessibility label carries the state too — colour alone cannot convey it to assistive tech. See "The 'What's In Here' Discuss-Card".

    **THE ITEM 29 DISPLAY SURFACE BUILT 2026-09-09 — `reading-card.tsx`. Recorded here because it is where item 29's notes finally surface, and MISLABELLED “slice 4” when first written, which is wrong: item 30's slice 4 is the `react-native-view-shot` capture and is deliberately still last, being a fifth native module that must not stack ahead of verifying the current four. Slices 1–3 are built. The commit that shipped this carries the wrong label in its message and cannot be amended, so the correction lives here.** It closed a gap rather than adding a feature. Item 29 has persisted a point-in-time interpretation note against every body-measurement row since 2026-08-31, and `loadNoteFor` was written to read them back. It had **zero callers**: the notes were being written, stored, and shown to nobody. The reason is worth recording because it is a shape that will recur — `persistNote` is only ever called with `'body_measurement'`, and the only discuss card that existed was `food-breakdown-card.tsx`. **The writer and the only viewer did not overlap at all**, so neither half looked broken on its own.

    Measurement week-table rows now carry the same eye affordance every other detail surface uses, opening a card with the reading and its stored note. **A day with no reading renders no affordance** — a sparse week would otherwise be a row of controls that do nothing — but keeps the column width so rows still line up. **A reading with no note renders no note section**, no placeholder and no explanation: item 29 stores a note only when there was something worth saying, so silence is the ordinary case and principle 8 applies inside a modal as much as on a screen.

    The note is labelled **“What I said at the time”** rather than anything mentioning interpretation, because what makes it worth reading is that it is not recomputed. `food-breakdown-card.tsx` states the same principle from the other side — it deliberately has no note section because “computing one live here would contradict exactly what item 29 exists to guarantee”. **“Ask about this” remains absent**, exactly as on the food card: item 30's interactive half is unbuilt, and rendering the button first would be the dead control principle 8 forbids.

    `MeasurementRow` gained an **optional** `id`, and only the query feeding the card asks for it — `overview-panel` and `log-acknowledgment-facts` build the same rows from selects that do not, so a required field would have broken them.
31. Hydration tracking (Overview) — log water intake against a sensible daily target, surfaced as a tap/fill bar on the Overview in the same visual family as the calorie and protein bars. An honest same-day wellbeing nudge, **explicitly not gamified** (no streaks, no "don't break the chain", no badges). Real backend work: a water-intake log (likely reusing the existing silent-logging pattern, overlapping the zero-calorie-drinks path, item 26) plus a daily target. See Hydration Tracking. **BUILT — corrected 2026-09-09, and it had been built for some time.** `hydration_logs` carries the entries; `lib/hydration.ts` holds `QUICK_MEASURES`, `DAILY_TARGET_ML` and `hydrationLabel`; `hydration-quick-tap.tsx` (180 lines) is mounted in `overview-panel.tsx` and inserts by measure with an undo that removes the row it just wrote, by id. Conversational logging goes through `app/lib/hydration-logging.ts` on the `ask-unflump` path, so both routes in exist. The component's own header names the not-gamified constraint and honours it — no streak, no chain, no badge.

    **What shipped is not the tap/fill bar this line describes.** Part Five settled on the day's total with the quick-tap beside it and *no heading*, deliberately, because it is the one thing on that screen you DO rather than a view you go into. That paragraph is the current design; this sentence describes the superseded one and is kept only because the “same visual family as the calorie and protein bars” reasoning still governs anything added here later.
...
35. Structured workout tracking with progressive-overload — the interactive Almanac Workouts depth (see Part Ten, Workouts): program-type-driven conditional grouping (strength→body area, rehab→flat, skill→by skill, none→plain list), per-user per-goal computed sets × reps, and an exercise-specific "Before you start" safety note card (real failure modes, never boilerplate). **Design settled 2026-08-17 (data-model mechanics); gated on item 15 (the Almanac data model) — a workout plan IS an Almanac entry, so it can't be built until "what an Almanac entry is" exists.** The settled model separates two genuinely different things, which resolves the mockup controls' apparent clash with the Almanac's chat-only editing rule:
   ...
   ~~Not yet built~~ — **BUILT, corrected 2026-09-09.** It was correctly ordered after item 15 and duly followed it. `workout_weight_log` and `workout_completion_log` both exist and both carry real rows; `log-working-weight` and `log-workout-completion` are the routes; `workout-plan-view.tsx` is the interactive plan view whose checkbox writes a completion and keeps the session's single `activity_logs` row in step, rolling eccentric load up to the hardest work in the session so the DOMS flag sees it. The Q1(c) reuse described above is what shipped.
36. Per-exercise movement-demo assets — the licensed-animation demonstrations for Almanac movements (per Part Ten, Exercise demonstration content). **Coverage for the first release is confident movement-*pattern* coverage** (the phased-rollout decision, Part Ten), with the exhaustive every-exercise floor retained as the longer-term target — *not* the flat "every single exercise" bar earlier stated here, and *not* measured against the retired 750-item probe list. **Vendor and licensing are settled** (Exercise Animatic, Ultimate Bundle $229 — Part Ten), so the earlier "locate and reuse the licensing research" note is retired. **Still open, rewritten 2026-09-09 — three of the four are closed.** ~~(a) the two vendor questions blocking the batch recolour~~ — **closed: the batch is done, 770 clips on disk.** ~~(d) the undesigned conversational-substitution mechanism~~ — **closed: item 46's design was resolved 2026-08-21 and its prerequisite metadata was built 2026-08-28.** (b) the RDL preview gating that movement's safety copy — still open, and unchanged. (c) how movements map to asset files — **still the real work, and now the ONLY thing between the plans and the screen.** The manifest supplies a `join_key` per clip and the files carry the `_Female` suffix, so the mapping is defined; what does not exist is anywhere for the app to fetch 770 videos from, or a player to show them in. **The licence decides how that hosting is shaped** — a private bucket and short-lived signed URLs, never a public one and never a download control, and no clip ever sent to a model. Both constraints, with the clause numbers, are in Part Ten under *What the licence actually permits*; the captured documents are in Drive under `Licences & Legals/`.

    **BUILT 2026-09-09. `demoRef` is no longer null.** The private `movement-demos` bucket holds **770 objects, 253 MB**; `public.movement_assets` holds **770 rows**, every one matching an object, with no orphans in either direction. `/api/movement-demo` is the only route to a clip — it authenticates as the user, looks up the row, and mints a **5-minute signed URL** with the service role. The bucket has **no storage policy for end users at all**, so the client cannot address storage even with a valid session. Verified rather than assumed, and re-runnable via `scripts/verify-movement-demo-access.mjs`: signed URL 200 and a real mp4; **unsigned public path 400**; anon key cannot list; expired signature 400. The player sits in the exercise detail beneath the safety note, never above it, and shows **nothing at all** when there is no asset — no placeholder, no explanation. No fullscreen and no picture-in-picture, because both put a licensed clip on a surface outside the app.

    **THE COVERAGE NUMBER, AND IT IS THE UNCOMFORTABLE PART.** Matching a plan's exercise name to a clip is **exact, deliberately**. Measured against the six exercise names in real saved plans on 2026-09-09, **zero matched**, and every near-miss was a different movement: *frog stretch* → nearest is **frog jumps**, a plyometric; *romanian deadlift* → only the landmine and kettlebell single-leg variants; *wide-leg forward fold* → *seated forward bend*; and *butterfly stretch*, *side lunge stretch* and *wall-supported middle split slide* have nothing within reach at all. Fuzzy matching would have shown somebody **a jump when they asked for a stretch**. Part Ten already forbids repointing safety copy at a different implement or stance; a wrong animation is that same error with a bigger picture attached. A missing demo is a small disappointment; a confidently wrong one is a form fault.

    **What that exposes is a library-shape problem, not a build problem.** Only **113 of the 770 clips** are `Stretching - Mobility` or `Yoga`; the rest is gym strength work. The plans Selodía has actually been asked for so far are mobility and flexibility programmes, which is the thin end of the library. So the demonstration slot will be **empty for most of what the app currently prescribes**, and that is a fact about coverage rather than a defect in item 36. It is the phased-rollout decision above meeting real data: movement-*pattern* coverage is genuine for strength, and thin for mobility.

    Three consequences worth naming. **Item 46 matters more, not less** — it is now the primary experience for a mobility plan rather than an edge case. **The vendor conversation has a shape**: the gap is specific, nameable and evidenced, which is a far better basis for asking about commissioned work than “we would like more clips”. And **exact matching should not be softened to make the number look better** — if coverage is to improve it improves by adding assets or by mapping synonyms deliberately, never by loosening the match until something plausible appears.

    **The synonym pass, done 2026-09-09, and it bought less than hoped — which is the finding.** Matching now happens in one place, `public.resolve_movement_refs`, so the save-time resolver and the serving route cannot drift into disagreeing about what a name means. It matches on an exact name **or an exact word set**, and on nothing weaker. Word-set matching is not fuzzy matching: the vendor writes `glute bridge bodyweight` and a model writes *bodyweight glute bridge* — the same words, reordered, with no word added, dropped or approximated. It was **verified collision-free across all 770 clips** before it was built, and the `match_key` column carries a unique index so a future clip that would collide fails the insert loudly instead of making one name mean two movements. It also absorbs punctuation, doubled spaces and plurals.

    **A subset rule was measured and rejected, and this is the part worth keeping.** Matching when the plan's words merely *appear* in a clip's name scores far better on a coverage count. On real plan names it also matched **“kettlebell romanian deadlifts” to the single-leg variant** and **“dead bug” to a resistance-band one** — each the only candidate, so even a uniqueness guard accepts both. **Uniqueness is not correctness:** it means only that the library happens to hold one clip containing those words, and that clip may be a different movement under a different load. A third of the matches that rule added were wrong, and each wrong one is a form fault. `glute bridge` has **24 candidates** and now resolves to nothing, on purpose.
    ...
    **What a saved plan looked like on the morning of 2026-09-09**, from a real Almanac row written by voice: every exercise carried `"demoRef": null`. The model was doing the right thing — there was nothing to reference, and that null was the whole of what was missing. **By that evening it was not**: the bucket, the table, the serving route and the save-time resolver all shipped the same day, so a plan written now carries a real `join_key` wherever the library covers the movement. Kept as written because it is the clearest one-line statement of what item 36 actually was. Batches with the other native-additions rebuild. Not yet built.
...
37. **The visual-design pass (Part Fifteen)** — added 2026-08-21, because Part Fifteen is fully decided and had **no build item at all**. Covers: applying the locked brand palette (cream / sand / terracotta / sage / forest / charcoal) in place of the current Expo-scaffold black-and-white theme; the wordmark, logo mark and app-icon assets from the Drive asset pack; the breathing-pulse loading state; and the on-brand error states. **One part is not cosmetic:** Part Fifteen specifies that light/dark mode is a **deliberate, persistent user choice that never follows the system setting**, and calls that a safety constraint rather than a preference. The app currently re-exports React Native's system `useColorScheme` and `app.json` sets `userInterfaceStyle: "automatic"` — doing exactly what the spec forbids. That piece should not wait for the rest of the cosmetic pass.
    ...
    **Intent recorded 2026-08-21 — the discuss-card “has discussion” state must be genuinely eye-catching, not a subtle shift.** Two reasons, and the second is about the product as a whole: a quiet indicator risks being effectively invisible in real use, and the app otherwise risks reading as *too* visually restrained overall. Real options to explore at that point: a bold, saturated brand-colour treatment rather than a muted one; a small badge or dot alongside the icon; or a brief one-time animation on the state change. **The exact treatment is deliberately not decided here** — this is captured so item 37 does not quietly default to something too understated. The current placeholder (a `theme.textSecondary` → `theme.text` token swap, marked in `food-today-view.tsx`) is explicitly a stand-in, not a candidate.
38. **The Measurements segment** — the Body tab's second detail segment: key measurements (waist / thigh / hip), the minimized weekly table with its 7d deltas and week-stepping, Then & Now, and the measurement-detail view. **Also the home the interpretation layer was waiting on** — step 19's client half sat built-but-unsurfaced from 14 to 24 August, and reaches a screen here (slice 2, below). The `food-week-view.tsx` weekly-table mechanic ports here (Part Five, Food Segment build note) — **it is parked with no host as of 2026-08-21**, since the Food segment now renders its own today's-log view (item 40). Recorded rather than left silent, which is the whole lesson of the interpretation-layer drift. The `?week=` deep-link param and `week.ts`'s parsing helpers are parked with it: nothing reads the param today, and it comes back when this segment does.

    **Slices 1-2 built (2026-08-24).** The segment exists and is reachable as the Body tab's third switcher segment; `?view=measurements&week=YYYY-MM-DD` now resolves, so the deep-link param is un-parked. *Slice 1 - the weekly table*: `measurements-week.ts` + `measurements-view.tsx`, one row per local day with Day / Weight / Body fat / Muscle and a trailing 7d **percentage** delta, plus week-stepping and a back-to-this-week control that appears only once you have moved away. Written fresh rather than moved: `food-week-view.tsx` stays where it is until item 40 replaces it, so nothing breaks in passing. The Overview keeps its **absolute** "↘ -0.4 vs last wk" - a kilogram is what you feel at a glance, while three metrics on different scales can only be read down a column as percentages. A delta is only labelled "7d" when the reference reading genuinely sits 4-11 days back; with sparse data the nearest earlier reading can be months old, and `findWeekAgoReading` (correctly, for the Overview's softer claim) returns it anyway, so this segment adds the tolerance rather than changing that. Deliberately **no seeded data**: seeded body numbers would feed the interpretation layer, which would then draw real-looking conclusions from invented readings - a different and worse kind of fiction than a seeded Almanac row. Sparse and empty are the primary path. *Slice 2 - the interpretation layer reaches a screen*: `measurement-context.ts` gathers the five inputs `interpretLatestReading()` has needed since 14 August (readings, cycle, activity 72h back for pump + DOMS, prior timestamps, food 24h back for sodium) and `reading-interpretation.tsx` renders the composed message above the table. It is anchored to the **latest** reading, not the displayed week, so it stays put while you step back through history. Silence is a real output: it renders nothing for a clean drop and nothing for a sparse history with no noise flag, both correct.

    **Verification (Development Workflow Principle 4):** rung 2 for the composition - 41 node tests across both libs, including the honest-labelling cases (a months-old reference is not called "7d"; two readings claim no direction). Rung 1 for the rendering: tsc + lint clean, never seen on a device. **The wording itself is at rung 0 and cannot be lifted by a test** - node can confirm a message exists and names the right cause, not whether it sounds like someone on your side. Two things to read once real data exists: three simultaneous flags compose into one long sentence (period + pump + DOMS), and the trend-up branch reads "...water retention is common, which can hold a little water", which doubles back on itself. Both are pre-existing composition in `measurement-interpretation.ts`, left alone rather than tuned blind.

    **Item 38 is complete as of 2026-08-31, bar one deliberately-blocked piece.** Slice 3, Then & Now, shipped (`6e6d3f5`). Slice 4, "key measurements (waist / thigh / hip)", is **superseded rather than outstanding**: Part Five replaced those cards with the stacked "Everything else" table, and that is built — `personal-metrics-view.tsx` renders Metric / Latest / Change, ordered chronologically by first-logged. The **per-row eye icon remains deliberately absent**, since it needs the discuss-card capture path (item 30 slice 4) and an icon that opens nothing is a dead control. *(The previous version of this line listed slices 3 and 4 as open. Slice 3 has since been built and slice 4 was superseded before it was ever built — an item can be finished by its requirement changing, not only by code.)* **Persisted Interpretation Notes remain unbuilt**: the note shown here is computed on read, which is right for a live statement about the current reading but is *not* the point-in-time per-entry note Part Nine describes. That one needs a column on `body_measurements`, a write at log time, and the discuss-card to display it - and, because `measurement-interpretation.ts` lives in `mobile/src/lib/`, either a duplicate in `app/lib/` (the existing `cycle.ts` precedent, which has already drifted between the two copies) or a genuine shared module. That fork is unresolved and should be decided before the note is persisted, not during.
...
41. **Data export, account settings, and deletion** — added 2026-08-21; specified in Part Five and Part Seventeen with no build item until now. A UK GDPR export with its two discoverable entry points (asking Unflump directly, and a quiet link from the history/week view), plus the standing deletion mechanism that must exist independently of research consent. Needs an account-settings surface, which nothing currently builds. **COMPLETE as of 2026-09-01. It shipped partial on 31 August and was finished the next day; both halves of that are worth keeping.** Built 31 August in three pieces: the Settings surface reached from a quiet link at the top of Chat (not a fourth tab — Part Five keeps the app to three destinations) with the sign-out that had until then existed nowhere in the codebase at all; the export, with both entry points Part Five requires, offering a readable summary alongside the full JSON; and deletion across 17 tables plus storage objects, **verified by re-counting rather than by the delete call not erroring**, with `deletionCoverageGap()` guarding against the export list and the delete list drifting apart. That last day it stopped there: `auth.admin.deleteUser` needs a service-role key the project did not have, so the login credential survived, and **the copy said so in those words rather than calling it account deletion** — a half-deletion described as a deletion is worse than no deletion, because the person believes they are gone.

**Finished 1 September** once `SUPABASE_SERVICE_ROLE_KEY` reached the deployed environment: `app/api/delete-account/route.ts` removes the auth user with a service-role client, which cascades all 17 tables by itself. Three things about it are load-bearing. **The user id comes from the verified token, never from the request body** — a body-supplied id would make it an endpoint that deletes any account by number. **The client-side wipe still runs first**, redundant against the cascade but not against the requirement, because a cascade succeeds or fails as one opaque statement and cannot report which table it could not clear. **The shell goes last**, after the verification counts, because the moment the auth user disappears the session token refers to nobody and those counts would start reporting empty tables as unclearable.

**The honesty was re-aimed, not retired.** Completeness is claimed from `authAccountRemains`, set by what the server actually reported — so a deployment whose key is missing, or whose admin call fails, still tells the person their sign-in survived, and now asks them to say so. **`SUPABASE_SERVICE_ROLE_KEY` is not in local `.env.local`** (verified absent 1 September); it exists in the deployed environment only, so this path works against the deployed API and would report `not_configured` if run against a local server. That is by design rather than an oversight: the key bypasses RLS entirely, and the fewer places it lives the better.
42. **Allergies and dietary restrictions as a hard structured constraint** (Part Twelve) — added 2026-08-21, **design resolved the same day**. Flagged in Part Four as "separately scoped, out of this pass" but never actually scoped. **This is a safety item, not a preference**, and should not sit behind cosmetic work. Scope: (a) **conversational capture** — recognised whenever an allergy naturally comes up, in onboarding or years later, never a dedicated screen or form; (b) **its own storage**, deliberately separate from softer preferences and dislikes, which stay in ordinary conversational memory; (c) **a real filter gate on every food suggestion path** — an executed check, not a prompt instruction, since a long session can truncate context and a model can be talked around; (d) permanence — once disclosed, never suggested again, with no softening over time. Distinct in mechanism from Health Context, which tilts guidance by prompt injection rather than hard-excluding. **Status, 2026-09-09: COMPLETE.** Parts (a), (b) and (d) were built 2026-08-31; **part (c), the filter gate, was built 2026-09-09** in `app/lib/allergy-gate.ts`, with all four layers this item scoped. Layer 1 (prevention) is the prompt block in `allergies.ts`, unchanged and still labelled awareness rather than guarantee. Layer 2 is `suggestsFood` on the classify tool the model already calls — structured output rather than prose it can drift out of, at no extra round trip. Layer 3 matches the stored allergen names against the reply with word boundaries, so “nut” does not fire on “nutrition” nor “egg” on “eggplant”; it runs **regardless of what layer 2 reported**, because the self-report is the model's own claim. Layer 4 is a gated Haiku call catching an allergen hidden inside a composite dish — the failure the other three cannot see, since “pad thai” names no peanut for anything to match. It fires **only** when the person has exclusions and the reply suggests food, so it never runs for logging, body questions or ordinary chat, and the whole thing short-circuits to zero cost for anybody with no allergies.

    **The blocked reply is never stored.** The gate runs after the trailing correction and honesty notes are decided but before anything is written to `chat_messages` — otherwise the next turn would read the blocked suggestion back as context and the model would believe it had said the thing it was stopped from saying. The notes themselves survive a block: they are statements about what the app did with the person's data, still true and still owed.

    **Layer 4 fails open, deliberately, and only layer 4.** If the check throws, the reply goes out and the failure is logged loudly. Blocking every food suggestion whenever Anthropic has a wobble would break the app in a way the person cannot diagnose, and layers 1–3 have already run. Layer 3 cannot fail open, because a string comparison cannot fail.

    **THE DEPENDENCY ON ITEM 22 WAS BACKWARDS, and that is the finding worth keeping.** This item and `allergies.ts` both said the gate was blocked on the Meal/Order Advisor, “where the first real food-suggestion path will exist”, and that the app *has no food-suggestion path at all*. That was false in practice, and the proof was sitting in the allergy prompt block itself: it says never suggest an allergen **“in anything you propose”** — an instruction that only exists because proposing happens. There is no food-suggestion *feature*, but Selodía will answer “what should I have for dinner?” today, and did so throughout the period this gate was deferred. **A safety item was waiting on a feature while the risk it guards against was already live.** The general lesson: a gate belongs with the risk, not with the feature that makes the risk legible.

    **Verified rather than assumed**, by `scripts/probe-allergy-gate.mjs` — 15 checks, all passing: literal matches with singular/plural and punctuation variants; the false positives that would get a safety feature switched off within a week (*nutrition*, *eggplant*); pad thai, carbonara and pesto each blocked against peanut, egg and pine nut; a safe suggestion and ordinary chat left alone; and **“you had pad thai yesterday” correctly not flagged**, because reporting what somebody ate is not suggesting it. There is still no test suite (Part Four), so this is a runnable probe rather than a test.

    **Technical feasibility — checked against the real pipeline, 2026-08-21.** `ask-unflump` is **one model call per message**, and that call dominates the latency; every database read is milliseconds beside it. So the only thing that could meaningfully slow messages is a *second* model call. **Three layers cost one concurrent read and a handful of tokens — no extra round trip:** (1) **prevention** — the exclusion list in the system prompt, which handles the overwhelming majority by stopping the suggestion being made; (2) **structured self-report** — a field on the classify tool the model already calls, genuinely stronger than a prompt instruction because it is structured output rather than prose it can drift away from; (3) **a deterministic backstop** — exact-matching the stored allergen names and synonyms against the reply. The whole thing **short-circuits when the person has no exclusions**, which is most people most of the time, so the default cost is zero.

    **The honest limit:** those three are *not* a hard guarantee for an allergen hidden inside a composite dish. A suggestion of "pad thai" with no declared peanut defeats all three — layer 2 missed it, and layer 3 cannot match a string that is not there. Layer 3 alone is also a closed keyword list, exactly what principle 13 rules out. **Only a second model call closes that gap, and it must be gated** — fire it only when the person has exclusions *and* the reply actually suggests food, so it never runs for logging, body questions, or ordinary chat. That confines the latency cost to the small slice of messages that could carry the risk, rather than doubling it everywhere. Scope item 42 as layers 1-3 **plus** that gated fourth check; describing layers 1-3 alone as a "filter gate" would overstate what they guarantee.
43. **Cross-cutting safety principle — unsafe-goal handling** (Part Twelve) — added 2026-08-21, previously unrepresented in the build order. Detect when a stated or implied goal calculates into an unsafe range, flag it kindly at the point the target is set, stop precise numeric coaching built on that goal while keeping logging fully available, and offer a resource once without repeating it unprompted.
    ...
    **BUILT 2026-09-09.** `app/lib/goal-safety.ts` plus a `unsafe_goal_flagged_at` stamp on `user_profile`. **The model reports the number; the app judges it** — a new `statedGoalWeightKg` field on the classify tool asks only what weight was said (converting stones if that is how they said it) and forbids the model from assessing it. Asking a model “is this goal unsafe?” is a prompt instruction a long session can truncate and a conversation can argue with; asking “what number did they say?” is extraction, and the arithmetic afterwards is arithmetic. Same split as the allergy gate, for the same reason.

    **The line is BMI 18.5 against their stored height, with a separate severe band under 16** that changes tone rather than mechanism. BMI is a poor instrument for an individual and that is acceptable here precisely because it is used to decide when to STOP COACHING, never to diagnose: it errs toward caution, and the cost of erring is that Selodía declines to produce one number. **No height means `'unknown'`, never `'safe'`** — height is optional in this app, and a fabricated assessment would either nag somebody whose goal is fine or, far worse, clear one that is not.

    **The reply is regenerated on that turn, and this is the awkward, unavoidable part.** The model writes its answer before anything knows the goal is unsafe, so that answer may already be coaching toward the number — the exact harm. A canned replacement would be safe and cold; the spec asks for kind, and kind has to be written in context. So the turn re-runs with the instruction included and **only the reply text is taken from it**; everything the first call decided about logging, corrections and saves is untouched, because the goal changes none of it. That costs a full turn's latency on a rare turn, which is the right place to spend it. **It fails CLOSED**, unlike the allergy gate's fourth layer: if the rewrite cannot happen the original cannot go out, because the thing that could not be checked is whether it coaches toward a dangerous number.

    **Two prompt blocks, and the standing one is what makes this durable.** The turn block handles the moment. The standing block goes into every later turn once the stamp exists, because the exchange scrolls out of the history window within a few turns and without it the app drifts back into coaching two days later, having forgotten it said it would not. Neither block contains a BMI figure, a threshold or a category word — handing somebody a number and a label about their own goal is the verdict-delivering register principle 2 rules out. **Logging is explicitly protected in both**: what stops is coaching toward the number, never the ability to record anything.

    **The resource goes out once, ever** (Beat), gated on the stamp being null. A live distress card always wins the slot — two cards at once reads as the app panicking.

    **What does not exist, recorded so it is not assumed:** `fat_focus_target_value` is in the schema but nothing reads or writes it, so “the point the target is set” has no code path. Like item 42, the live risk was conversational all along, and the spec already covered it: this applies “anywhere a goal or logged pattern looks like it is drifting unsafe, not only at initial goal-setting”. A future target-setting screen calls the same assessment. **Not yet covered:** a logged *pattern* drifting unsafe, as opposed to a stated goal — that needs trend data and is a separate pass.

    **Verified** by `scripts/probe-goal-safety.mjs` — 29 checks, all passing, all pure arithmetic and free to run. Two of them failed first time and **the probe was wrong, not the code**: 45kg at 165cm is BMI 16.5, under the floor but above the severe line, so `severe: false` was correct.
...
46. **Conversational substitution for movements with no asset** (Part Ten) — added 2026-08-21, **design resolved the same day**; gates item 36. Unflump explains the muscle group and movement pattern a requested exercise targets, then either offers genuine functional alternatives or asks what outcome the person is after and suggests from there — **never referencing the library's limits in any wording**. **The prerequisite metadata EXISTS (corrected 2026-09-09).** This item said it "does not exist in usable form today" for nineteen days after it was built. `movement-classification.json` in `Build Specs/Excersise Animatics/` carries all **770** female clips with `primary_muscles`, `secondary_muscles`, `equipment`, `movement_pattern` and a `join_key`, plus per-row provenance — 313 vendor_female, 60 vendor_unsuffixed, 4 fuzzy variants and 393 ai_classified, of which **377 are human-verified**.

    **Option (a) is dead, and that is settled rather than pending: the vendor has confirmed they hold no muscle metadata or tagging data at all.** So it was never coming, and the route actually taken was option (b) — classify once with AI at ingest, the principle-13-correct approach — which is why the data is here. The three options are recorded below as the reasoning that led to it, not as a live decision:

    ~~(a) **ask the vendor for their official muscle-tagging data** — tracked as question 5 in `VENDOR_QUESTIONS.md`, and much the cheapest if it exists;~~ **— confirmed unavailable, 2026-09-09.** (b) **classify once with AI at asset-ingest time** and store per movement — the principle-13-correct approach, the same log-time-classification pattern already used for `eccentric_load`, `intensity`, and `protein_source`; (c) keyword-match filenames — **rejected**, since the coverage workbook already demonstrates its limits and it re-imports precisely the closed-list problem principle 13 forbids. Note the store is **internal only**: it never becomes a user-facing picker or search, which stays permanently out of scope.
47. **Extraction uncertainty — held, not discarded (investigated 2026-08-23, not implemented).** The idea: let the extraction schema say *"I cannot tell which of two things this means"* (`interpretationUncertain` plus a note naming the fork), and have the app ask about that specific ambiguity rather than guessing. It was **built, tested clean, and then deliberately removed before shipping** — recorded here so the investigation is not lost and is not silently retried on the same assumption.

    **Why it was pulled.** It was designed against a real failure: on 2026-08-23, asked what a typical week looked like, "Just yoga and a run" was read as a log of *today* and had to be corrected. But **twenty replay trials against the real reconstructed 40-turn history never reproduced it** — the baseline handled it 5/5, one reply even correctly referencing her answer from 13 August. Meanwhile the mechanism itself measurably *worsened* the complaint it sat next to: with the phase opener alone the model set an activity level **5/5**; with uncertainty enabled, **0/5 across ten trials**, spending the turn re-asking instead. Ruth's central complaint was being re-asked things she had already said clearly, so shipping it would have deepened the real problem to fix an undemonstrable one.

    **What actually fixed the case** was structural and provable by reading code, not sampling: the phase opener was never sent to the model at all, so it was answering a question it could not see.

    **What would justify revisiting.** A genuinely reproducible ambiguity case — one that fails repeatedly with the openers already in place. The most likely untested cause of the original failure is **tool-schema load**: the live call fills roughly fifteen extraction fields *and* composes the reply in one pass, where the replay filled three. If that turns out to be the mechanism, the answer is separating extraction from reply generation, **not** this flag. Test that hypothesis before rebuilding this.

48. **Equipment gentle re-check and deferred-topic resurfacing** (Part Seven, Part Eight) — added 2026-08-21. The schema landed in Phase 1 step 1; the behaviours never got items. A later, gentle equipment re-check once someone has plausibly had time to acquire scales, and the in-session resurfacing of any topic deferred with a "let's brainstorm that later."

49. **Mobility, yoga and physio illustration library** (Part Ten) — added 2026-09-09, **needed before public launch, not after beta.** Item 36 solved demonstrations for gym strength movements and did it with licensed video; **this is the other half of the problem**, and it exists because item 36's coverage was measured rather than assumed. Only **113 of the 770 licensed clips** are `Stretching - Mobility` or `Yoga`, and the plans Selodía has actually been asked for are mobility and flexibility programmes — so the demonstration slot is empty for most of what the app currently prescribes. The evidence is in Part Ten under item 36: of six exercise names in real saved plans, **zero** had a matching clip.

    **Decision (2026-09-09): a custom minimalist character, posed per movement, delivered as static SVGs.** Three things follow from that shape and each is deliberate. **Static, not animated** — a held stretch has no movement to animate; its content is the shape of the pose, which a still renders better than a loop of nothing happening. That is a genuine difference from strength work, where the eccentric phase is the thing worth watching, and not a compromise. **A single character across the whole set**, so a mobility sequence reads as one body doing several things rather than a collage of stock figures. **SVG**, so the illustrations carry the brand palette natively at any size and need no bucket, no signed URLs and no licence architecture — they ship inside the app.

    **Approach is open: AI generation from a character sheet, or a commissioned library.** Both are live and neither is chosen. What is worth writing down now, because it will otherwise be re-argued: **Part Ten's rejection of AI-generated exercise video does not automatically settle this.** That rejection turned on movement — current generation produces subtly wrong *motion*, and form is safety-relevant. A static pose drawn from a fixed character sheet and **reviewed by a human against the real posture** is a different risk with a different mitigation. It may still fail that review; the point is that it has to be judged on its own terms rather than by inheriting an argument about video.

    **Not blocked, and not gated on the vendor.** Exercise Animatic confirmed they do not offer commissioned or custom production, so this was never something they could supply. It does, though, give the eventual vendor conversation something specific to be about.

    **Alias curation for the existing library is folded into this item (deferred here 2026-09-09).** Item 36's matching is exact and stays exact, so a movement the library holds under different words — “dead bug” for `resistance band upper body dead bug` — finds nothing. Closing that needs a person deciding, one movement at a time, which clip a name really means; automatic rules were measured and produce form faults (Part Ten, item 36). The work was scoped and then **deliberately not started**, because this item may replace most of the mobility library outright, and aliases curated against clips that get superseded are work thrown away. Decide the illustration approach first, then curate once against whatever the library actually ends up being.
...
51. **Record consent: the three tick boxes are collected and never stored** (Part Seven, Part Sixteen). Added 2026-09-12, **needed before beta. BUILT 2026-09-19 (commit f7d01ae); see "As built" below.** The consent screen (`mobile/src/app/onboarding/consent.tsx`) asks for core consent to process health data, which is required to continue, plus an email-updates opt-in and a research opt-in for de-identified data. **None of the three has ever been saved, for anyone.** Continue only navigates to the account screen. There is no consent table and no consent column except `voice_consent_at`, and nothing in the codebase carries the answers forward. Found on 12 September when a sign-out and Google sign-in showed the screen again.

    **Why it matters more than the re-asking.** The live privacy policy names explicit consent as the basis for holding health and cycle data, which is special category data under UK GDPR, and Article 7(1) requires the controller to be able to demonstrate that consent was given. Today Selodía cannot, for any account. No real users exist yet (both accounts are Ruth's), so nothing has been breached, but no tester should sign up until this is fixed. The opt-ins fail safe in the meantime: an answer that was never stored means nobody is marked as agreeing to marketing or research, so neither can happen by accident. Ruth's view, recorded because it is right: being shown the screen again after signing out is minor, since people rarely sign out of an app.

    **The work.** Store all three answers with a timestamp and the privacy-policy version they were given against, written as soon as a session exists after sign-up, at the same point `upsertProfile` writes date of birth. Ask existing accounts once, on next launch, rather than assuming consent. Offer returning users a direct sign-in path from the first screen, and after sign-in ask only if nothing is recorded. In the same flow, `upsertProfile` in `account.tsx` writes `null` over an existing value when only one of date of birth and sex was filled before a Google sign-in; it should write only the fields that were given.

    **As built (2026-09-19).** A new append-only `consent_records` table holds each answer set with `policy_version` (the privacy policy's "Last updated" date), `source` (`onboarding` or `reconfirm`), `given_at` and `recorded_at`. Owners can read, insert and delete their own rows, never edit them; a change of mind is a new row. `mobile/src/lib/consent.ts` carries the answers two ways, in memory and in the sign-up metadata, because on the email-confirmation path no session exists until the link is clicked. The auth guard first writes any carried answers, then checks for a record; an account with none is sent once to the consent screen in reconfirm mode ("One thing before you carry on"), which writes at once and returns to the app. Nothing is assumed. The first screen has an "Already have an account? Sign in" link, and `upsertProfile` now writes only the fields given. Ruth's own accounts will each be asked once on next launch.

    **Found while building it: the export and erasure lists had fallen behind the schema.** Five tables holding personal data had never joined `EXPORT_TABLES` or the deletion order: `custom_reminders`, `daily_activity_summaries` (missing from exports since 4 September), `food_cache`, `consent_records` and `client_error_log`. All five are now in both. `consent_records` and `client_error_log` gained owner delete policies, without which account deletion's leftover count would have falsely reported them as "could not be removed". Expired `me_exports` copies are now cleared at each new export by `purge_expired_me_exports()`. Whenever a table holding user data is added, it goes into `data-export.ts` and `account-deletion.ts` in the same change.
...
**Sequencing decision, 2026-09-12: the Almanac before voice.** Ruth: voice exists to populate the Almanac properly, so the Almanac is built first and voice follows it. Recorded with its clock rather than argued: demo day is 21 October and the app demonstration needs one reliable voice path, so the Almanac work runs on that deadline too.
...
- **Ruth is the primary tester, and this is a deliberate choice rather than a stopgap.** She is squarely in the target demographic, and founder-led use has already proved it surfaces real, actionable problems: the 2026-08-23 on-device session produced four substantive onboarding findings (no progress indicator, no bounded-phase framing, possibly over-careful conversational pacing, and no visible sense of a profile being built) that no amount of spec review had caught. That is the standard the testing structure exists to reproduce, not a placeholder for "proper" testers.
- **Carol is the one external tester**, kept on demonstrated engagement and responsiveness rather than availability.
- **Matty and Nikki are dropped.** Matty is not in the target demographic at all, and Nikki has self-identified low testing discipline. Neither was likely to test rigorously, and a tester who does not test rigorously produces false confidence rather than no information — which is worse. Recorded so the earlier plan is not revived out of momentum.
- **The gate to public beta is Ruth's own quality bar**, not a date and not a tester count. Once it is met, the next wave is a **real public beta aimed at strangers already active in fitness-tracking and self-improvement spaces** — people with existing habits and existing comparisons, who will judge it against what they already use.

**Native-module milestone.** Not feature-based, distribution-based: not ready until every remaining native-module addition is complete — basic chat-logging's `expo-image-picker`, the Almanac's licensed animation library, notifications' `expo-notifications`, and **voice input's microphone + speech-to-text (item 34, added to this batch 2026-08-16)**. EAS Update can only push ordinary JS/TSX changes over the air, not new native libraries, so sharing before all four land would mean a tester needing a manual reinstall each time one does. Once all four are done, everything remaining ships via EAS Update with zero reinstalls required. **Note (2026-08-21):** the feature threshold in the paragraph above (auth, listener, real onboarding, itemized breakdown through step 11) **has now been met** — the native batch is the only remaining gate, alongside clearing the unverified-native-module debt recorded in Part Four. Not a reason to reprioritize the build order — a checkpoint to notice once naturally reached, not chased.
...
A real deletion mechanism must exist for anyone who wants their data removed, independent of whether they opted into research use — this is a standing right, not conditional on that checkbox. **Built and complete as of 2026-09-01 (item 41), including the auth credential itself** — nothing is retained after a deletion, and the app no longer has to qualify the word.
...
- **Consumer bioimpedance scale accuracy:** the ±3-5% figure previously recorded here was removed on 2026-09-03. No peer-reviewed source establishes it as a validated individual margin, and the evidence points the other way — see [R3].
...
- **Habit formation timeline [RS1]:** a **2010** UCL study (Lally et al.; online 2009, which is where this document’s earlier "2009" came from) found habits formed in a range of 18-254 days, averaging roughly 66 days overall; "eating healthier" specifically averaged roughly 59 days. Used to set the 9-week habit-formation window used throughout this document. **Those three figures are unverified from an open-access source** — the paper has no abstract record in Europe PMC — so this carries the same status as the DOMS window: the paper is cited, the numbers are flagged for expert review.
...
- **Weight maintenance duration and regain risk:** National Weight Control Registry data found **a strong predictor** of avoiding weight regain is how long weight loss has already been maintained [RS7] — individuals who had sustained loss for 2-5 years had markedly higher odds of continuing to maintain it. *(Narrowed from "the single best predictor" on 2026-09-03: the source lists duration alongside physical activity, eating breakfast, self-monitoring and low depression, and does not rank it first.)* Used to calibrate the Graduation Moment's celebration timing and language honestly.
- **Protein complementation and meal timing (two separate claims, only one settled):** *Well supported* — complementary plant proteins do **not** need to be eaten in the same meal to give complete essential-amino-acid coverage; this is the Academy of Nutrition and Dietetics' own position statement (2016) and broad consensus, refuting the 1970s "protein combining" idea (overview: https://en.wikipedia.org/wiki/Protein_combining). *Genuinely unsettled* — whether same-day-but-different-meal complementation is equally effective **specifically for muscle protein synthesis**: the metabolic advantage of same-meal complementation "has been questioned… requires further investigation" (https://www.sciencedirect.com/science/article/pii/S0022316624010770), and a clinical trial is currently running precisely because "this has never been tested" (https://clinicaltrials.gov/study/NCT07151625). This split is why the meal-level assessment claims day-level adequacy only and says nothing about timing for training. Sourced by Ruth, 2026-08-27.
- **Dichotomous "good/bad" food thinking and disordered eating:** rigid, all-or-none food thinking is associated with higher rates of binge eating, body dissatisfaction, cognitive restraint and drive for thinness [RS3]. *("Across multiple studies" was removed on 2026-09-03: the citation is a single study of 630 women, and the paper’s own argument is that this area is **understudied**. Claiming a literature the source says does not yet exist would be contradicted by the source itself.)* Intuitive eating consistently predicts lower disordered eating. Grounds Part Two principle 12 and the no-moralizing-food-language voice rule.
- **Eating with others and mental health:** a large Korean population study (n=14,125) found eating with others significantly associated with lower depression and lower suicidal ideation, independent of nutritional content [RS2]. Honest nuance: eating socially is not simply "healthier" across the board — some studies show increased intake in social settings too — so this grounds connection as its own good, not a claim that shared meals are nutritionally superior.
- **Positive emotion, health, and longevity:** a real, credible correlational link, grounded in the Nun Study [RS4] (positive emotional language in early-life writing prospectively predicting longer lifespan), Diener & Chan's peer-reviewed review [RS5], and Cohen & Pressman's review in *Psychological Bulletin*. Honest limitation carried from the sources: the effect is strongest for trait/habitual positive emotion rather than isolated moments, and does not generalize identically across every population.
...
- **Positive emotion actively speeds stress recovery (experimental, not just correlational):** Fredrickson's "undoing hypothesis" [RS6], tested across multiple studies (n≈170, n≈185), showed faster cardiovascular recovery after stress from contentment/amusement versus neutral content, with the "simple replacement" alternative explanation specifically ruled out. Grounds treating joy and connection as genuine markers of a successful day (Part Two, principle 12), not consolation.
- **Exercise enjoyment and long-term adherence:** enjoyment is a well-documented predictor of sustained exercise adherence. **The specific study previously cited (n=645 older adults) could not be located** across three search strategies, and the superlative claim — strongest predictor ahead of health-, fitness-, or appearance-based reasons — is unsupported and removed (2026-09-03). A superlative across motivation types needs a review ranking them, not a single cohort. Honest limitation: no single study directly compares life expectancy between "joy exercisers" and "chore exercisers" — the longevity link is a reasonable two-step inference (joy motivation → sustained adherence → the separately well-established longevity benefit of sustained activity), not one unified finding. Grounds the Almanac's movement-plan design (Part Ten) and the onboarding activity step (Part Seven, step 11).

## Resources citations

*Vancouver entries for the bullets above, prefixed `[RS]` for Resources Section. Verified against Crossref and Europe PMC on 2026-09-03. Kept separate from the `[R]` series so a bare `[1]` cannot mean three different things inside one document.*

[RS1] Lally P, van Jaarsveld CHM, Potts HWW, Wardle J. How are habits formed: Modelling habit formation in the real world. Eur J Soc Psychol. 2010;40(6):998-1009. doi: 10.1002/ejsp.674
The habit-formation study behind the 9-week window. **Year corrected to 2010** (the issue year; it appeared online in 2009, which is where the earlier date came from). **The 18-254 day range, the ~66 day mean and the ~59 days for "eating healthier" are NOT verified from an open-access source** - there is no abstract record in Europe PMC and the article is closed. Cited for the study; the figures are flagged.

[RS2] Son YH, Oh SS, Jang SI, Park EC, Park SH. Association between commensality with depression and suicidal ideation of Korean adults: the sixth and seventh Korean National Health and Nutrition Examination Survey. Nutr J. 2020;19:131. doi: 10.1186/s12937-020-00650-9
KNHANES 2013/2015/2017, **14,125 Korean adults (5,854 men, 8,271 women)**. Confirms commensality significantly associated with both depression and suicidal ideation. Closes the largest gap the citation audit found: a suicidality claim that previously carried no source at all.

[RS3] Levinson JA, Parker JE, Hunger JM, Tomiyama AJ. Dichotomous thinking about food as an understudied subclinical disordered eating cognition. J Eat Disord. 2025. doi: 10.1186/s40337-025-01388-7
630 women aged 18-44. "Good/bad" food sorting significantly associated with binge eating, body dissatisfaction, cognitive restraint, restriction, excessive exercise, purging and drive for thinness. **One study, not a literature** - the paper's own case is that the area is understudied, which is why the bullet no longer claims "multiple studies".

[RS4] Danner DD, Snowdon DA, Friesen WV. Positive emotions in early life and longevity: Findings from the nun study. J Pers Soc Psychol. 2001;80(5):804-813. doi: 10.1037/0022-3514.80.5.804
The Nun Study. Supports positive emotional language in early-life writing prospectively predicting longer lifespan.

[RS5] Diener E, Chan MY. Happy People Live Longer: Subjective Well-Being Contributes to Health and Longevity. Appl Psychol Health Well-Being. 2011;3(1):1-43. doi: 10.1111/j.1758-0854.2010.01045.x
The peer-reviewed review behind the correlational positive-emotion and longevity link.

[RS6] Fredrickson BL, Mancuso RA, Branigan C, Tugade MM. The Undoing Effect of Positive Emotions. Motiv Emot. 2000;24(4):237-258. doi: 10.1023/A:1010796329158
The undoing hypothesis: faster cardiovascular recovery after stress following positive emotion. Experimental rather than correlational, which is why this bullet stands separately from [RS4] and [RS5].

[RS7] Wing RR, Phelan S. Long-term weight loss maintenance. Am J Clin Nutr. 2005;82(1 Suppl):222S-225S. doi: 10.1093/ajcn/82.1.222S
National Weight Control Registry. States that after 2-5 years of maintained loss the chance of longer-term success greatly increases. **Does not rank duration as the single best predictor** - it lists it alongside physical activity, eating breakfast, self-monitoring, and low depression and disinhibition - which is why the bullet was narrowed to "a strong predictor".

---

# REFERENCES

*Peer-reviewed sources for factual and clinical claims made in this document, verified against Crossref at the date given. **Numbered `[R1]`, `[R2]`… deliberately**, because Part Eight's Health Flower section carries its own self-contained list numbered `[1]`-`[11]` scoped to that section. Two lists sharing one numbering would make `[4]` ambiguous.*

[R1] Wang Z, Ying Z, Bosy-Westphal A, et al. Specific metabolic rates of major organs and tissues across adulthood: evaluation by mechanistic model of resting energy expenditure. Am J Clin Nutr. 2010;92(6):1369-1377. doi: 10.3945/ajcn.2010.29885
Measures the resting metabolic rate of existing muscle tissue at 13 kcal/kg/day; age-adjusted values are slightly lower in the >50y group, which supports quoting the range rather than a flat figure. **The on-screen extrapolation "for every kilogram of muscle gained" is an inference from tissue-level rate to marginal gain** — this distinction should be noted before expert review. Section: metabolic claims (Part Eight, Basal Metabolism Tracking).

[R2] Karastergiou K, Smith SR, Greenberg AS, Fried SK. Sex differences in human adipose tissues — the biology of pear shape. Biol Sex Differ. 2012;3(1):13. doi: 10.1186/2042-6410-3-13
Supports fat distribution being driven by sex steroids, intrinsic adipocyte differences, and developmental factors. **Does not address food-type effects on distribution** — which is why the claim it supports was narrowed on 2026-09-03 from a broad "no food lands anywhere, calorie for calorie" to the food-type statement the source actually covers. Section: reactive physiological education, Part Nine.
...
> **STATUS: Scoped and decided 1 September 2026. Research pass complete and the architectural gate closed on 4 September 2026 — see "Custom LLM: decided" below. Server side built 4 September (adapter, session route, agent configured, latency measured). Mobile UI built 4 September (`c7e0c28`): the mic in the composer with its four states, the consent sheet, the conversation provider at the root, and the token-to-session path. **A real spoken conversation was confirmed working on a device the same evening** — long-press, consent, Freya answering. Bugs were reported at that session's close and never written down; that is the outstanding item, not the build.**
>
...
### Three modes, and all three ship

**Part Eighteen and build item 34 are complementary, not alternatives.** They answer different questions, and the earlier "superseded in scope" note was too blunt — item 34 is not cancelled and remains a planned build item.

| Mode | How it starts | What it is for |
|---|---|---|
| **Voice note** (item 34) | **Tap** the mic | Speak, the transcript appears, the person reviews it and taps send. No AI conversation is started. For logging something fast. |
| **Full conversation** (Part Eighteen) | **Long-press** the mic | Fluid voice in, voice out; Unflump answers by voice. For talking something through. |
| **Text** | Type | Existing, unchanged. |

The tap/long-press split is what makes one mic icon carry both without a mode picker: the quick thing is the quick gesture. Someone logging a meal on the way out of the kitchen should not have to end a conversation afterwards, and someone who wants to think out loud should not have to press send between turns.
...
**Claude is a first-class LLM option in ElevenLabs Agents, selected directly in the dashboard. No proxy and no workaround.** This was the one research question that could have invalidated the whole design, and it does not.

**The models actually offered** (read from the docs on 1 September, and *not* what was assumed): **Claude Opus 4.8, Claude Opus 4.7, Claude Sonnet 5, Claude Sonnet 4.6, Claude Sonnet 4.5, Claude Haiku 4.5.** Note there is no "Sonnet 4" in the list — the earliest Sonnet offered is 4.5, and the only Haiku is 4.5. Recorded precisely because a stale model name in a spec becomes a dashboard selection that does not exist.

**Recommendation: Claude Haiku 4.5 for the voice path** — optimised for low latency, which is what a spoken exchange actually needs, sufficient for body-data logging conversation, and significantly cheaper than Sonnet. ElevenLabs' own hosted models have lower latency still, but Haiku 4.5 is the right Claude option for real-time voice.

**There is also a custom-LLM option**, and the choice between the two is a real architectural decision rather than a preference:
...
**What actually ended the calls: an empty answer, not the screen (2026-09-12).** Read from ElevenLabs' own records and the chat thread, four calls that evening ended as "custom_llm generation failed: Brain returned no response", and the first report ("the phone goes to sleep and cuts it off") was one of them. In every case the pipeline had answered in 3 to 9 seconds and written the reply. Ruth paused mid-sentence; ElevenLabs sent the half ("Yes, please."), then dropped that answer unspoken when she carried on and sent the whole ("Yes, please. Yes, save it, please."). The adapter's restatement guard (2026-09-10) saw the half as a prefix of the whole and answered with an empty completion, which ElevenLabs treats as a failed generation and hangs up on. A routine was saved and never confirmed aloud. **Fixed in the adapter:** a restatement now replays the answer already written for the turn it restates, waiting up to 20 seconds for it, and no spoken turn can be empty; the pipeline still runs once per turn. **Still open:** (1) the same splitting double-logged a 90-minute ballet session, because "90 minutes." is under the guard's 12-character floor; (2) the agent's turn detection was on "normal"; changed to "patient" the same evening on Ruth's yes to the exact change (`{"conversation_config": {"turn": {"turn_eagerness": "patient"}}}`, agent version agtvrsn_7401m2bnsm3ce3j8fgzzr01psd4r), so fewer sentences are split in the first place; (3) an empty completion still answers pure silence ("..."), and may end the call the same way, where the agent's skip_turn tool is the likely proper answer; (4) ElevenLabs keeps every conversation with no retention limit, and each record holds the person's Supabase access token in plain text (short-lived, about an hour). Ruth agreed it is set before any tester uses voice; not yet changed.

**The second test, the same evening, and what changed after it.** A six-minute call held through her pauses. The offer the app now asks was heard, "yes, please, save it" kept the symptom, and the confirmation was spoken. Three things were still wrong, and all three were fixed after it on Ruth's yes:

1. **Replaying the first half's answer dropped what she said after the pause.** "What is a Baker's cyst?" was never answered, and a "yes, save it" survived only because a comma defeated the prefix match. The adapter now decides by whether she HEARD the last answer, which it can tell because ElevenLabs sends the conversation as it happened with every request. The same words sent again replay the answer being written. More words get the whole sentence answered, once the half-answered turn has finished, with the pipeline told its first answer went unheard and not to log, save or correct the same thing again; an offer made only in that unheard answer is dropped. This replaces the prefix match and its 12-character floor, so "90 minutes." then "90 minutes. Log it." is one turn now. The remaining risk is stated: the model can still repeat a log despite the instruction.
2. **The call ended as a failure on a "..." after her goodbye**, which settles the open question: an empty completion ends the call. Silence is now answered with the agent's skip_turn tool, added to the agent on Ruth's yes (version agtvrsn_7201m2bq49jjfc5b0k40fjqf47e6, end_call sent unchanged alongside it).
3. **"Can you close the chat?" could not close it.** The pipeline is now told it is a spoken conversation and sets endVoiceSession when she asks to stop, close or switch to text, or says goodbye; the adapter speaks the goodbye and then calls the agent's end_call tool.

The helpers live in `app/lib/voice-turns.ts`, probed by `scripts/probe-voice-turns.mjs` (18 checks). **Unverified until the next call:** that ElevenLabs sends the tools on every request and acts on streamed tool calls as its documentation describes, and that it finishes speaking the goodbye before ending. A tool it did not offer is never called, so if the tools do not arrive, silence falls back to the old empty answer and a goodbye leaves the call open, as before.
