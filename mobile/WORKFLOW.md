...
4. ~~Still manual: the session log in the Google Sheet.~~ **RETIRED 2026-09-09. Nothing in the close-out is manual any more.** The Checklist and Decisions Log are appended automatically (see **Automated close-out** below), and the session log this step described turned out to be **the one artefact that could not be automated and also did not exist**. Two separate reasons, and both are worth keeping:
   ...
   **And every field it listed is already captured, in that same Word build log.** Its header line carries session number, date, location, start time and tools; the close-out workbook carries what was done, what is next and the notes, in two continuous sheets. Duration and cumulative time come from **Toggl exports, batched** — not per session — which is why the build log leaves an em-dash placeholder rather than a blank waiting to be filled tonight. **Do not re-add a manual step here.** If something genuinely needs recording that these two artefacts do not hold, add a column to the close-out workbook, which is written automatically and lives on a drive that can actually be written to.
