...
- **GATE ON THE PENDING NATIVE BUILD BATCH: the Vercel project rename is blocked behind it (set 2026-09-03).** The rename turns off `unflump-app.vercel.app`, which is the host every installed build still calls. Three things must be true first, in order, and none of them is true yet:
  1. `EXPO_PUBLIC_API_URL` set to `https://api.selodia.app` **in EAS**, not only in `.env.local`. `eas env:list --environment production` currently returns nothing, and EAS Build clones from git where `.env.local` is ignored, so a production build today has no source for this value at all.
  2. A new build shipped carrying that value.
  3. That build confirmed working on a device against `api.selodia.app`.
  Only then is the rename safe. **The target name is `selodia-app`, not `selodia`** — it matches the GitHub repository, which was renamed to `ruthinprogress/selodia-app` on 2026-09-03. `api.selodia.app` survives the rename either way, because a custom domain is attached to the project rather than to its name.
