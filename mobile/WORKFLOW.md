...
- **Branding/visual design exploration** — its own separate Drive folder (`…/Build Specs/Branding`), not part of the technical spec. **Mockup-file convention:** mockup HTML/image files here use **dated filenames** (e.g. `unflump-all-screens_YYYY-MM-DD.html`), and once a new one is confirmed as the current reference, the previous undated or differently-dated version is **trashed**. This is the same "one canonical version, no competing sources of truth" discipline applied to the technical docs — but achieved via dated-filename-plus-trash rather than the technical docs' overwrite-in-place-under-a-fixed-name. (No contradiction with the "never dated copies" rule above: that rule governs the *Claude Code Working Build Specs* folder; this governs the separate *Branding* folder, where keeping iterations briefly distinguishable while a new one is confirmed is worth the dated name. The end state is the same either way: exactly one current reference, nothing stale competing with it.)
