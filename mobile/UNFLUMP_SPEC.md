...
**Preferred approach — inference, not a new manual input (build item 33; feasibility to confirm before building):** the parse step already understands free-text activity descriptions and very likely already reads intensity language ("gentle", "easy", "vigorous", "to failure", "all-out"). So the tag should, where feasible, be **inferred at log time** from the description (the log-time classification principle, Part Two principle 13) and stored on `activity_logs`, rather than adding a manual control — to be confirmed by a feasibility check first. Not yet built.
