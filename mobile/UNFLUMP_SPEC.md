...
**Not yet built at all:** the itemized food breakdown, the Almanac, the Daily and Weekly Roundups, push notifications, the Fat Focus/Muscle Focus category model beyond its schema, and everything else described in Parts Five through Nine below. (Cycle tracking's discovery + logging foundation, step 9, is now built — see the bullet above; its interpretation layer is step 19. Real authentication, step 5, is done — see the auth bullet above; the remaining auth-side gap is the session-restoration listener, step 6.)
