...
- **Tab-label rename pending (decided 2026-08-16):** the middle tab is now labelled **"Body"** in the design (Part Five, Screen Structure), person-outline icon unchanged. The current code still labels it **"Dashboard"** (`app-tabs.tsx`, `dashboard.tsx`, the switcher host), so the build-status references above that say "Dashboard" describe the current code accurately; renaming the code/UI (and, optionally, the file) to "Body" is a tracked follow-up, not yet done.
...
32. Overview "personal line" (with a separate Almanac variant) — a small, warm, quiet line that **never reads as a motivational quote or slogan** (the "personal, never generic" principle). Placement + the hard tone constraint are settled, and a candidate ~10-phrase MVP shortlist (Chat/Overview) plus a separate Almanac set now exist (2026-08-17); the **content-selection logic** remains separate future work. See The Overview Personal Line. Not yet built.
