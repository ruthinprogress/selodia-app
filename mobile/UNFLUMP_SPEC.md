...
11. Itemized food breakdown and its rules. **Slices 1 + 2a built 2026-08-14.** Slice 1: `food_items` child table + `breakdown_type` on `food_logs`; both parse paths classify the meal type and store the model's itemised components silently (rules 1-2 and the structural half of 3-4). Slice 2a: the consistent-ratio confirm loop (rule 3) — log-then-clarify-then-refine, with `food_logs.clarification_pending` pinning the question and a re-parse-in-place update on the answer; backend/conversational, recency-guarded. **Slice 2b** (rule 4: high-variability item-by-item verification, reusing 2a's mechanism) is the remaining, independently-resumable unit. Display is item 13; protein-quality flagging over items is item 12.
