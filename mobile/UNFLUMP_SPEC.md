...
- **Interpretation layer — time-of-day flagger built (2026-08-14, step 19, fourth slice):** from prior readings' `measured_at`, the user's usual reading time is derived as a circular mean of hours (UTC, so a constant offset cancels), guarded by a concentration check so a scattered logger isn't given a false "usual"; the latest reading is flagged as less comparable when it lands more than ~3h off it (Reliability Framework: consistent conditions). Needs 3+ prior readings; composes with cycle and pump. Remaining step-19 slices: DOMS (needs training-type detection to tell leg/lower-body sessions apart), then sodium (its own project — sodium isn't captured yet, so it needs a `parse-food` capture decision first).
