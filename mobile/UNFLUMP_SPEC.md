...
34. Voice input in Chat (mic button) — the Chat composer's mic (see Core Interaction Model). **Undecided and needs a real decision before building:** is it trivial OS dictation (effectively free — the native keyboard's own mic) or an intended first-class voice-input feature (recording → transcription → the same parse path)? Scope the intent first; don't assume. Not yet built.
