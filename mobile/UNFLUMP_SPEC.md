...
**Implementation status (2026-08-14, build item 11, slice 1):** the itemisation *structure* is built — a `food_items` child table and a `breakdown_type` on `food_logs`, with both parse paths (`app/lib/food-logging.ts` text/chat, `app/api/parse-food/route.ts` photo) classifying the meal type and storing the model's component breakdown in one silent pass. This covers rules 1-2 and the structural half of 3-4. The *interactive* half — confirming the one variable that matters (consistent-ratio) and verifying item by item with the quantity-confidence fallback (high-variability) — is **slice 2**, a separate resumable unit not yet built; until then the breakdown is the model's best single-pass guess. The breakdown's *display* (the eye-icon "What's In Here" card) is build item 13; protein-quality flagging over the items is item 12.
...
11. Itemized food breakdown and its rules. **Slice 1 built 2026-08-14** (`food_items` child table + `breakdown_type` on `food_logs`; both parse paths classify the meal type and store the model's itemised components silently — rules 1-2 and the structural half of 3-4). **Slice 2** (interactive clarification — confirm the variable for consistent-ratio, verify item-by-item with the quantity-confidence fallback for high-variability) is the remaining, independently-resumable unit. Display is item 13; protein-quality flagging over items is item 12.
