...
Targets are overrideable (like protein) and recompute as BMR/TDEE and bodyweight update. **Status:** the combination logic is decided (below) and the **pure matrix calc is built** — `mobile/src/lib/calorie-target.ts` (`calculateCalorieTarget`, node-tested against all 9 fat×muscle combinations + the deficit arithmetic and null/edge cases). **Still open:** TDEE *sourcing* on the client (a small BMR/TDEE helper — scale `bmr` preferred, Mifflin-St Jeor estimate as fallback, × activity multiplier; mirroring `app/lib/body-metrics.ts` across the Next/Expo boundary) and the Overview UI wiring. The mockup's 1,700 kcal is an illustrative placeholder until that wiring lands.
