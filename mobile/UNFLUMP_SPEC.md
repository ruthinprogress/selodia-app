...
## Historical Browsing
Recent/current-week views use the same minimized weekly table described in Part Eight. To view further back, a quiet "view another week" control (not a popup, never nagging at a fixed threshold) steps back one week at a time, shown in the same weekly table component, never displaying more than about seven rows at once. Navigation is deliberately week-by-week: there is **no open date picker or calendar jump to an arbitrary date** — stepping one week at a time maintains day-to-day continuity and keeps the interaction from feeling like scrubbing a database. Returning to the current week is the dedicated "back to present" button below, not a date selector. No separate table design is needed for "old" versus "current" data — one component, reused. (Clarified 2026-08-15: week-by-week only, no date picker — reconciling an earlier verbal agreement that had only partially reached the spec, superseding the prior calendar/month-picker wording.)
