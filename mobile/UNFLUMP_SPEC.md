...
- **Dashboard** — logged data across food, body, and activity, accessed via an **in-screen switcher** rather than separate top-level tabs for each. The switcher has four segments: **Overview** (the default landing — lightweight summary cards spanning all facets, each tapping through into the relevant detail segment), **Measurements** (full body/measurement detail and its interpretation), **Food** (the full weekly table — see Part Five's "What's In Here" and Historical Browsing), and **Activity** (movement detail). Overview is a genuine summary in its own right, deliberately distinct from the detail segments — not any one facet's detail doing double duty. Only segments with real content are shown as they are built (no empty segments, per Part Two principle 8). A neutral human silhouette icon (chosen because it represents food, body, and activity together as facets of one body — not a chart icon, which would read as clinical analytics rather than personal data).
...
For a genuine far jump, a **deliberately-invoked month/year picker** is the single far-jump entry point — it opens only when the user taps to reach for it, is never persistent, and is styled on-brand rather than as a native OS date picker: square month buttons and a year scroller in the app's palette. Selecting a month opens that month's weeks; selecting a week lands on the Food log tab at that week, after which normal day-by-day stepping (Monday, Tuesday, Wednesday…) works from there exactly as it does anywhere else. Ordinary browsing never needs the picker; it exists solely for the intentional far jump.
...
12. Protein quality flagging UI — needs itemized breakdown (step 11). **Built 2026-08-14:** `protein_source` classified at parse (log + per-item, principle 13's log-time classification); the day-level nudge (>½ incomplete → complementary pairing + buffered target) on the Dashboard protein card, pure logic node-tested. Per-item flags (collagen/plant) ride item 13's breakdown display using the captured source.
