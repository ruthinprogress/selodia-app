...
- Muscle-mass-based protein calculation and basal metabolism (Phase 1 steps 2–3): calculation utilities (`protein.ts`, `basal-metabolism.ts`) done, and now **surfaced as understated stats on the Dashboard** (`dashboard.tsx`) — the protein target with its source note, and latest BMR with a trend delta and muscle overlay. **The full visual trend chart is deliberately deferred** (needs a charting/SVG library — a separate native addition; cosmetic per principle 10), so step 3's numeric surfacing is done but its chart is not.
...
**Not yet built at all:** the itemized food breakdown, the Almanac, the Daily and Weekly Roundups, push notifications, the Fat Focus/Muscle Focus category model beyond its schema, and everything else described in Parts Five through Nine below. (Cycle tracking's discovery + logging foundation, step 9, is now built — see the bullet above; its interpretation layer is step 19. Real authentication (step 5) and the auth-state listener / route guard (step 6) are both done — see the auth bullets above.)
...
## Historical Browsing
Recent/current-week views use the same minimized weekly table described in Part Eight, and **week-by-week stepping is the calm default for ordinary browsing**: a quiet "view another week" control (not a popup, never nagging at a fixed threshold) steps back one week at a time in the same weekly table component, never displaying more than about seven rows at once — day-to-day continuity, with one component reused for "old" and "current" data alike.
...
Targets are overrideable (like protein) and recompute as BMR/TDEE and bodyweight update. **Status:** the combination logic is decided (below) and the **pure calc + TDEE sourcing are built**: `mobile/src/lib/calorie-target.ts` (`calculateCalorieTarget`, node-tested against all 9 fat×muscle combinations + deficit arithmetic + null/edge cases) and `mobile/src/lib/body-metrics.ts` (`resolveTDEE` — scale `bmr` preferred, Mifflin-St Jeor estimate as fallback, × activity multiplier; a client mirror of `app/lib/body-metrics.ts`'s BMR/TDEE primitives, guarded by a server/client parity node-test). The two compose at the call site: `resolveTDEE(...)` → `calculateCalorieTarget({ tdeeKcal, ... })`. **Still open:** the Overview UI wiring (fetch scale `bmr`/weight + the profile fields + focus states, chain the two functions, render on the calorie bar). The mockup's 1,700 kcal is an illustrative placeholder until that wiring lands.
...
- Every exercise shows **sets AND reps, not just working weight** (e.g. "4×8 · working weight 60kg"). Sets/reps are **computed per user, per goal, from the chat conversation** — a different rep range for "building muscle" than for other goals — never a fixed default per exercise.
- The **exercise detail** view holds, in order:
  - a **"Before you start" safety note card** — **exercise-specific** commentary on that movement's actual common failure modes, **never generic safety boilerplate**. E.g. Romanian deadlift: start with a low weight, stop on any lower-back twinge or discomfort, push the hips back as far as possible, hinge from the hips only. Every exercise card requires this.
...
15. The Almanac. **Data-layer design settled 2026-08-17 (scoping only, not built) — the plan-as-content / progress-as-logs pattern from item 35, generalized:**
    ...
    - **Distinct from `user_context`:** a new table for a different job — `user_context` stays for atomic, prompt-injected durable facts (allergies, goals); `almanac_entries` are browsable reference documents. The **write pattern is reused** (an LLM-emitted structured save, like `[REMEMBER: …]`), not the table.
...
35. Structured workout tracking with progressive-overload — the interactive Almanac Workouts depth (see Part Ten, Workouts): program-type-driven conditional grouping (strength→body area, rehab→flat, skill→by skill, none→plain list), per-user per-goal computed sets × reps, and an exercise-specific "Before you start" safety note card (real failure modes, never boilerplate). **Design settled 2026-08-17 (data-model mechanics); gated on item 15 (the Almanac data model) — a workout plan IS an Almanac entry, so it can't be built until "what an Almanac entry is" exists.** The settled model separates two genuinely different things, which resolves the mockup controls' apparent clash with the Almanac's chat-only editing rule:
   ...
   - **Q1(c) reuse:** a completed plan exercise also writes an **`activity_logs`** row, so a resistance session feeds the existing DOMS / eccentric-load interpretation (item 27) for free.
   - **New schema:** the working-weight history + completion logs, and the plan structure (likely JSONB on an Almanac entry, once item 15 defines that). **Reuse:** `activity_logs` + `eccentric_load` (item 27), the discuss-card "Ask about this" (already entry-type-agnostic), and the chat-only editing rule for plan *content*.
   Not yet built — and correctly ordered after item 15.
36. Per-exercise movement-demo assets — the licensed-animation demonstrations for Almanac movements (per Part Ten, Exercise demonstration content). **Coverage is every single exercise (a competitive floor, not optional)**; sourcing is licensed animated content, and **existing project licensing research should be located and reused, not redone**. The asset/embed strategy (which library, how movements map to assets, exact licensing) is still to finalise. Batches with the other native-additions rebuild. Not yet built.
