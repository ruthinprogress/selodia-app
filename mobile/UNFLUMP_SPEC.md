...
- **Interpretation layer — DOMS flagger built (2026-08-14, step 19, fifth slice):** a leg/eccentric session 24-72h before the reading is flagged for delayed-onset soreness swelling (Reliability Framework). DOMS is eccentric loading, not a training "category" — so the flat keyword proxy spans eccentric leg resistance (squat, deadlift, lunge, glute…) *and* intensity/terrain-qualified cardio (hill, hike, stairs, sprint, trail, "long run/ride"), while bare/easy cardio (run, walk, cycle) is excluded; "long" is phrase-only so "long walk" doesn't match. It's a deliberately imperfect lexical proxy for a semantic signal, justified by the benign failure surface (a miss = no worse than no flag; a false positive is still reassurance in the safe direction, outranked by the trend engine). The activity input was generalised to `{ happenedAt, activityType }` so pump and DOMS share it; the Dashboard activity query widened to ~80h. The principled accuracy upgrade, if ever needed, is classifying eccentric load at log time in `parse-activity`. Remaining step-19 slice: sodium (its own project — sodium isn't captured yet, so it needs a `parse-food` capture decision first).
