...
- **Insight-card pattern** — sage background, logo mark, Save / Not now: the single consistent way Unflump ever surfaces a pattern for saving to the Almanac.
...
**Workouts — a worked category (2026-08-16 mockup).** "Workouts" is a real example of an interactive Almanac category page, linked to a saved training plan (e.g. the "Posterior-chain training plan" entry). It shows how a structured Almanac category looks:
- Each **workout row** carries a **checkbox** (tick when the movement is done) and an **eye icon** (opens the workout detail — the discuss-card/detail pattern).
- The **workout detail** view holds: the **movement-demo animation** (per Exercise demonstration content above — a licensed animation, not AI-generated; a clearly-labelled placeholder until the real asset lands); an **"Ask about this"** button (the entry-type-agnostic discuss-card mechanic, routing the movement into Chat as context); and a **working-weight control** — a "+2.5kg" chip beside the current working weight that, when tapped, **updates this Almanac entry**. That write-back is the mechanism for tracking **progressive overload** over time, which matters specifically for the resistance-training side of a build-muscle focus.

The interactive depth here — done-checkboxes, working-weight logging that **writes back to the Almanac entry**, and per-movement demo assets — is **feature-depth beyond visual design and needs its own scoping** (build items 35 and 36). This records the design, not a build-ready spec.
...
32. Overview "personal line" — a small, warm line reflecting something genuinely specific to the user (a saved Almanac pattern, something recently discussed), **never a generic rotating affirmation** (the "personal, never generic" principle — whoosh personalization, food-and-joy tracing). This item is placement/scoping; the content-selection logic is real, separate future work, dependent on enough saved Almanac/pattern data. See The Overview Personal Line. Not yet built.
...
34. Voice input for logging (Chat mic button) — **confirmed a real, intended feature (2026-08-16), not incidental OS dictation:** someone can *speak* a food or activity entry instead of typing it. **Architecture:** capture speech and transcribe it, then feed the transcribed text into the **exact same text pipeline already built** (intent classification, parsing, safety checks) — no new understanding logic, only speech capture + speech-to-text on the front. Because it needs microphone access and speech-to-text, this is a genuine **native-dependency addition**, so it **joins the existing Beta Tester 1 native batch** (alongside `expo-image-picker`, the Almanac animation library, and `expo-notifications`) and ships in that rebuild rather than over-the-air via EAS Update. Not yet built.
35. Structured workout tracking with progressive-overload write-back — the interactive Almanac Workouts depth (see Part Ten, Workouts): per-movement done-checkboxes and a working-weight control whose "+2.5kg"-style adjustment **writes back to the Almanac entry**, so progressive overload is tracked over time. Real feature-depth beyond visual design (a data model for per-movement working weights + their history, edit-through-chat consistency per the Almanac's single-entry-point rule); needs its own scoping. Not yet built.
36. Per-exercise movement-demo assets — the licensed-animation demonstrations for Almanac movements (per Part Ten, Exercise demonstration content). The asset/embed strategy (which library, how movements map to assets, licensing) is undecided; the mockup shows a labelled placeholder. Batches with the other native-additions rebuild. Not yet built.
