...
- The **exercise detail** view holds, in order:
  ...
  - a **working-weight control** — a "+2.5kg" chip that, when tapped, **updates this Almanac entry** (progressive-overload tracking over time, for the resistance-training side of a build-muscle focus).
...
35. Structured workout tracking with progressive-overload write-back — the interactive Almanac Workouts depth (see Part Ten, Workouts): per-movement done-checkboxes and a working-weight control whose "+2.5kg"-style adjustment **writes back to the Almanac entry**, so progressive overload is tracked over time. Now also carries (2026-08-17): **program-type-driven conditional list grouping** (strength→body area, rehab→flat, skill→by skill, none→plain list); **per-user, per-goal computed sets × reps** (not a fixed default per exercise); and an **exercise-specific "Before you start" safety note card** (real failure modes, never boilerplate). Real feature-depth (a data model for per-movement working weights + their history + computed sets/reps, edit-through-chat consistency per the single-entry-point rule); needs its own scoping. Not yet built.
