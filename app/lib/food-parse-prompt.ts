...
export const FOOD_PARSE_CLASSIFICATION_RULES =
  ...
  'For protein_source (on the log and on each item), classify the dominant protein source as "animal" (meat, fish, eggs, dairy, whey), "plant" (legumes, tofu, grains, nuts, seeds), or "collagen" (collagen or gelatin supplements), or null when the food has negligible protein; on the log, use whichever source contributes most of the protein. EVERY DISTINCT THING THEY MENTIONED GETS ITS OWN ROW IN items, ALWAYS, and items is NEVER empty. "Mug of tea and a cookie" is two rows. "Herbal tea, English tea with milk, and a biscuit" is three. A single thing is one row - itself. This is not a judgement about whether a breakdown is interesting: it is the record, and a figure stored as one lump can never be separated afterwards, so anything not itemised at this moment is lost for good. breakdown_type says only how far to go INSIDE a single dish, and never whether to itemise at all: "simple" for one plain or branded thing like an apple or a branded yoghurt (one row, the thing itself); "multi_component" for a meal of distinct parts like steak with a sauce (a row per part); "consistent_ratio" for a composite whose make-up is reliably the same, like a lasagne (one row for the dish, not its ingredients); "high_variability" for a composite that really varies like shakshuka or a full English (a row per part, with quantities). When somebody lists several things, the type describes each of them; it never licenses merging them. Item macros should roughly sum to the totals; use the person\'s own portion words for quantity, or a typical portion if none given. Each item name must be SPECIFIC enough to be read on its own in a table - "white olive bread + butter" and "scrambled eggs x2", never a bare "bread" or "eggs", and never a category like "dairy" or "carbs". Keep the person\'s own words for what a thing was where they gave them. Include a zero-calorie item they mentioned (a black coffee, a tea) as its own row rather than dropping it: a breakdown that silently omits part of what they said reads as an error, not a tidy-up. For amino_profile (on the log and on each item), say which essential amino acid that food is LIMITING in, which is what decides whether two foods complete each other: "complete" for animal protein, and also for soy, quinoa, buckwheat and amaranth, which are complete in their own right; "limiting_methionine" for legumes (beans, lentils, peas); "limiting_lysine" for grains, nuts and seeds; "limiting_tryptophan" for collagen and gelatin, which are very low in it. Use null when the food carries negligible protein - a lettuce leaf or a black coffee has no amino profile worth stating. Judge the FOOD, not the meal: whether the meal as a whole is complete is worked out afterwards from these values, so never adjust one item\'s profile to account for another item.';
